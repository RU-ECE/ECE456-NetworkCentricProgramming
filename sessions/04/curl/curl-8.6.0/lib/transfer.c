 ...
 */
static ssize_t Curl_xfer_recv_resp(struct Curl_easy* data, char* buf, size_t blen, bool eos_reliable, CURLcode* err) {
	ssize_t nread;
 ...
 * buffer)
 */
static CURLcode readwrite_data(struct Curl_easy* data, struct SingleRequest* k, int* didwhat, bool* done) {
	struct connectdata* conn = data->conn;
	...
	*done = FALSE;
	...
	do {
		...
		nread = Curl_xfer_recv_resp(data, buf, bytestoread, is_multiplex, &result);
		if (nread < 0) {
	...
	if (((k->keepon & (KEEP_RECV | KEEP_SEND)) == KEEP_SEND) && (conn->bits.close || is_multiplex)) {
...
static CURLcode readwrite_upload(struct Curl_easy* data, struct connectdata* conn, int* didwhat) {
	...
	*didwhat |= KEEP_SEND;
	...
	do {
		...
		if (0 != k->upload_present && k->upload_present < curl_upload_refill_watermark(data) &&
...
static int select_bits_paused(struct Curl_easy* data, int select_bits) {
	...
	if (((select_bits & CURL_CSELECT_IN) && !(data->req.keepon & KEEP_RECV_PAUSE)) ||
	...
	if (k->keepon) {
		...
		if (!(data->req.no_body) && (k->size != -1) && (k->bytecount != k->size) &&
			...
			*/
			...
			!k->newurl) {
			failf(data, "transfer closed with %" CURL_FORMAT_CURL_OFF_T " bytes remaining to read",
				  k->size - k->bytecount);
					 ...
					 followtype type) /* see transfer.h */
{
	...
	if ((type != FOLLOW_RETRY) && (data->req.httpcode != 401) && (data->req.httpcode != 407) &&
	...
	case 301: /* Moved Permanently */
		 ...
		 */
		if ((data->state.httpreq == HTTPREQ_POST || data->state.httpreq == HTTPREQ_POST_FORM ||
	...
	case 303: /* See Other */
		...
		if (data->state.httpreq != HTTPREQ_GET &&
			((data->state.httpreq != HTTPREQ_POST && data->state.httpreq != HTTPREQ_POST_FORM &&
			  data->state.httpreq != HTTPREQ_POST_MIME) ||
...
CURLcode Curl_xfer_write_resp(struct Curl_easy* data, char* buf, size_t blen, bool is_eos, bool* done) {
	CURLcode result = CURLE_OK;
	...
	if (data->conn->handler->write_resp) {
		...
		if (blen || is_eos) {
			...
			if (blen && data->conn->handler->protocol & PROTO_FAMILY_POP3)
				result = data->req.ignorebody ? CURLE_OK : Curl_pop3_write(data, buf, blen);
			else
