...
#ifdef CURLDEBUG
static const char* statenames[] = {"MQTT_FIRST",		 "MQTT_REMAINING_LENGTH", "MQTT_CONNACK",	 "MQTT_SUBACK",
								   "MQTT_SUBACK_COMING", "MQTT_PUBWAIT",		  "MQTT_PUB_REMAIN",
...
static void mqstate(struct Curl_easy* data, enum mqttstate state, enum mqttstate nextstate) /* used if state == FIRST */
{
	...
	struct mqtt_conn* mqtt = &conn->proto.mqtt;
#ifdef CURLDEBUG
	infof(data, "%s (from %s) (next is %s)", statenames[state], statenames[mqtt->state],
