...
static void tcpkeepalive(struct Curl_easy* data, curl_socket_t sockfd) {
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, (void*)&optval, sizeof(optval)) < 0) {
		infof(data,
			  "Failed to set SO_KEEPALIVE on fd "
		...
		if (WSAIoctl(sockfd, SIO_KEEPALIVE_VALS, (LPVOID)&vals, sizeof(vals), NULL, 0, &dummy, NULL, NULL) != 0) {
			infof(data,
				  "Failed to set SIO_KEEPALIVE_VALS on fd "
				  "%" CURL_FORMAT_SOCKET_T ": errno %d",
				  sockfd, SOCKERRNO);
		}
#else
		...
		if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPALIVE, (void*)&optval, sizeof(optval)) < 0) {
			infof(data,
				  "Failed to set TCP_KEEPALIVE on fd "
				  "%" CURL_FORMAT_SOCKET_T ": errno %d",
				  sockfd, SOCKERRNO);
		}
#endif
...
static CURLcode bindlocal(struct Curl_easy* data, struct connectdata* conn, curl_socket_t sockfd, int af,
						  unsigned int scope) {
...
static void cf_socket_close(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (ctx && CURL_SOCKET_BAD != ctx->sock) {
		CURL_TRC_CF(data, cf, "cf_socket_close(%" CURL_FORMAT_SOCKET_T ")", ctx->sock);
	...
	if (is_tcp_fastopen) {
		...
		if (__builtin_available(macOS 10.11, iOS 9.0, tvOS 9.0, watchOS 2.0, *)) {
			...
			rc = connectx(ctx->sock, &endpoints, SAE_ASSOCID_ANY,
						  CONNECT_RESUME_ON_READ_WRITE | CONNECT_DATA_IDEMPOTENT, NULL, 0, NULL, NULL);
		} else {
...
static CURLcode cf_tcp_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done) {
	...
	*done = FALSE; /* a very negative world view is best */
	if (ctx->sock == CURL_SOCKET_BAD) {
		...
		error = SOCKERRNO;
		set_local_ip(cf, data);
		CURL_TRC_CF(data, cf, "local address %s port %d...", ctx->l_ip, ctx->l_port);
		if (-1 == rc) {
...
out:
	if (result) {
		if (ctx->error) {
			...
			{
				char buffer[STRERROR_LEN];
				infof(data, "connect to %s port %u from %s port %d failed: %s", ctx->r_ip, ctx->r_port, ctx->l_ip,
					  ctx->l_port, Curl_strerror(ctx->error, buffer, sizeof(buffer)));
			}
#endif
		}
		if (ctx->sock != CURL_SOCKET_BAD) {
...
static void cf_socket_get_host(struct Curl_cfilter* cf, struct Curl_easy* data, const char** phost,
...
static ssize_t cf_socket_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							  CURLcode* err) {
	struct cf_socket_ctx* ctx = cf->ctx;
	...
	*err = CURLE_OK;
	...
	if (cf->conn->bits.tcp_fastopen) {
		nwritten =
			sendto(ctx->sock, buf, len, MSG_FASTOPEN, &cf->conn->remote_addr->sa_addr, cf->conn->remote_addr->addrlen);
...
struct Curl_cftype Curl_cft_tcp = {
	"TCP",			 CF_TYPE_IP_CONNECT, CURL_LOG_LVL_NONE,		   cf_socket_destroy,			cf_tcp_connect,
	cf_socket_close, cf_socket_get_host, cf_socket_adjust_pollset, cf_socket_data_pending,		cf_socket_send,
	cf_socket_recv,	 cf_socket_cntrl,	 cf_socket_conn_is_alive,  Curl_cf_def_conn_keep_alive, cf_socket_query,
};

CURLcode Curl_cf_tcp_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn,
...
static CURLcode cf_udp_setup_quic(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	CURL_TRC_CF(data, cf, "%s socket %" CURL_FORMAT_SOCKET_T " connected: [%s:%d] -> [%s:%d]",
				(ctx->transport == TRNSPRT_QUIC) ? "QUIC" : "UDP", ctx->sock, ctx->l_ip, ctx->l_port, ctx->r_ip,
				ctx->r_port);
	...
	*done = FALSE;
	if (ctx->sock == CURL_SOCKET_BAD) {
		...
		if (ctx->transport == TRNSPRT_QUIC) {
			...
			if (result)
				goto out;
			CURL_TRC_CF(data, cf, "cf_udp_connect(), opened socket=%" CURL_FORMAT_SOCKET_T " (%s:%d)", ctx->sock,
						ctx->l_ip, ctx->l_port);
		} else {
...
struct Curl_cftype Curl_cft_udp = {
	"UDP",			 CF_TYPE_IP_CONNECT, CURL_LOG_LVL_NONE,		   cf_socket_destroy,			cf_udp_connect,
	cf_socket_close, cf_socket_get_host, cf_socket_adjust_pollset, cf_socket_data_pending,		cf_socket_send,
	cf_socket_recv,	 cf_socket_cntrl,	 cf_socket_conn_is_alive,  Curl_cf_def_conn_keep_alive, cf_socket_query,
};
...
struct Curl_cftype Curl_cft_unix = {
	"UNIX",			 CF_TYPE_IP_CONNECT, CURL_LOG_LVL_NONE,		   cf_socket_destroy,			cf_tcp_connect,
	cf_socket_close, cf_socket_get_host, cf_socket_adjust_pollset, cf_socket_data_pending,		cf_socket_send,
	cf_socket_recv,	 cf_socket_cntrl,	 cf_socket_conn_is_alive,  Curl_cf_def_conn_keep_alive, cf_socket_query,
};

CURLcode Curl_cf_unix_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn,
							 const struct Curl_addrinfo* ai, int transport) {
	struct cf_socket_ctx* ctx = NULL;
	struct Curl_cfilter* cf = NULL;
	CURLcode result;
...
CURLcode Curl_conn_tcp_accepted_set(struct Curl_easy* data, struct connectdata* conn, int sockindex, curl_socket_t* s) {
	...
	ctx->connected_at = Curl_now();
	cf->connected = TRUE;
	CURL_TRC_CF(data, cf, "accepted_set(sock=%" CURL_FORMAT_SOCKET_T ", remote=%s port=%d)", ctx->sock, ctx->r_ip,
				ctx->r_port);
...
static bool cf_is_socket(struct Curl_cfilter* cf) {
	return cf &&
		(cf->cft == &Curl_cft_tcp || cf->cft == &Curl_cft_udp || cf->cft == &Curl_cft_unix ||
		 cf->cft == &Curl_cft_tcp_accept);
}

CURLcode Curl_cf_socket_peek(struct Curl_cfilter* cf, struct Curl_easy* data, curl_socket_t* psock,
							 const struct Curl_sockaddr_ex** paddr, const char** pr_ip_str, int* pr_port,
