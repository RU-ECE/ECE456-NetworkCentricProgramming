...
static void suboption(struct Curl_easy* data) {
	...
	case CURL_TELOPT_TTYPE:
		...
		msnprintf((char*)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_TTYPE, CURL_TELQUAL_IS,
				  tn->subopt_ttype, CURL_IAC, CURL_SE);
	...
	case CURL_TELOPT_XDISPLOC:
		...
		msnprintf((char*)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB, CURL_TELOPT_XDISPLOC, CURL_TELQUAL_IS,
		...
		for (v = tn->telnet_vars; v; v = v->next) {
			...
			if (len + tmplen < (int)sizeof(temp) - 6) {
				...
				if (!s) {
					...
					len += msnprintf((char*)&temp[len], sizeof(temp) - len, "%c%.*s%c%s", CURL_NEW_ENV_VAR, (int)vlen,
									 v->data, CURL_NEW_ENV_VALUE, ++s);
				}
			}
		}
	...
	while (keepon) {
		...
		default: /* read! */
			...
			if (poll_cnt == 2) {
				...
				/* read from user-supplied method */
				nread = (int)data->state.fread_func(buffer, 1, sizeof(buffer), data->state.in);
