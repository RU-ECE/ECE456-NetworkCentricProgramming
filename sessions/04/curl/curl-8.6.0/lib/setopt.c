...
CURLcode Curl_vsetopt(struct Curl_easy* data, CURLoption option, va_list param) {
...
#ifdef USE_SSL
	{
		...
		if (version < CURL_SSLVERSION_DEFAULT || version == CURL_SSLVERSION_SSLv2 || version == CURL_SSLVERSION_SSLv3 ||
			version >= CURL_SSLVERSION_LAST || version_max < CURL_SSLVERSION_MAX_NONE ||
	...
	case CURLOPT_COPYPOSTFIELDS:
		...
		if (!argptr || data->set.postfieldsize == -1) {
			...
			if ((data->set.postfieldsize < 0) ||
				((sizeof(curl_off_t) != sizeof(size_t)) && (data->set.postfieldsize > (curl_off_t)((size_t)-1)))) {
				result = CURLE_OUT_OF_MEMORY;
			} else {
	...
	case CURLOPT_USERPWD:
		 ...
		 */
		result =
			setstropt_userpwd(va_arg(param, char*), &data->set.str[STRING_USERNAME], &data->set.str[STRING_PASSWORD]);
		break;
	...
	case CURLOPT_PROXYUSERPWD:
		 ...
		 */
		result = setstropt_userpwd(va_arg(param, char*), &data->set.str[STRING_PROXYUSERNAME],
	...
	case CURLOPT_GSSAPI_DELEGATION:
		...
		data->set.gssapi_delegation =
			(unsigned char)uarg & (CURLGSSAPI_DELEGATION_POLICY_FLAG | CURLGSSAPI_DELEGATION_FLAG);
		break;
