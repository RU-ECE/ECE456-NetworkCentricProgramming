...
static CURLcode pop3_perform_command(struct Curl_easy* data) {
	...
	if (pop3->id[0] != '\0') {
		result = Curl_pp_sendf(data, &conn->proto.pop3c.pp, "%s %s",
							   (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command), pop3->id);
	} else {
...
static CURLcode pop3_state_starttls_resp(struct Curl_easy* data, struct connectdata* conn, int pop3code,
										 pop3state instate) {
	CURLcode result = CURLE_OK;
	(void)instate; /* no use for this yet */
...
static CURLcode pop3_state_command_resp(struct Curl_easy* data, int pop3code, pop3state instate) {
	...
	if (pop3->transfer == PPTRANSFER_BODY) {
		...
		if (pp->overflow) {
			...
			if (!data->req.no_body) {
				result = Curl_pop3_write(data, Curl_dyn_ptr(&pp->recvbuf), Curl_dyn_len(&pp->recvbuf));
				if (result)
					return result;
			}
