...
static void sasl_state(struct SASL* sasl, struct Curl_easy* data, saslstate newstate) {
	...
	static const char* const names[] = {
		"STOP",			  "PLAIN",			"LOGIN",	   "LOGIN_PASSWD",	"EXTERNAL", "CRAMMD5",
		"DIGESTMD5",	  "DIGESTMD5_RESP", "NTLM",		   "NTLM_TYPE2MSG", "GSSAPI",	"GSSAPI_TOKEN",
...
#if defined(USE_NTLM) || defined(USE_GSASL) || defined(USE_KERBEROS5) || !defined(CURL_DISABLE_DIGEST_AUTH)
...
CURLcode Curl_sasl_start(struct SASL* sasl, struct Curl_easy* data, bool force_ir, saslprogress* progress) {
	...
	const char* service =
		data->set.str[STRING_SERVICE_NAME] ? data->set.str[STRING_SERVICE_NAME] : sasl->params->service;
#endif
	...
	*progress = SASL_IDLE;
	...
	if ((enabledmechs & SASL_MECH_EXTERNAL) && !conn->passwd[0]) {
		...
		if ((enabledmechs & SASL_MECH_GSSAPI) && Curl_auth_is_gssapi_supported() &&
			...
			if (force_ir || data->set.sasl_ir) {
				result = Curl_auth_create_gssapi_user_message(data, conn->user, conn->passwd, service, conn->host.name,
			...
			if ((enabledmechs & SASL_MECH_SCRAM_SHA_256) &&
				Curl_auth_gsasl_is_supported(data, SASL_MECH_STRING_SCRAM_SHA_256, &conn->gsasl)) {
			...
			if (result == CURLE_OK && (force_ir || data->set.sasl_ir))
				result = Curl_auth_gsasl_token(data, &nullmsg, &conn->gsasl, &resp);
				   ...
				   Curl_auth_gsasl_is_supported(data, SASL_MECH_STRING_SCRAM_SHA_1, &conn->gsasl)) {
			...
			if (force_ir || data->set.sasl_ir) {
				result = Curl_auth_create_ntlm_type1_message(data, conn->user, conn->passwd, service, hostname,
															 &conn->ntlm, &resp);
			}
		} else
#endif
...
CURLcode Curl_sasl_continue(struct SASL* sasl, struct Curl_easy* data, int code, saslprogress* progress) {
	...
	*progress = SASL_INPROGRESS;
	...
	case SASL_NTLM:
		...
		result =
			Curl_auth_create_ntlm_type1_message(data, conn->user, conn->passwd, service, hostname, &conn->ntlm, &resp);
		...
		if (!result) {
			if (sasl->mutual_auth) {
				/* Decode the user token challenge and create the optional response
				   message */
				result = Curl_auth_create_gssapi_user_message(data, NULL, NULL, NULL, NULL, sasl->mutual_auth,
															  &serverdata, &conn->krb5, &resp);
				newstate = SASL_GSSAPI_NO_DATA;
			} else {
				/* Decode the security challenge and create the response message */
				result =
					Curl_auth_create_gssapi_security_message(data, conn->sasl_authzid, &serverdata, &conn->krb5, &resp);
			}
		}
		break;
