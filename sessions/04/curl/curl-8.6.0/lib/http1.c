...
static ssize_t next_line(struct h1_req_parser* parser, const char* buf, const size_t buflen, int options,
						 CURLcode* err) {
	ssize_t nread = 0;
...
static CURLcode start_req(struct h1_req_parser* parser, const char* scheme_default, int options) {
	...
	if (url) {
		...
		result =
			Curl_http_req_make(&parser->req, m, m_len, scheme, scheme_len, authority, authority_len, path, path_len);
	}
...
ssize_t Curl_h1_req_parse_read(struct h1_req_parser* parser, const char* buf, size_t buflen, const char* scheme_default,
...
CURLcode Curl_h1_req_write_head(struct httpreq* req, int http_minor, struct dynbuf* dbuf) {
	...
	result = Curl_dyn_addf(dbuf, "%s %s%s%s%s HTTP/1.%d\r\n", req->method, req->scheme ? req->scheme : "",
						   req->scheme ? "://" : "", req->authority ? req->authority : "", req->path ? req->path : "",
						   http_minor);
	if (result)
		goto out;
