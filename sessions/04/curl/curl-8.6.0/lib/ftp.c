...
static CURLcode ftp_readresp(struct Curl_easy* data, curl_socket_t sockfd, struct pingpong* pp, int* ftpcode,
							 size_t* size);
...
const struct Curl_handler Curl_handler_ftp = {
	...
	PROTOPT_DUAL | PROTOPT_CLOSEACTION | PROTOPT_NEEDSPWD | PROTOPT_NOURLQUERY | PROTOPT_PROXY_AS_HTTP |
...
static CURLcode AllowServerConnect(struct Curl_easy* data, bool* connected) {
	...
	*connected = FALSE;
	...
	if (*connected) {
		...
		Curl_expire(data, data->set.accepttimeout ? data->set.accepttimeout : DEFAULT_ACCEPT_TIMEOUT,
					EXPIRE_FTP_ACCEPT);
	}
...
static CURLcode ftp_readresp(struct Curl_easy* data, curl_socket_t sockfd, struct pingpong* pp,
...
static void _ftp_state(struct Curl_easy* data, ftpstate newstate
	...
	if (ftpc->state != newstate) {
		infof(data, "FTP %p (line %d) state change from %s to %s", (void*)ftpc, lineno, ftp_state_names[ftpc->state],
...
static CURLcode ftp_state_use_port(struct Curl_easy* data, ftpport fcmd) /* start with this */
{
	...
	for (; fcmd != DONE; fcmd++) {
		...
		if (EPRT == fcmd) {
			...
			result = Curl_pp_sendf(data, &ftpc->pp, "%s |%d|%s|%hu|", mode[fcmd], sa->sa_family == AF_INET ? 1 : 2,
								   myhost, port);
			if (result) {
...
static CURLcode ftp_state_quote(struct Curl_easy* data, bool init, ftpstate instate) {
	...
	if (!quote) {
		...
		case FTP_RETR_PREQUOTE:
			if (ftp->transfer != PPTRANSFER_BODY) {
				ftp_state(data, FTP_STOP);
			} else if (ftpc->known_filesize != -1) {
				Curl_pgrsSetDownloadSize(data, ftpc->known_filesize);
				result = ftp_state_retr(data, ftpc->known_filesize);
			} else if (data->set.ignorecl || data->state.prefer_ascii) {
...
static CURLcode ftp_state_pasv_resp(struct Curl_easy* data, int ftpcode) {
	...
	struct pingpong* pp = &ftpc->pp;
	char* str = Curl_dyn_ptr(&pp->recvbuf) + 4; /* start on the first letter */
	...
	if ((ftpc->count1 == 0) && (ftpcode == 229)) {
		...
		if (data->set.ftp_skip_ip) {
			...
			infof(data, "Skip %u.%u.%u.%u for data connection, reuse %s instead", ip[0], ip[1], ip[2], ip[3],
				  conn->host.name);
			ftpc->newhost = strdup(control_address(conn));
		} else {
...
static CURLcode ftp_state_mdtm_resp(struct Curl_easy* data, int ftpcode) {
	...
	case 213: {
		...
		if (ftp_213_date(resp, &year, &month, &day, &hour, &minute, &second)) {
			...
			char timebuf[24];
			msnprintf(timebuf, sizeof(timebuf), "%04d%02d%02d %02d:%02d:%02d GMT", year, month, day, hour, minute,
					  second);
...
static CURLcode ftp_state_get_resp(struct Curl_easy* data, int ftpcode, ftpstate instate) {
	...
	} else if ((instate == FTP_LIST) && (ftpcode == 450)) {
...
static CURLcode ftp_done(struct Curl_easy* data, CURLcode status, bool premature) {
	...
	if (result || premature)
		...
		if ((-1 != data->state.infilesize) && (data->state.infilesize != data->req.writebytecount) && !data->set.crlf &&
			(ftp->transfer == PPTRANSFER_BODY)) {
			failf(data,
				  "Uploaded unaligned file size (%" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes)",
				  data->req.writebytecount, data->state.infilesize);
			result = CURLE_PARTIAL_FILE;
		}
	} else if ((-1 != data->req.size) && (data->req.size != data->req.bytecount) &&
