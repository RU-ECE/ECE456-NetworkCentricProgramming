 ...
 */
static bool auth_digest_get_key_value(const char* chlg, const char* key, char* value, size_t max_val_len,
									  char end_char) {
	char* find_pos;
	size_t i;
 ...
 */
static CURLcode auth_decode_digest_md5_message(const struct bufref* chlgref, char* nonce, size_t nlen, char* realm,
											   size_t rlen, char* alg, size_t alen, char* qop, size_t qlen) {
...
CURLcode Curl_auth_create_digest_md5_message(struct Curl_easy* data, const struct bufref* chlg, const char* userp,
	...
	CURLcode result = auth_decode_digest_md5_message(chlg, nonce, sizeof(nonce), realm, sizeof(realm), algorithm,
 ...
 * Returns CURLE_OK on success.
 */
static CURLcode auth_create_digest_http_message(struct Curl_easy* data, const char* userp, const char* passwdp,
	...
	*/
	...
	if (digest->qop) {
		response = aprintf("username=\"%s\", "
						   ...
						   "qop=%s, "
						   "response=\"%s\"",
						   userp_quoted, realm_quoted, nonce_quoted, uripath, digest->cnonce, digest->nc, digest->qop,
						   request_digest);
...
CURLcode Curl_auth_create_digest_http_message(struct Curl_easy* data, const char* userp, const char* passwdp,
	...
	if (digest->algo <= ALGO_MD5SESS) {
		return auth_create_digest_http_message(data, userp, passwdp, request, uripath, digest, outptr, outlen,
