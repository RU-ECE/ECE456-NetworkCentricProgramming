...
static void socksstate(struct socks_state* sx, struct Curl_easy* data, enum connect_t state
	...
	static const char* const socks_statename[] = {
		"INIT",			  "SOCKS_INIT", "SOCKS_SEND",  "SOCKS_READ_INIT", "SOCKS_READ",	   "GSSAPI_INIT",
		"AUTH_INIT",	  "AUTH_SEND",	"AUTH_READ",   "REQ_INIT",		  "RESOLVING",	   "RESOLVED",
		"RESOLVE_REMOTE", "REQ_SEND",	"REQ_SENDING", "REQ_READ",		  "REQ_READ_MORE", "DONE"};
#endif
...
static CURLproxycode socks_state_send(struct Curl_cfilter* cf, struct socks_state* sx, struct Curl_easy* data,
									  CURLproxycode failcode, const char* description) {
	ssize_t nwritten;
	CURLcode result;
...
static CURLproxycode socks_state_recv(struct Curl_cfilter* cf, struct socks_state* sx, struct Curl_easy* data,
									  CURLproxycode failcode, const char* description) {
	ssize_t nread;
	CURLcode result;
	...
	if (nread <= 0) {
		...
		failf(data, "SOCKS: Failed receiving %s: %s", description, curl_easy_strerror(result));
		return failcode;
	}
...
static CURLproxycode do_SOCKS4(struct Curl_cfilter* cf, struct socks_state* sx, struct Curl_easy* data) {
	...
	case CONNECT_SOCKS_INIT:
		...
		if (conn->bits.httpproxy) {
			infof(data, "SOCKS4%s: connecting to HTTP proxy %s port %d", protocol4a ? "a" : "", sx->hostname,
				  sx->remote_port);
		}
...
static void socks_cf_get_host(struct Curl_cfilter* cf, struct Curl_easy* data, const char** phost,
