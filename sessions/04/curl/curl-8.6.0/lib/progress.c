...
static void time2str(char* r, curl_off_t seconds) {
	...
	if (h <= CURL_OFF_T_C(99)) {
		...
		msnprintf(r, 9, "%2" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T, h, m,
				  s);
	} else {
...
void Curl_pgrsTimeWas(struct Curl_easy* data, timerid timer, struct curltime timestamp) {
	...
	case TIMER_POSTQUEUE:
		...
		data->progress.t_postqueue = Curl_timediff_us(timestamp, data->progress.t_startop);
		break;
 ...
 */
timediff_t Curl_pgrsLimitWaitTime(curl_off_t cursize, curl_off_t startsize, curl_off_t limit, struct curltime start,
								  struct curltime now) {
 ...
 */
void Curl_pgrsSetUploadCounter(struct Curl_easy* data, curl_off_t size) { data->progress.uploaded = size; }
...
int Curl_pgrsUpdate(struct Curl_easy* data) {
	...
	if (!(data->progress.flags & PGRS_HIDE)) {
		if (data->set.fxferinfo) {
			...
			result = data->set.fxferinfo(data->set.progress_client, data->progress.size_dl, data->progress.downloaded,
			...
			Curl_set_in_callback(data, true);
			result = data->set.fprogress(data->set.progress_client, (double)data->progress.size_dl,
