...
static CURLcode tftp_set_timeouts(struct tftp_state_data* state) {
	...
	infof(state->data, "set timeouts for state %d; Total % " CURL_FORMAT_CURL_OFF_T ", retry %d maxtry %d",
...
static CURLcode tftp_parse_option_ack(struct tftp_state_data* state, const char* ptr, int len) {
	...
	while (tmp < ptr + len) {
		...
		if (checkprefix(TFTP_OPTION_BLKSIZE, option)) {
			...
			infof(data, "%s (%d) %s (%d)", "blksize parsed from OACK", state->blksize, "requested",
...
static CURLcode tftp_send_first(struct tftp_state_data* state, tftp_event_t event) {
	...
	case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
		...
		senddata = sendto(state->sockfd, (void*)state->spacket.data, (SEND_TYPE_ARG3)sbytes, 0,
...
static CURLcode tftp_rx(struct tftp_state_data* state, tftp_event_t event) {
	...
	case TFTP_EVENT_DATA:
		...
		if (NEXT_BLOCKNUM(state->block) == rblock) {
			...
			infof(data, "Received unexpected DATA packet block %d, expecting block %d", rblock,
		...
		sbytes = sendto(state->sockfd, (void*)state->spacket.data, 4, SEND_4TH_ARG,
	...
	case TFTP_EVENT_ERROR:
		...
		setpacketblock(&state->spacket, state->block);
		(void)sendto(state->sockfd, (void*)state->spacket.data, 4, SEND_4TH_ARG, (struct sockaddr*)&state->remote_addr,
		...
		if (event == TFTP_EVENT_ACK) {
			...
			if (rblock != state->block &&
				...
				if (state->retries > state->retry_max) {
					...
					sbytes = sendto(state->sockfd, (void*)state->spacket.data, 4 + state->sbytes, SEND_4TH_ARG,
									(struct sockaddr*)&state->remote_addr, state->remote_addrlen);
...
static CURLcode tftp_receive_packet(struct Curl_easy* data) {
	...
	state->rbytes = (int)recvfrom(state->sockfd, (void*)state->rpacket.data, state->blksize + 4, 0,
