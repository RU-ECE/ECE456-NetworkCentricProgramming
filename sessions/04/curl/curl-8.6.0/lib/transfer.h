...
void Curl_init_CONNECT(Curl_easy* data);

CURLcode Curl_pretransfer(Curl_easy* data);
CURLcode Curl_posttransfer(Curl_easy* data);
...
CURLcode Curl_readwrite(Curl_easy* data, bool* done);
int Curl_single_getsock(Curl_easy* data, struct connectdata* conn, curl_socket_t* socks);
...
bool Curl_meets_timecondition(Curl_easy* data, time_t timeofdoc);
CURLcode Curl_get_upload_buffer(Curl_easy* data);

CURLcode Curl_done_sending(Curl_easy* data, struct SingleRequest* k);
 ...
 */
CURLcode Curl_xfer_write_resp(Curl_easy* data, char* buf, size_t blen, bool is_eos, bool* done);

/* This sets up a forthcoming transfer */
void Curl_setup_transfer(Curl_easy* data, int sockindex, /* socket index to read from or -1 */
