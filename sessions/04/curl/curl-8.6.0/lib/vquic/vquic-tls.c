...
#if defined(ENABLE_QUIC) && (defined(USE_OPENSSL) || defined(USE_GNUTLS) || defined(USE_WOLFSSL))
...
static CURLcode curl_ossl_init_ctx(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data,
	...
	{
		const char* curves = conn_config->curves ? conn_config->curves : QUIC_GROUPS;
	...
	{
		const char* ciphers13 = conn_config->cipher_list13 ? conn_config->cipher_list13 : QUIC_CIPHERS;
	...
	SSL_CTX_set_verify(ctx->ssl_ctx, conn_config->verifypeer ? SSL_VERIFY_PEER : SSL_VERIFY_NONE, NULL);
	...
	if (data->set.ssl.fsslctx) {
		...
		result = (*data->set.ssl.fsslctx)(data, ctx->ssl_ctx, data->set.ssl.fsslctxp);
...
static CURLcode curl_ossl_set_client_cert(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (ssl_config->primary.clientcert || ssl_config->primary.cert_blob || ssl_config->cert_type) {
		return Curl_ossl_set_client_cert(data, ssl_ctx, ssl_config->primary.clientcert, ssl_config->primary.cert_blob,
										 ssl_config->cert_type, ssl_config->key, ssl_config->key_blob,
...
static CURLcode curl_ossl_init_ssl(struct quic_tls_ctx* ctx, struct Curl_easy* data, struct ssl_peer* peer,
								   const char* alpn, size_t alpn_len, void* user_data) {
	DEBUGASSERT(!ctx->ssl);
...
static CURLcode curl_gtls_init_ctx(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data,
								   struct ssl_peer* peer, const char* alpn, size_t alpn_len,
								   Curl_vquic_tls_ctx_setup* ctx_setup, void* user_data) {
	struct ssl_primary_config* conn_config;
	CURLcode result;
	...
	result = gtls_client_init(data, conn_config, &data->set.ssl, peer, ctx->gtls, pverifyresult);
	if (result)
		return result;
	...
	if (alpn) {
		...
		if (alen) /* not all alpn chars used, wrong format or too many */
			return CURLE_FAILED_INIT;
		if (i)
			gnutls_alpn_set_protocols(ctx->gtls->session, alpns, (unsigned int)i, GNUTLS_ALPN_MANDATORY);
	}
...
static CURLcode curl_wssl_init_ctx(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data,
								   Curl_vquic_tls_ctx_setup* ctx_setup) {
	struct ssl_primary_config* conn_config;
	CURLcode result = CURLE_FAILED_INIT;
	...
	if (wolfSSL_CTX_set_cipher_list(ctx->ssl_ctx,
									conn_config->cipher_list13 ? conn_config->cipher_list13 : QUIC_CIPHERS) != 1) {
	...
	if (wolfSSL_CTX_set1_groups_list(ctx->ssl_ctx, conn_config->curves ? conn_config->curves : (char*)QUIC_GROUPS) !=
		1) {
	...
	if (conn_config->verifypeer) {
		...
		if (ssl_cafile || ssl_capath) {
			...
			int rc = wolfSSL_CTX_load_verify_locations_ex(ctx->ssl_ctx, ssl_cafile, ssl_capath,
...
static CURLcode curl_wssl_init_ssl(struct quic_tls_ctx* ctx, struct Curl_easy* data, struct ssl_peer* peer,
								   const char* alpn, size_t alpn_len, void* user_data) {
	(void)data;
	DEBUGASSERT(!ctx->ssl);
	DEBUGASSERT(ctx->ssl_ctx);
	...
	if (alpn)
		wolfSSL_set_alpn_protos(ctx->ssl, (const unsigned char*)alpn, (int)alpn_len);

	if (peer->sni)
		wolfSSL_UseSNI(ctx->ssl, WOLFSSL_SNI_HOST_NAME, peer->sni, (unsigned short)strlen(peer->sni));

	return CURLE_OK;
}
#endif /* defined(USE_WOLFSSL) */

CURLcode Curl_vquic_tls_init(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data,
							 struct ssl_peer* peer, const char* alpn, size_t alpn_len,
							 Curl_vquic_tls_ctx_setup* ctx_setup, void* user_data) {
	CURLcode result;
...
#elif defined(USE_GNUTLS)
	(void)result;
	return curl_gtls_init_ctx(ctx, cf, data, peer, alpn, alpn_len, ctx_setup, user_data);
#elif defined(USE_WOLFSSL)
...
CURLcode Curl_vquic_tls_before_recv(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data) {
#ifdef USE_OPENSSL
	if (!ctx->x509_store_setup) {
...
CURLcode Curl_vquic_tls_verify_peer(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data,
	...
	if (conn_config->verifyhost) {
		...
		if (result)
			return result;
#elif defined(USE_GNUTLS)
		result = Curl_gtls_verifyserver(data, ctx->gtls->session, conn_config, &data->set.ssl, peer,
										data->set.str[STRING_SSL_PINNEDPUBLICKEY]);
		if (result)
			return result;
#elif defined(USE_WOLFSSL)
		if (!peer->sni || wolfSSL_check_domain_name(ctx->ssl, peer->sni) == SSL_FAILURE)
			return CURLE_PEER_FAILED_VERIFICATION;
#endif
