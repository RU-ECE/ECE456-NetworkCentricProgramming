...
static void h3_data_done(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (stream) {
		...
		if (ctx->h3conn && !stream->closed) {
			...
			nghttp3_conn_close_stream(ctx->h3conn, stream->id, NGHTTP3_H3_REQUEST_CANCELLED);
...
static int cb_h3_acked_req_body(nghttp3_conn* conn, int64_t stream_id, uint64_t datalen, void* user_data,
...
static int cb_recv_stream_data(ngtcp2_conn* tconn, uint32_t flags, int64_t stream_id, uint64_t offset,
							   const uint8_t* buf, size_t buflen, void* user_data, void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
	...
	if (nconsumed < 0) {
		if (!data) {
			...
			CURL_TRC_CF(cdata, cf,
						"[%" PRId64 "] nghttp3 error on stream not "
						"used by us, ignored",
						stream_id);
			return 0;
		}
...
static int cb_acked_stream_data_offset(ngtcp2_conn* tconn, int64_t stream_id, uint64_t offset, uint64_t datalen,
...
static int cb_stream_close(ngtcp2_conn* tconn, uint32_t flags, int64_t stream3_id, uint64_t app_error_code,
						   void* user_data, void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct Curl_easy* data = stream_user_data;
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
...
static int cb_stream_reset(ngtcp2_conn* tconn, int64_t stream_id, uint64_t final_size, uint64_t app_error_code,
						   void* user_data, void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
	struct Curl_easy* data = stream_user_data;
...
static int cb_stream_stop_sending(ngtcp2_conn* tconn, int64_t stream_id, uint64_t app_error_code, void* user_data,
								  void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
	int rv;
...
static int cb_extend_max_stream_data(ngtcp2_conn* tconn, int64_t stream_id, uint64_t max_data, void* user_data,
									 void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
...
static int cb_get_new_connection_id(ngtcp2_conn* tconn, ngtcp2_cid* cid, uint8_t* token, size_t cidlen,
									void* user_data) {
	CURLcode result;
	(void)tconn;
	(void)user_data;
...
static void cf_ngtcp2_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps) {
	...
	if (want_recv || want_send) {
		...
		c_exhaust = want_send && (!ngtcp2_conn_get_cwnd_left(ctx->qconn) || !ngtcp2_conn_get_max_data_left(ctx->qconn));
		s_exhaust = want_send && stream && stream->id >= 0 && stream->quic_flow_blocked;
...
static int cb_h3_deferred_consume(nghttp3_conn* conn, int64_t stream3_id, size_t consumed, void* user_data,
								  void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
	(void)conn;
...
static nghttp3_ssize cb_h3_read_req_body(nghttp3_conn* conn, int64_t stream_id, nghttp3_vec* vec, size_t veccnt,
	...
	CURL_TRC_CF(data, cf,
				...
				stream->id, (int)nvecs, *pflags == NGHTTP3_DATA_FLAG_EOF ? " EOF" : "", nwritten,
...
static ssize_t h3_stream_open(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
	...
	*err = Curl_http_req_to_h2(&h2_headers, stream->h1.req, data);
	...
	if (Curl_trc_is_verbose(data)) {
		...
		for (i = 0; i < nheader; ++i) {
			infof(data, "[HTTP/3] [%" PRId64 "] [%.*s: %.*s]", stream->id, (int)nva[i].namelen, nva[i].name,
...
static ssize_t cf_ngtcp2_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							  CURLcode* err) {
	struct cf_ngtcp2_ctx* ctx = cf->ctx;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
	ssize_t sent = 0;
...
static CURLcode recv_pkt(const unsigned char* pkt, size_t pktlen, struct sockaddr_storage* remote_addr,
	...
	if (rv) {
		CURL_TRC_CF(pktx->data, pktx->cf, "ingress, read_pkt -> %s (%d)", ngtcp2_strerror(rv), rv);
	...
	for (;;) {
		...
		if (ctx->h3conn && ngtcp2_conn_get_max_data_left(ctx->qconn)) {
			...
			if (veccnt < 0) {
				...
				ngtcp2_ccerr_set_application_error(&ctx->last_error, nghttp3_err_infer_quic_app_error_code((int)veccnt),
												   NULL, 0);
				*err = CURLE_SEND_ERROR;
				return -1;
			}
		...
		n = ngtcp2_conn_writev_stream(ctx->qconn, &x->ps.path, NULL, buf, buflen, &ndatalen, flags, stream_id,
...
static CURLcode tls_ctx_setup(struct quic_tls_ctx* ctx, struct Curl_cfilter* cf, struct Curl_easy* data) {
	(void)cf;
#ifdef USE_OPENSSL
#if defined(OPENSSL_IS_BORINGSSL) || defined(OPENSSL_IS_AWSLC)
...
#define H3_ALPN "\x2h3\x5h3-29"
	result = Curl_vquic_tls_init(&ctx->tls, cf, data, &ctx->peer, H3_ALPN, sizeof(H3_ALPN) - 1, tls_ctx_setup,
								 &ctx->conn_ref);
	if (result)
		return result;
	...
	rc = ngtcp2_conn_client_new(&ctx->qconn, &ctx->dcid, &ctx->scid, &ctx->connected_path, NGTCP2_PROTO_VER_V1,
								&ng_callbacks, &ctx->settings, &ctx->transport_params, NULL, cf);
	if (rc)
		return CURLE_QUIC_CONNECT_ERROR;
...
CURLcode Curl_cf_ngtcp2_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn,
