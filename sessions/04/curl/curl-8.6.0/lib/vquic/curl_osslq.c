...
static CURLcode cf_progress_ingress(struct Curl_cfilter* cf, struct Curl_easy* data);
...
static CURLcode make_bio_addr(BIO_ADDR** pbio_addr, const struct Curl_sockaddr_ex* addr) {
	BIO_ADDR* ba;
	...
	switch (addr->family) {
	case AF_INET: {
		struct sockaddr_in* const sin = (struct sockaddr_in* const)(void*)&addr->sa_addr;
		if (!BIO_ADDR_rawmake(ba, AF_INET, &sin->sin_addr, sizeof(sin->sin_addr), sin->sin_port))
			goto out;
		result = CURLE_OK;
		break;
	}
#ifdef ENABLE_IPV6
	case AF_INET6: {
		struct sockaddr_in6* const sin = (struct sockaddr_in6* const)(void*)&addr->sa_addr;
		if (!BIO_ADDR_rawmake(ba, AF_INET6, &sin->sin6_addr, sizeof(sin->sin6_addr), sin->sin6_port)) {
		}
		result = CURLE_OK;
		break;
	}
...
static CURLcode cf_osslq_stream_open(struct cf_osslq_stream* s, SSL* conn, uint64_t flags, struct bufc_pool* bufcp,
									 void* user_data) {
	DEBUGASSERT(!s->ssl);
...
static CURLcode cf_osslq_h3conn_add_stream(struct cf_osslq_h3conn* h3, SSL* stream_ssl, struct Curl_cfilter* cf,
	...
	if (h3->remote_ctrl_n >= ARRAYSIZE(h3->remote_ctrl)) {
		...
		CURL_TRC_CF(data, cf, "[%" PRId64 "] rejecting additional remote stream", stream_id);
		SSL_free(stream_ssl);
		return CURLE_FAILED_INIT;
	}
	...
	case SSL_STREAM_TYPE_READ: {
		...
		CURL_TRC_CF(data, cf, "[%" PRId64 "] accepted new remote uni stream", stream_id);
		break;
	}
	default:
		CURL_TRC_CF(data, cf,
					"[%" PRId64 "] rejecting remote non-uni-read"
					" stream",
					stream_id);
		SSL_free(stream_ssl);
		return CURLE_FAILED_INIT;
...
static CURLcode cf_osslq_ssl_err(struct Curl_cfilter* cf, struct Curl_easy* data, int detail, CURLcode def_result) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	...
	if ((lib == ERR_LIB_SSL) &&
		((reason == SSL_R_CERTIFICATE_VERIFY_FAILED) || (reason == SSL_R_SSLV3_ALERT_CERTIFICATE_EXPIRED))) {
		result = CURLE_PEER_FAILED_VERIFICATION;

		lerr = SSL_get_verify_result(ctx->tls.ssl);
		if (lerr != X509_V_OK) {
			ssl_config->certverifyresult = lerr;
			msnprintf(ebuf, sizeof(ebuf), "SSL certificate problem: %s", X509_verify_cert_error_string(lerr));
		} else {
	...
	if (CURLE_SSL_CONNECT_ERROR == result && errdetail == 0) {
		...
		failf(data, "QUIC connect: %s in connection to %s:%d (%s)", extramsg[0] ? extramsg : SSL_ERROR_to_str(detail),
...
static CURLcode cf_osslq_verify_peer(struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
...
#define H3_STREAM_CTX(d)                                                                                               \
	((struct h3_stream_ctx*)(((d) && (d)->req.p.http) ? ((struct HTTP*)(d)->req.p.http)->h3_ctx : NULL))
#define H3_STREAM_LCTX(d) ((struct HTTP*)(d)->req.p.http)->h3_ctx
#define H3_STREAM_ID(d) (H3_STREAM_CTX(d) ? H3_STREAM_CTX(d)->s.id : -2)
...
static void h3_data_done(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (stream) {
		...
		if (ctx->h3.conn && !stream->closed) {
			...
			nghttp3_conn_close_stream(ctx->h3.conn, stream->s.id, NGHTTP3_H3_REQUEST_CANCELLED);
...
static struct cf_osslq_stream* cf_osslq_get_qstream(struct Curl_cfilter* cf, struct Curl_easy* data,
													int64_t stream_id) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
...
static int cb_h3_stream_close(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* user_data,
	...
	if (stream->error3 != NGHTTP3_H3_NO_ERROR) {
		...
		CURL_TRC_CF(data, cf, "[%" PRId64 "] RESET: error %" PRId64, stream->s.id, stream->error3);
	} else {
...
static int cb_h3_recv_data(nghttp3_conn* conn, int64_t stream3_id, const uint8_t* buf, size_t buflen, void* user_data,
						   void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct Curl_easy* data = stream_user_data;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
	...
	if (result) {
		CURL_TRC_CF(data, cf, "[%" PRId64 "] DATA len=%zu, ERROR receiving %d", stream->s.id, buflen, result);
		return NGHTTP3_ERR_CALLBACK_FAILURE;
	}
	stream->download_recvd += (curl_off_t)buflen;
	CURL_TRC_CF(data, cf, "[%" PRId64 "] DATA len=%zu, total=%zd", stream->s.id, buflen, stream->download_recvd);
	h3_drain_stream(cf, data);
	return 0;
}

static int cb_h3_deferred_consume(nghttp3_conn* conn, int64_t stream_id, size_t consumed, void* user_data,
								  void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct Curl_easy* data = stream_user_data;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
	...
	(void)stream_id;
	if (stream)
		CURL_TRC_CF(data, cf, "[%" PRId64 "] deferred consume %zu bytes", stream->s.id, consumed);
	return 0;
}

static int cb_h3_recv_header(nghttp3_conn* conn, int64_t stream_id, int32_t token, nghttp3_rcbuf* name,
	...
	if (token == NGHTTP3_QPACK_TOKEN__STATUS) {
		...
		CURL_TRC_CF(data, cf, "[%" PRId64 "] header: %.*s: %.*s", stream_id, (int)h3name.len, h3name.base,
...
static int cb_h3_stop_sending(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* user_data,
							  void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct Curl_easy* data = stream_user_data;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
...
static int cb_h3_reset_stream(nghttp3_conn* conn, int64_t stream_id, uint64_t app_error_code, void* user_data,
							  void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct Curl_easy* data = stream_user_data;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
...
static nghttp3_ssize cb_h3_read_req_body(nghttp3_conn* conn, int64_t stream_id, nghttp3_vec* vec, size_t veccnt,
	...
	if (stream->sendbuf_len_in_flight < Curl_bufq_len(&stream->sendbuf)) {
		...
		while (nvecs < veccnt &&
			   Curl_bufq_peek_at(&stream->sendbuf, stream->sendbuf_len_in_flight,
								 (const unsigned char**)&vec[nvecs].base, &vec[nvecs].len)) {
	...
	if (stream->upload_left == 0) {
		*pflags = NGHTTP3_DATA_FLAG_EOF;
		...
		CURL_TRC_CF(data, cf, "[%" PRId64 "] read req body -> AGAIN", stream->s.id);
	...
	CURL_TRC_CF(data, cf,
				...
				stream->s.id, (int)nvecs, *pflags == NGHTTP3_DATA_FLAG_EOF ? " EOF" : "", nwritten,
				Curl_bufq_len(&stream->sendbuf), stream->upload_left);
	return (nghttp3_ssize)nvecs;
}

static int cb_h3_acked_stream_data(nghttp3_conn* conn, int64_t stream_id, uint64_t datalen, void* user_data,
								   void* stream_user_data) {
	struct Curl_cfilter* cf = user_data;
	struct Curl_easy* data = stream_user_data;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
...
static CURLcode cf_osslq_h3conn_init(struct cf_osslq_ctx* ctx, SSL* conn, void* user_data) {
	struct cf_osslq_h3conn* h3 = &ctx->h3;
	CURLcode result;
	int rc;

	nghttp3_settings_default(&h3->settings);
	rc = nghttp3_conn_client_new(&h3->conn, &ngh3_callbacks, &h3->settings, nghttp3_mem_default(), user_data);
	if (rc) {
		result = CURLE_OUT_OF_MEMORY;
		goto out;
	}

	result = cf_osslq_stream_open(&h3->s_ctrl, conn, SSL_STREAM_FLAG_ADVANCE | SSL_STREAM_FLAG_UNI, &ctx->stream_bufcp,
								  NULL);
	if (result) {
		result = CURLE_QUIC_CONNECT_ERROR;
		goto out;
	}
	result = cf_osslq_stream_open(&h3->s_qpack_enc, conn, SSL_STREAM_FLAG_ADVANCE | SSL_STREAM_FLAG_UNI,
								  &ctx->stream_bufcp, NULL);
	if (result) {
		result = CURLE_QUIC_CONNECT_ERROR;
		goto out;
	}
	result = cf_osslq_stream_open(&h3->s_qpack_dec, conn, SSL_STREAM_FLAG_ADVANCE | SSL_STREAM_FLAG_UNI,
								  &ctx->stream_bufcp, NULL);
	if (result) {
		result = CURLE_QUIC_CONNECT_ERROR;
		goto out;
	...
	rc = nghttp3_conn_bind_qpack_streams(h3->conn, h3->s_qpack_enc.id, h3->s_qpack_dec.id);
	if (rc) {
		result = CURLE_QUIC_CONNECT_ERROR;
		goto out;
	}
...
static CURLcode cf_osslq_ctx_start(struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result;
	int rv;
...
#define H3_ALPN "\x2h3"
	result = Curl_vquic_tls_init(&ctx->tls, cf, data, &ctx->peer, H3_ALPN, sizeof(H3_ALPN) - 1, NULL, NULL);
	if (result)
		goto out;
	...
	result = CURLE_QUIC_CONNECT_ERROR;
	Curl_cf_socket_peek(cf->next, data, &ctx->q.sockfd, &peer_addr, NULL, &peer_port, NULL, NULL);
	if (!peer_addr)
		goto out;
	...
	SSL_set_incoming_stream_policy(ctx->tls.ssl, SSL_INCOMING_STREAM_POLICY_ACCEPT, 0);
...
static ssize_t h3_quic_recv(void* reader_ctx, unsigned char* buf, size_t len, CURLcode* err) {
	...
	if (rv <= 0) {
		...
		if (detail == SSL_ERROR_WANT_READ || detail == SSL_ERROR_WANT_WRITE) {
			*err = CURLE_AGAIN;
			...
			CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] h3_quic_recv -> EOS", x->s->id);
			x->s->recvd_eos = TRUE;
			return 0;
		} else if (SSL_get_stream_read_state(x->s->ssl) == SSL_STREAM_STATE_RESET_REMOTE) {
			uint64_t app_error_code = NGHTTP3_H3_NO_ERROR;
			SSL_get_stream_read_error_code(x->s->ssl, &app_error_code);
			CURL_TRC_CF(x->data, x->cf,
						"[%" PRId64 "] h3_quic_recv -> RESET, "
						"rv=%d, app_err=%" PRIu64,
...
static CURLcode cf_osslq_stream_recv(struct cf_osslq_stream* s, struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result = CURLE_OK;
	ssize_t nread;
	...
	x.data = data;
	x.s = s;
	while (s->ssl && !s->closed && !eagain && (total_recv_len < H3_STREAM_CHUNK_SIZE)) {
		...
		if (!Curl_bufq_is_empty(&s->recvbuf)) {
			...
			while (Curl_bufq_peek(&s->recvbuf, &buf, &blen)) {
				nread = nghttp3_conn_read_stream(ctx->h3.conn, s->id, buf, blen, 0);
				CURL_TRC_CF(data, cf,
							"[%" PRId64 "] forward %zu bytes "
							"to nghttp3 -> %zd",
							s->id, blen, nread);
				if (nread < 0) {
					failf(data, "nghttp3_conn_read_stream(len=%zu) error: %s", blen, nghttp3_strerror((int)nread));
		...
		if (Curl_bufq_is_empty(&s->recvbuf) && !s->closed) {
			...
			if (s->reset) {
				...
				if (rv < 0 && rv != NGHTTP3_ERR_STREAM_NOT_FOUND) {
					failf(data, "nghttp3_conn_close_stream returned error: %s", nghttp3_strerror(rv));
					result = CURLE_RECV_ERROR;
					goto out;
				}
			} else if (s->recvd_eos) {
				rv = nghttp3_conn_close_stream(ctx->h3.conn, s->id, NGHTTP3_H3_NO_ERROR);
				s->closed = TRUE;
				CURL_TRC_CF(data, cf, "[%" PRId64 "] close nghttp3 stream -> %d", s->id, rv);
				if (rv < 0 && rv != NGHTTP3_ERR_STREAM_NOT_FOUND) {
					failf(data, "nghttp3_conn_close_stream returned error: %s", nghttp3_strerror(rv));
					result = CURLE_RECV_ERROR;
					goto out;
...
out:
	if (result)
		CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_osslq_stream_recv -> %d", s->id, result);
	return result;
}
...
static CURLcode cf_osslq_check_and_unblock(struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	struct Curl_easy* sdata;
	struct h3_stream_ctx* stream;
	...
	if (ctx->h3.conn) {
		for (sdata = data->multi->easyp; sdata; sdata = sdata->next) {
			if (sdata->conn == data->conn) {
				stream = H3_STREAM_CTX(sdata);
				if (stream && stream->s.ssl && stream->s.send_blocked && !SSL_want_write(stream->s.ssl)) {
...
static CURLcode h3_send_streams(struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result = CURLE_OK;
	...
	for (;;) {
		...
		n = nghttp3_conn_writev_stream(ctx->h3.conn, &stream_id, &eos, vec, ARRAYSIZE(vec));
		if (n < 0) {
			failf(data, "nghttp3_conn_writev_stream returned error: %s", nghttp3_strerror((int)n));
		...
		if (!s) {
			failf(data, "nghttp3_conn_writev_stream gave unknown stream %" PRId64, stream_id);
			result = CURLE_SEND_ERROR;
			goto out;
		}
		...
		for (i = 0; (i < n) && !blocked; ++i) {
			...
			written = vec[i].len;
			ok = !s->ssl || SSL_write_ex(s->ssl, vec[i].base, vec[i].len, &written);
			if (ok) {
				/* As OpenSSL buffers the data, we count this as acknowledged
				 * from nghttp3's point of view */
				CURL_TRC_CF(data, cf, "[%" PRId64 "] send %zu bytes to QUIC ok", s->id, vec[i].len);
				acked_len += vec[i].len;
			} else {
				...
				case SSL_ERROR_WANT_READ:
					...
					CURL_TRC_CF(data, cf, "[%" PRId64 "] send %zu bytes to QUIC blocked", s->id, vec[i].len);
					written = 0;
					nghttp3_conn_block_stream(ctx->h3.conn, s->id);
					s->send_blocked = blocked = TRUE;
					break;
				default:
					failf(data, "[%" PRId64 "] send %zu bytes to QUIC, SSL error %d", s->id, vec[i].len, detail);
		...
		if (acked_len > 0 || (eos && !s->send_blocked)) {
			...
			CURL_TRC_CF(data, cf,
						"[%" PRId64 "] forwarded %zu/%zu h3 bytes "
						"to QUIC, eos=%d",
...
static CURLcode cf_progress_egress(struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result = CURLE_OK;
...
static CURLcode check_and_set_expiry(struct Curl_cfilter* cf, struct Curl_easy* data) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result = CURLE_OK;
	struct timeval tv;
	...
	if (ctx->tls.ssl && SSL_get_event_timeout(ctx->tls.ssl, &tv, &is_infinite) && !is_infinite) {
...
static CURLcode cf_osslq_connect(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result = CURLE_OK;
	struct cf_call_data save;
	struct curltime now;
...
static ssize_t h3_stream_open(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							  CURLcode* err) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	...
	*err = cf_osslq_stream_open(&stream->s, ctx->tls.ssl, 0, &ctx->stream_bufcp, data);
	if (*err) {
	...
	rc = nghttp3_conn_submit_request(ctx->h3.conn, stream->s.id, nva, nheader, preader, data);
	if (rc) {
		switch (rc) {
		case NGHTTP3_ERR_CONN_CLOSING:
			...
			break;
		default:
			CURL_TRC_CF(data, cf, "h3sid[%" PRId64 "] failed to send -> %d (%s)", stream->s.id, rc,
						nghttp3_strerror(rc));
			break;
		}
		*err = CURLE_SEND_ERROR;
	...
	if (Curl_trc_is_verbose(data)) {
		infof(data, "[HTTP/3] [%" PRId64 "] OPENED stream for %s", stream->s.id, data->state.url);
		for (i = 0; i < nheader; ++i) {
			infof(data, "[HTTP/3] [%" PRId64 "] [%.*s: %.*s]", stream->s.id, (int)nva[i].namelen, nva[i].name,
...
static ssize_t cf_osslq_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							 CURLcode* err) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
	struct cf_call_data save;
	...
	*err = CURLE_OK;
	...
	if (stream && nwritten > 0 && stream->sendbuf_len_in_flight) {
		...
		CURL_TRC_CF(data, cf,
					...
					stream->s.id, len, stream->sendbuf_len_in_flight);
		*err = CURLE_AGAIN;
		nwritten = -1;
	}
...
out:
	result = check_and_set_expiry(cf, data);
	CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_send(len=%zu) -> %zd, %d", stream ? stream->s.id : -1, len, nwritten, *err);
	CF_DATA_RESTORE(cf, save);
	return nwritten;
}

static ssize_t recv_closed_stream(struct Curl_cfilter* cf, struct Curl_easy* data, struct h3_stream_ctx* stream,
								  CURLcode* err) {
	ssize_t nread = -1;

	(void)cf;
	if (stream->reset) {
		failf(data, "HTTP/3 stream %" PRId64 " reset by server", stream->s.id);
...
static ssize_t cf_osslq_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
	ssize_t nread = -1;
	struct cf_call_data save;
...
out:
	...
	CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_recv(len=%zu) -> %zd, %d", stream ? stream->s.id : -1, len, nread, *err);
	CF_DATA_RESTORE(cf, save);
	return nread;
}
 ...
 */
static bool cf_osslq_data_pending(struct Curl_cfilter* cf, const struct Curl_easy* data) {
	const struct h3_stream_ctx* stream = H3_STREAM_CTX(data);
	(void)cf;
	return stream && !Curl_bufq_is_empty(&stream->recvbuf);
}

static CURLcode cf_osslq_data_event(struct Curl_cfilter* cf, struct Curl_easy* data, int event, int arg1, void* arg2) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	CURLcode result = CURLE_OK;
	struct cf_call_data save;
...
static bool cf_osslq_conn_is_alive(struct Curl_cfilter* cf, struct Curl_easy* data, bool* input_pending) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	bool alive = FALSE;
	struct cf_call_data save;
...
static void cf_osslq_adjust_pollset(struct Curl_cfilter* cf, struct Curl_easy* data, struct easy_pollset* ps) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	...
	if (!ctx->tls.ssl) {
		...
		Curl_pollset_set(data, ps, ctx->q.sockfd, SSL_net_read_desired(ctx->tls.ssl),
...
static CURLcode cf_osslq_query(struct Curl_cfilter* cf, struct Curl_easy* data, int query, int* pres1, void* pres2) {
	struct cf_osslq_ctx* ctx = cf->ctx;
	struct cf_call_data save;
