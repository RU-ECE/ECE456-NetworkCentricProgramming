...
static const MSH3_CONNECTION_IF msh3_conn_if = {msh3_conn_connected, msh3_conn_shutdown_complete,
...
static const MSH3_REQUEST_IF msh3_request_if = {msh3_header_received, msh3_data_received, msh3_complete,
...
static ssize_t cf_msh3_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							CURLcode* err) {
	struct cf_msh3_ctx* ctx = cf->ctx;
	struct stream_ctx* stream = H3_STREAM_CTX(data);
	...
	if (!stream->req) {
		...
		*err = Curl_http_req_to_h2(&h2_headers, h1.req, data);
		...
		stream->req = MsH3RequestOpen(ctx->qconn, &msh3_request_if, data, nva, nheader,
...
CURLcode Curl_cf_msh3_create(struct Curl_cfilter** pcf, struct Curl_easy* data, struct connectdata* conn,
