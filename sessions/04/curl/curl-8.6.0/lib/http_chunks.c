...
void Curl_httpchunk_init(struct Curl_easy* data, struct Curl_chunker* ch, bool ignore_body) {
	(void)data;
...
void Curl_httpchunk_reset(struct Curl_easy* data, struct Curl_chunker* ch, bool ignore_body) {
	(void)data;
	ch->hexindex = 0; /* start at 0 */
	ch->state = CHUNK_HEX; /* we get hex first! */
	ch->last_code = CHUNKE_OK;
...
static CURLcode httpchunk_readwrite(struct Curl_easy* data, struct Curl_chunker* ch, struct Curl_cwriter* cw_next,
									const char* buf, size_t blen, size_t* pconsumed) {
	...
	while (blen) {
		...
		case CHUNK_DATA:
			...
			*/
			...
			if (!data->set.http_te_skip && !ch->ignore_body) {
				if (cw_next)
					result = Curl_cwriter_write(data, cw_next, CLIENTWRITE_BODY, buf, piece);
				else
					result = Curl_client_write(data, CLIENTWRITE_BODY, (char*)buf, piece);
				if (result) {
					ch->state = CHUNK_FAILED;
					ch->last_code = CHUNKE_PASSTHRU_ERROR;
					return result;
			...
			if ((*buf == 0x0d) || (*buf == 0x0a)) {
				...
				if (tr) {
					...
					if (!data->set.http_te_skip) {
						if (cw_next) {
							result =
								Curl_cwriter_write(data, cw_next, CLIENTWRITE_HEADER | CLIENTWRITE_TRAILER, tr, trlen);
						} else {
...
CURLcode Curl_httpchunk_read(struct Curl_easy* data, struct Curl_chunker* ch, char* buf, size_t blen,
							 size_t* pconsumed) {
...
static CURLcode cw_chunked_init(struct Curl_easy* data, struct Curl_cwriter* writer) {
...
static void cw_chunked_close(struct Curl_easy* data, struct Curl_cwriter* writer) {
	struct chunked_writer* ctx = (struct chunked_writer*)writer;
	Curl_httpchunk_free(data, &ctx->ch);
}

static CURLcode cw_chunked_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
								 size_t blen) {
	struct chunked_writer* ctx = (struct chunked_writer*)writer;
	CURLcode result;
	size_t consumed;
	...
	consumed = 0;
	result = httpchunk_readwrite(data, &ctx->ch, writer->next, buf, blen, &consumed);

	if (result) {
		if (CHUNKE_PASSTHRU_ERROR == ctx->ch.last_code)
			failf(data, "Failed reading the chunked-encoded stream");
		else
			failf(data, "%s in chunked-encoding", Curl_chunked_strerror(ctx->ch.last_code));
		return result;
	}
...
const struct Curl_cwtype Curl_httpchunk_unencoder = {
	"chunked", NULL, cw_chunked_init, cw_chunked_write, cw_chunked_close, sizeof(struct chunked_writer)};
