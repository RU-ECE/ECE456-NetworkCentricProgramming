...
char* curl_version(void) {
...
#ifdef USE_LIBPSL
	{
		...
		msnprintf(psl_version, sizeof(psl_version), "libpsl/%d.%d.%d", num >> 16, (num >> 8) & 0xff, num & 0xff);
		src[i++] = psl_version;
	}
#endif
...
#ifdef USE_LIBRTMP
	{
		...
		msnprintf(rtmp_version, sizeof(rtmp_version), "librtmp/%d.%d%s", RTMP_LIB_VERSION >> 16,
				  (RTMP_LIB_VERSION >> 8) & 0xff, suff);
		src[i++] = rtmp_version;
	}
#endif
#ifdef USE_HYPER
...
#if defined(USE_LIBIDN2)
static int idn_present(curl_version_info_data* info) { return info->libidn != NULL; }
#else
#define idn_present NULL
#endif

#if defined(USE_SSL) && !defined(CURL_DISABLE_PROXY) && !defined(CURL_DISABLE_HTTP)
...
static curl_version_info_data version_info = {CURLVERSION_NOW,
											  LIBCURL_VERSION,
											  LIBCURL_VERSION_NUM,
