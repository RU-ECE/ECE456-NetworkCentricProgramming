...
static unsigned short smb_swap16(unsigned short x) { return (unsigned short)((x << 8) | ((x >> 8) & 0xff)); }
...
static void request_state(struct Curl_easy* data, enum smb_req_state newstate) {
	...
	/* For debug purposes */
	static const char* const names[] = {
		"SMB_REQUESTING", "SMB_TREE_CONNECT",	 "SMB_OPEN", "SMB_DOWNLOAD", "SMB_UPLOAD",
		"SMB_CLOSE",	  "SMB_TREE_DISCONNECT", "SMB_DONE",
		/* LAST */
	};
...
static CURLcode smb_send_and_recv(struct Curl_easy* data, void** msg) {
	...
	*msg = NULL; /* if it returns early */
	...
	if (!smbc->send_size && smbc->upload_size) {
		size_t nread = smbc->upload_size > (size_t)data->set.upload_buffer_size ? (size_t)data->set.upload_buffer_size
																				: smbc->upload_size;
