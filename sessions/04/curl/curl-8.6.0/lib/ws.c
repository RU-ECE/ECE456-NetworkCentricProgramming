...
static struct ws_frame_meta WS_FRAMES[] = {
	{WSBIT_OPCODE_CONT, CURLWS_CONT, "CONT"}, {WSBIT_OPCODE_TEXT, CURLWS_TEXT, "TEXT"},
	{WSBIT_OPCODE_BIN, CURLWS_BINARY, "BIN"}, {WSBIT_OPCODE_CLOSE, CURLWS_CLOSE, "CLOSE"},
	{WSBIT_OPCODE_PING, CURLWS_PING, "PING"}, {WSBIT_OPCODE_PONG, CURLWS_PONG, "PONG"},
};
...
static void ws_dec_info(struct ws_decoder* dec, struct Curl_easy* data, const char* msg) {
	...
	default:
		if (dec->head_len < dec->head_total) {
			infof(data, "WS-DEC: %s [%s%s](%d/%d)", msg, ws_frame_name_of_op(dec->head[0]),
				  (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL", dec->head_len, dec->head_total);
		} else {
			infof(data, "WS-DEC: %s [%s%s payload=%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T "]", msg,
				  ws_frame_name_of_op(dec->head[0]), (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL",
...
typedef ssize_t ws_write_payload(const unsigned char* buf, size_t buflen, int frame_age, int frame_flags,
...
static CURLcode ws_dec_read_head(struct ws_decoder* dec, struct Curl_easy* data, struct bufq* inraw) {
	...
	while (Curl_bufq_peek(inraw, &inbuf, &inlen)) {
		...
		case 10:
			...
			dec->payload_len = ((curl_off_t)dec->head[2] << 56) | (curl_off_t)dec->head[3] << 48 |
				(curl_off_t)dec->head[4] << 40 | (curl_off_t)dec->head[5] << 32 | (curl_off_t)dec->head[6] << 24 |
				(curl_off_t)dec->head[7] << 16 | (curl_off_t)dec->head[8] << 8 | dec->head[9];
			break;
		default:
...
static CURLcode ws_dec_pass_payload(struct ws_decoder* dec, struct Curl_easy* data, struct bufq* inraw,
	...
	while (remain && Curl_bufq_peek(inraw, &inbuf, &inlen)) {
		if ((curl_off_t)inlen > remain)
			inlen = (size_t)remain;
		nwritten = write_payload(inbuf, inlen, dec->frame_age, dec->frame_flags, dec->payload_offset, dec->payload_len,
								 write_ctx, &result);
		if (nwritten < 0)
			return result;
...
static CURLcode ws_dec_pass(struct ws_decoder* dec, struct Curl_easy* data, struct bufq* inraw,
							ws_write_payload* write_payload, void* write_ctx) {
	CURLcode result;
...
static void update_meta(struct websocket* ws, int frame_age, int frame_flags, curl_off_t payload_offset,
...
static CURLcode ws_cw_init(struct Curl_easy* data, struct Curl_cwriter* writer) {
...
static ssize_t ws_cw_dec_next(const unsigned char* buf, size_t buflen, int frame_age, int frame_flags,
	...
	if ((frame_flags & CURLWS_PING) && !remain) {
		...
		*err = Curl_cwriter_write(data, ctx->next_writer, ctx->cw_type, (const char*)buf, buflen);
		if (*err)
			return -1;
	}
	*err = CURLE_OK;
...
static CURLcode ws_cw_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
							size_t nbytes) {
	struct ws_cw_ctx* ctx = (struct ws_cw_ctx*)writer;
	struct websocket* ws;
	CURLcode result;
	...
	if (nbytes) {
		ssize_t nwritten;
		nwritten = Curl_bufq_write(&ctx->buf, (const unsigned char*)buf, nbytes, &result);
		if (nwritten < 0) {
	...
	while (!Curl_bufq_is_empty(&ctx->buf)) {
		...
		result = ws_dec_pass(&ws->dec, data, &ctx->buf, ws_cw_dec_next, &pass_ctx);
		if (result == CURLE_AGAIN) {
	...
	if ((type & CLIENTWRITE_EOS) && !Curl_bufq_is_empty(&ctx->buf)) {
		infof(data, "WS: decode ending with %zd frame bytes remaining", Curl_bufq_len(&ctx->buf));
		return CURLE_RECV_ERROR;
	}
...
static const struct Curl_cwtype ws_cw_decode = {"ws-decode", NULL,		  ws_cw_init,
												ws_cw_write, ws_cw_close, sizeof(struct ws_cw_ctx)};


static void ws_enc_info(struct ws_encoder* enc, struct Curl_easy* data, const char* msg) {
	infof(data, "WS-ENC: %s [%s%s%s payload=%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T "]", msg,
		  ws_frame_name_of_op(enc->firstbyte), (enc->firstbyte & WSBIT_OPCODE_MASK) == WSBIT_OPCODE_CONT ? " CONT" : "",
		  (enc->firstbyte & WSBIT_FIN) ? "" : " NON-FIN", enc->payload_len - enc->payload_remain, enc->payload_len);
}
...
static ssize_t ws_enc_write_head(struct Curl_easy* data, struct ws_encoder* enc, unsigned int flags,
								 curl_off_t payload_len, struct bufq* out, CURLcode* err) {
	...
	if (payload_len < 0) {
		failf(data, "WS: starting new frame with negative payload length %" CURL_FORMAT_CURL_OFF_T, payload_len);
...
static ssize_t ws_enc_write_payload(struct ws_encoder* enc, struct Curl_easy* data, const unsigned char* buf,
									size_t buflen, struct bufq* out, CURLcode* err) {
	ssize_t n;
	size_t i, len;
...
CURLcode Curl_ws_accept(struct Curl_easy* data, const char* mem, size_t nread) {
	...
	if (!ws) {
		...
		Curl_bufq_init2(&ws->recvbuf, WS_CHUNK_SIZE, WS_CHUNK_COUNT, BUFQ_OPT_SOFT_LIMIT);
	...
	*/
	...
	if (result)
		return result;
	infof(data, "Received 101, switch to WebSocket; mask %02x%02x%02x%02x", ws->enc.mask[0], ws->enc.mask[1],
		  ws->enc.mask[2], ws->enc.mask[3]);

	/* Install our client writer that decodes WS frames payload */
	result = Curl_cwriter_create(&ws_dec_writer, data, &ws_cw_decode, CURL_CW_CONTENT_DECODE);
	if (result)
		return result;
...
static ssize_t ws_client_collect(const unsigned char* buf, size_t buflen, int frame_age, int frame_flags,
								 curl_off_t payload_offset, curl_off_t payload_len, void* userp, CURLcode* err) {
...
CURL_EXTERN CURLcode curl_ws_recv(struct Curl_easy* data, void* buffer, size_t buflen, size_t* nread,
...
CURL_EXTERN CURLcode curl_ws_send(CURL* data, const void* buffer, size_t buflen, size_t* sent, curl_off_t fragsize,
								  unsigned int flags) {
	struct websocket* ws;
	ssize_t nwritten, n;
	size_t space;
	...
	*sent = 0;
	...
	if (flags & CURLWS_OFFSET) {
		if (fragsize) {
			...
			if (n < 0)
				return result;
		} else if ((curl_off_t)buflen > ws->enc.payload_remain) {
			infof(data, "WS: unaligned frame size (sending %zu instead of %" CURL_FORMAT_CURL_OFF_T ")", buflen,
...
CURL_EXTERN const struct curl_ws_frame* curl_ws_meta(struct Curl_easy* data) {
	...
	if (GOOD_EASY_HANDLE(data) && Curl_is_in_callback(data) && data->conn && data->conn->proto.ws &&
...
CURL_EXTERN CURLcode curl_ws_send(CURL* curl, const void* buffer, size_t buflen, size_t* sent, curl_off_t fragsize,
								  unsigned int flags) {
	(void)curl;
	(void)buffer;
	(void)buflen;
