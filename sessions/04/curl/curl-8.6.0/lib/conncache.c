...
struct connectbundle* Curl_conncache_find_bundle(struct Curl_easy* data, struct connectdata* conn,
...
CURLcode Curl_conncache_add_conn(struct Curl_easy* data) {
	...
	DEBUGF(infof(data,
				 "Added connection %" CURL_FORMAT_CURL_OFF_T ". "
 ...
 */
bool Curl_conncache_foreach(struct Curl_easy* data, struct conncache* connc, void* param,
							int (*func)(struct Curl_easy* data, struct connectdata* conn, void* param)) {
	struct Curl_hash_iterator iter;
	struct Curl_llist_element* curr;
	struct Curl_hash_element* he;
