 ...
 */
CURLcode Curl_client_write(Curl_easy* data, int type, char* ptr, size_t len) WARN_UNUSED_RESULT;
...
struct Curl_cwtype {
	...
	CURLcode (*do_init)(Curl_easy* data, struct Curl_cwriter* writer);
	CURLcode (*do_write)(Curl_easy* data, Curl_cwriter* writer, int type, const char* buf, size_t nbytes);
 ...
 */
CURLcode Curl_cwriter_create(Curl_cwriter** pwriter, Curl_easy* data, const Curl_cwtype* ce_handler,
...
void Curl_cwriter_remove_by_name(Curl_easy* data, const char* name);

/**
 * Convenience method for calling `writer->do_write()` that
 * checks for NULL writer.
 */
CURLcode Curl_cwriter_write(Curl_easy* data, Curl_cwriter* writer, int type, const char* buf, size_t nbytes);
...
CURLcode Curl_cwriter_def_write(Curl_easy* data, Curl_cwriter* writer, int type, const char* buf, size_t nbytes);
void Curl_cwriter_def_close(Curl_easy* data, Curl_cwriter* writer);


/* internal read-function, does plain socket, SSL and krb4 */
CURLcode Curl_read(Curl_easy* data, curl_socket_t sockfd, char* buf, size_t buffersize, ssize_t* n);

/* internal write-function, does plain socket, SSL, SCP, SFTP and krb4 */
CURLcode Curl_write(Curl_easy* data, curl_socket_t sockfd, const void* mem, size_t len, ssize_t* written);

/* internal write-function, using sockindex for connection destination */
CURLcode Curl_nwrite(Curl_easy* data, int sockindex, const void* buf, size_t blen, ssize_t* pnwritten);
