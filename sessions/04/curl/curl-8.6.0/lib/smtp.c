...
static void smtp_state(struct Curl_easy* data, smtpstate newstate) {
	...
	static const char* const names[] = {
		"STOP",	   "SERVERGREET", "EHLO", "HELO", "STARTTLS", "UPGRADETLS", "AUTH",
		"COMMAND", "MAIL",		  "RCPT", "DATA", "POSTDATA", "QUIT",
		/* LAST */
	};
...
static CURLcode smtp_perform_command(struct Curl_easy* data) {
	...
	if (smtp->rcpt) {
		...
		if ((!smtp->custom) || (!smtp->custom[0])) {
			...
			result = Curl_pp_sendf(data, &conn->proto.smtpc.pp, "VRFY %s%s%s%s", address, host.name ? "@" : "",
								   host.name ? host.name : "", utf8 ? " SMTPUTF8" : "");
			...
			result = Curl_pp_sendf(data, &conn->proto.smtpc.pp, "%s %s%s", smtp->custom, smtp->rcpt->data,
								   utf8 ? " SMTPUTF8" : "");
		}
	} else {
...
/* For EHLO responses */
static CURLcode smtp_state_ehlo_resp(struct Curl_easy* data, struct connectdata* conn, int smtpcode,
									 smtpstate instate) {
	CURLcode result = CURLE_OK;
	struct smtp_conn* smtpc = &conn->proto.smtpc;
...
/* For RCPT responses */
static CURLcode smtp_state_rcpt_resp(struct Curl_easy* data, struct connectdata* conn, int smtpcode,
									 smtpstate instate) {
	CURLcode result = CURLE_OK;
	struct SMTP* smtp = data->req.p.smtp;
