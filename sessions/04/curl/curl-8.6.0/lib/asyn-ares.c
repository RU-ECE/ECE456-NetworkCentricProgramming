...
int Curl_resolver_getsock(struct Curl_easy* data, curl_socket_t* socks) {
	...
	timeout = ares_timeout((ares_channel)data->state.async.resolver, &maxtime, &timebuf);
...
static int waitperform(struct Curl_easy* data, timediff_t timeout_ms) {
	...
	bitmask = ares_getsock((ares_channel)data->state.async.resolver, socks, ARES_GETSOCK_MAXNUM);
...
static void compound_results(struct thread_data* res, struct Curl_addrinfo* ai) {
	if (!ai)
		return;
	...
	*waitp = 0; /* default to synchronous response */
	...
	if (res) {
...
#ifdef HAVE_CARES_GETADDRINFO
		{
			...
			res->num_pending = 1;
			ares_getaddrinfo((ares_channel)data->state.async.resolver, hostname, service, &hints, addrinfo_cb, data);
		}
#else
		...
		if ((data->conn->ip_version != CURL_IPRESOLVE_V4) && Curl_ipv6works(data)) {
			...
			ares_gethostbyname((ares_channel)data->state.async.resolver, hostname, PF_INET, query_completed_cb, data);
			ares_gethostbyname((ares_channel)data->state.async.resolver, hostname, PF_INET6, query_completed_cb, data);
		} else
#endif
		{
			res->num_pending = 1;
...
CURLcode Curl_set_dns_local_ip4(struct Curl_easy* data, const char* local_ip4) {
	...
	} else if (Curl_inet_pton(AF_INET, local_ip4, &a4) != 1) {
		DEBUGF(infof(data, "bad DNS IPv4 address"));
		return CURLE_BAD_FUNCTION_ARGUMENT;
	}

	ares_set_local_ip4((ares_channel)data->state.async.resolver, ntohl(a4.s_addr));
...
CURLcode Curl_set_dns_local_ip6(struct Curl_easy* data, const char* local_ip6) {
	...
	} else if (Curl_inet_pton(AF_INET6, local_ip6, a6) != 1) {
