...
static CURLcode oldap_perform_mechs(struct Curl_easy* data) {
	...
	rc = ldap_search_ext(li->ld, "", LDAP_SCOPE_BASE, "(objectclass=*)", (char**)supportedSASLMechanisms, 0, NULL, NULL,
						 NULL, 0, &li->msgid);
	if (rc == LDAP_SUCCESS)
...
static bool ssl_installed(struct connectdata* conn) { return conn->proto.ldapc->recv != NULL; }
...
static CURLcode oldap_connect(struct Curl_easy* data, bool* done) {
	...
	hosturl =
		aprintf("ldap%s://%s:%d", conn->handler->flags & PROTOPT_SSL ? "s" : "", conn->host.name, conn->remote_port);
	if (!hosturl)
		return CURLE_OUT_OF_MEMORY;
...
static CURLcode oldap_state_mechs_resp(struct Curl_easy* data, LDAPMessage* msg, int code) {
	...
	case LDAP_RES_SEARCH_ENTRY:
		...
		for (rc = ldap_get_attribute_ber(li->ld, msg, ber, &bv, &bvals); rc == LDAP_SUCCESS;
...
static CURLcode oldap_do(struct Curl_easy* data, bool* done) {
	...
	if (!result) {
		...
		rc = ldap_search_ext(li->ld, lud->lud_dn, lud->lud_scope, lud->lud_filter, lud->lud_attrs, 0, NULL, NULL, NULL,
							 0, &msgid);
		ldap_free_urldesc(lud);
		if (rc != LDAP_SUCCESS) {
...
static CURLcode client_write(struct Curl_easy* data, const char* prefix, size_t plen, const char* value, size_t len,
...
static Sockbuf_IO ldapsb_tls = {ldapsb_tls_setup, ldapsb_tls_remove, ldapsb_tls_ctrl,
								ldapsb_tls_read,  ldapsb_tls_write,	 ldapsb_tls_close};
#endif /* USE_SSL */
