...
CURLcode Curl_init_userdefined(struct Curl_easy* data) {
	 ...
	 */
	if (Curl_ssl_backend() != CURLSSLBACKEND_SCHANNEL && Curl_ssl_backend() != CURLSSLBACKEND_SECURETRANSPORT) {
...
void Curl_disconnect(struct Curl_easy* data, struct connectdata* conn, bool dead_connection) {
	...
	DEBUGF(infof(data, "Curl_disconnect(conn #%" CURL_FORMAT_CURL_OFF_T ", dead=%d)", conn->connection_id,
				 dead_connection));
	/*
...
static bool ConnectionExists(struct Curl_easy* data, struct connectdata* needle, struct connectdata** usethis,
...
#ifdef USE_NTLM
	bool wantNTLMhttp = ((data->state.authhost.want & (CURLAUTH_NTLM | CURLAUTH_NTLM_WB)) &&
	...
	*waitpipe = FALSE;
	...
	infof(data, "Found bundle for host: %p [%s]", (void*)bundle,
	...
	while (curr) {
		...
		if (needle->bits.httpproxy) {
			...
			if (IS_HTTPS_PROXY(needle->http_proxy.proxytype)) {
				...
				if (!Curl_ssl_conn_config_match(data, check, TRUE)) {
					DEBUGF(infof(
						data, "Connection #%" CURL_FORMAT_CURL_OFF_T " has different SSL proxy parameters, can't reuse",
						check->connection_id));
					continue;
				}
		...
		if ((needle->handler->flags & PROTOPT_SSL)
			...
			if ((needle->bits.conn_to_host && !strcasecompare(needle->conn_to_host.name, check->conn_to_host.name)) ||
				(needle->bits.conn_to_port && needle->conn_to_port != check->conn_to_port)) {
				continue;
			}

			/* hostname and port must match */
			if (!strcasecompare(needle->host.name, check->host.name) || needle->remote_port != check->remote_port)
				continue;

			/* If talking TLS, check needs to use the same SSL options. */
			if ((needle->handler->flags & PROTOPT_SSL) && !Curl_ssl_conn_config_match(data, check, FALSE)) {
				DEBUGF(infof(data, "Connection #%" CURL_FORMAT_CURL_OFF_T " has different SSL parameters, can't reuse",
							 check->connection_id));
				continue;
			}
		}
		...
		if (CONN_INUSE(check)) {
			...
			if (CONN_INUSE(check) >= Curl_multi_max_concurrent_streams(data->multi)) {
				infof(data,
			...
			if (CONN_INUSE(check) >= Curl_conn_get_max_concurrent(data, check, FIRSTSOCKET)) {
				infof(data, "MAX_CONCURRENT_STREAMS reached, skip (%zu)", CONN_INUSE(check));
				continue;
			}
...
const struct Curl_handler* Curl_getn_scheme_handler(const char* scheme, size_t len) {
	...
	static const struct Curl_handler* const protocols[67] = {
		...
		NULL,
		NULL,
#if !defined(CURL_DISABLE_LDAP) && !defined(CURL_DISABLE_LDAPS) &&                                                     \
	((defined(USE_OPENLDAP) && defined(USE_SSL)) || (!defined(USE_OPENLDAP) && defined(HAVE_LDAP_SSL)))
		&Curl_handler_ldaps,
#else
		NULL,
#endif
#if defined(USE_WEBSOCKETS) && defined(USE_SSL) && !defined(CURL_DISABLE_HTTP)
		&Curl_handler_wss,
#else
		NULL,
#endif
		...
		NULL,
#endif
#if defined(USE_SSL) && !defined(CURL_DISABLE_SMB) && defined(USE_CURL_NTLM_CORE) && (SIZEOF_CURL_OFF_T > 4)
		&Curl_handler_smbs,
#else
		NULL,
#endif
...
static CURLcode findprotocol(struct Curl_easy* data, struct connectdata* conn, const char* protostr) {
	...
	failf(data, "Protocol \"%s\" %s%s", protostr, p ? "disabled" : "not supported",
...
static CURLcode parseurlandfillconn(struct Curl_easy* data, struct connectdata* conn) {
	...
	if (uc) {
		...
		conn->port = conn->remote_port =
			(data->set.use_port && data->state.allow_port) ? data->set.use_port : curlx_ultous(port);
	}
 ...
 */
static CURLcode parse_connect_to_host_port(struct Curl_easy* data, const char* host, char** hostname_result,
										   int* port_result) {
	char* host_dup;
	char* hostptr;
	char* host_portno;
	...
	*port_result = -1;
	...
	if (*hostptr == '[') {
		...
		if (*ptr == '%') {
			...
			while (
				*ptr &&
				(ISALPHA(*ptr) || ISXDIGIT(*ptr) || (*ptr == '-') || (*ptr == '.') || (*ptr == '_') || (*ptr == '~'))) {
				ptr++;
			}
		}
		if (*ptr == ']') {
 ...
 */
static CURLcode parse_connect_to_string(struct Curl_easy* data, struct connectdata* conn, const char* conn_to_host,
	...
	*port_result = -1;
	...
	if (*ptr == ':') {
		...
		char* hostname_to_match =
			aprintf("%s%s%s", conn->bits.ipv6_ip ? "[" : "", conn->host.name, conn->bits.ipv6_ip ? "]" : "");
 ...
 */
static CURLcode parse_connect_to_slist(struct Curl_easy* data, struct connectdata* conn,
	...
	if (data->asi && !host && (port == -1) &&
		...
		if (hit) {
			...
			infof(data, "Alt-svc connecting from [%s]%s:%d to [%s]%s:%d", Curl_alpnid2str(srcalpnid), host,
				  conn->remote_port, Curl_alpnid2str(as->dst.alpnid), hostd, as->dst.port);
...
static CURLcode resolve_fresh(struct Curl_easy* data, struct connectdata* conn, bool* async) {
	...
	if (!unix_path && conn->socks_proxy.host.name &&
		!strncmp(UNIX_SOCKET_PREFIX "/", conn->socks_proxy.host.name, sizeof(UNIX_SOCKET_PREFIX))) {
