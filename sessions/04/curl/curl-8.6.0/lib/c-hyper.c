...
static int hyper_each_header(void* userdata, const uint8_t* name, size_t name_len, const uint8_t* value,
							 size_t value_len) {
...
static CURLcode status_line(struct Curl_easy* data, struct connectdata* conn, uint16_t http_status, int http_version,
	...
	if (data->state.hconnect) {
		...
		conn->httpversion =
			http_version == HYPER_HTTP_VERSION_1_1 ? 11 : (http_version == HYPER_HTTP_VERSION_2 ? 20 : 10);
...
static CURLcode debug_request(struct Curl_easy* data, const char* method, const char* path) {
...
static CURLcode request_target(struct Curl_easy* data, struct connectdata* conn, const char* method,
							   hyper_request* req) {
	CURLcode result;
	struct dynbuf r;
	...
	if (hyper_request_set_uri(req, (uint8_t*)Curl_dyn_uptr(&r), Curl_dyn_len(&r))) {
...
static CURLcode bodysend(struct Curl_easy* data, struct connectdata* conn, hyper_headers* headers,
						 hyper_request* hyperreq, Curl_HttpReq httpreq) {
