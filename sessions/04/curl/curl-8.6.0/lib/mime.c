...
#include "warnless.h"
	...
	(!defined(CURL_DISABLE_HTTP) || !defined(CURL_DISABLE_SMTP) || !defined(CURL_DISABLE_IMAP))
...
static const struct mime_encoder encoders[] = {
	{"binary", encoder_nop_read, encoder_nop_size},			{"8bit", encoder_nop_read, encoder_nop_size},
...
static curl_off_t encoder_nop_size(curl_mimepart* part) { return part->datasize; }
...
static size_t readback_bytes(struct mime_state* state, char* buffer, size_t bufsize, const char* bytes, size_t numbytes,
...
static size_t readback_part(curl_mimepart* part, char* buffer, size_t bufsize, bool* hasread) {
	...
	while (bufsize) {
		...
		case MIMESTATE_BEGIN:
			mimesetstate(&part->state, (part->flags & MIME_BODY_ONLY) ? MIMESTATE_BODY : MIMESTATE_CURLHEADERS,
						 part->curlheaders);
			break;
	...
	while (nitems) {
		...
		case MIMESTATE_BOUNDARY2:
			if (part) {
				...
				sz =
					readback_bytes(&mime->state, buffer, nitems, mime->boundary, MIME_BOUNDARY_LEN, STRCONST("--\r\n"));
			}
			if (!sz)
...
curl_mime* curl_mime_init(struct Curl_easy* easy) {
	...
	if (mime) {
		...
		if (Curl_rand_alnum(easy, (unsigned char*)&mime->boundary[MIME_BOUNDARY_DASHES],
...
static CURLcode add_content_type(struct curl_slist** slp, const char* type, const char* boundary) {
	return Curl_mime_add_header(slp, "Content-Type: %s%s%s", type, boundary ? "; boundary=" : "",
								boundary ? boundary : "");
}
...
CURLcode Curl_mime_prepare_headers(struct Curl_easy* data, curl_mimepart* part, const char* contenttype,
								   const char* disposition, enum mimestrategy strategy) {
	curl_mime* mime = NULL;
	...
	if (!search_header(part->userheaders, STRCONST("Content-Disposition"))) {
		...
		if (disposition) {
			...
			if (!ret) {
				ret = Curl_mime_add_header(&part->curlheaders, "Content-Disposition: %s%s%s%s%s%s%s", disposition,
