 ...
 */
struct Curl_dns_entry* Curl_cache_addr(struct Curl_easy* data, struct Curl_addrinfo* addr, const char* hostname,
...
enum resolve_t Curl_resolv(struct Curl_easy* data, const char* hostname, int port, bool allowDOH,
...
enum resolve_t Curl_resolv_timeout(struct Curl_easy* data, const char* hostname, int port,
								   struct Curl_dns_entry** entry, timediff_t timeoutms) {
#ifdef USE_ALARM_TIMEOUT
...
#ifndef CURL_DISABLE_VERBOSE_STRINGS
			infof(data, "Added %.*s:%d:%s to DNS cache%s", (int)hlen, host_begin, port, addresses,
