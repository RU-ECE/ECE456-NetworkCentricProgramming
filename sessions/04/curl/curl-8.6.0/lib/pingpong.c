...
static CURLcode pingpong_read(struct Curl_easy* data, curl_socket_t sockfd, char* buffer, size_t buflen,
							  ssize_t* nread) {
	CURLcode result;
#ifdef HAVE_GSSAPI
 ...
 */
CURLcode Curl_pp_readresp(struct Curl_easy* data, curl_socket_t sockfd, struct pingpong* pp,
...
CURLcode Curl_pp_flushsend(struct Curl_easy* data, struct pingpong* pp) {
	...
	ssize_t written;
	CURLcode result =
		Curl_nwrite(data, FIRSTSOCKET, pp->sendthis + pp->sendsize - pp->sendleft, pp->sendleft, &written);
	if (result)
		return result;
...
bool Curl_pp_moredata(struct pingpong* pp) { return !pp->sendleft && Curl_dyn_len(&pp->recvbuf); }
