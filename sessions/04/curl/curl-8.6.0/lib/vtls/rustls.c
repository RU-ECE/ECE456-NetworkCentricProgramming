...
static ssize_t tls_recv_more(struct Curl_cfilter* cf, struct Curl_easy* data, CURLcode* err) {
	...
	if (rresult != RUSTLS_RESULT_OK) {
		...
		failf(data, "rustls_connection_process_new_packets: %.*s", (int)errorlen, errorbuf);
 ...
 * output buffer.
 */
static ssize_t cr_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* plainbuf, size_t plainlen,
					   CURLcode* err) {
	struct ssl_connect_data* const connssl = cf->ctx;
	struct rustls_ssl_backend_data* const backend = (struct rustls_ssl_backend_data*)connssl->backend;
	...
	while (plain_bytes_copied < plainlen) {
		...
		rresult =
			rustls_connection_read(rconn, (uint8_t*)plainbuf + plain_bytes_copied, plainlen - plain_bytes_copied, &n);
 ...
 */
static ssize_t cr_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* plainbuf, size_t plainlen,
					   CURLcode* err) {
	struct ssl_connect_data* const connssl = cf->ctx;
	struct rustls_ssl_backend_data* const backend = (struct rustls_ssl_backend_data*)connssl->backend;
	struct rustls_connection* rconn = NULL;
...
static enum rustls_result cr_verify_none(void* userdata UNUSED_PARAM,
...
static void cr_set_negotiated_alpn(struct Curl_cfilter* cf, struct Curl_easy* data,
 ...
 * `*done*`.
 */
static CURLcode cr_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool blocking, bool* done) {
	struct ssl_connect_data* const connssl = cf->ctx;
	...
	for (;;) {
		...
		what = Curl_socket_check(readfd, CURL_SOCKET_BAD, writefd, socket_check_timeout);
		if (what < 0) {
			/* fatal error */
...
static CURLcode cr_connect_blocking(struct Curl_cfilter* cf UNUSED_PARAM, struct Curl_easy* data UNUSED_PARAM) {
	bool done; /* unused */
