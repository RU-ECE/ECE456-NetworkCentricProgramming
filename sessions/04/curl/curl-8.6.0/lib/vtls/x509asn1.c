...
static const struct Curl_OID OIDtable[] = {{"1.2.840.10040.4.1", "dsa"},
 ...
 * Returns error.
 */
static CURLcode utf8asn1str(struct dynbuf* to, int type, const char* from, const char* end) {
...
static CURLcode GTime2str(struct dynbuf* store, const char* beg, const char* end) {
	...
	tzl = end - tzp;
	return Curl_dyn_addf(store, "%.4s-%.2s-%.2s %.2s:%.2s:%c%c%s%.*s%s%.*s", beg, beg + 4, beg + 6, beg + 8, beg + 10,
						 sec1, sec2, fracl ? "." : "", (int)fracl, fracp, sep, (int)tzl, tzp);
}
...
static CURLcode UTime2str(struct dynbuf* store, const char* beg, const char* end) {
	...
	tzl = end - tzp;
	return Curl_dyn_addf(store, "%u%.2s-%.2s-%.2s %.2s:%.2s:%.2s %.*s", 20 - (*beg >= '5'), beg, beg + 2, beg + 4,
						 beg + 6, beg + 8, sec, (int)tzl, tzp);
}
...
int Curl_parseX509(struct Curl_X509certificate* cert, const char* beg, const char* end) {
	...
	if (!beg)
		return -1;
	ccp = getASN1Element(&cert->subjectPublicKeyAlgorithm, cert->subjectPublicKeyInfo.beg,
...
static int do_pubkey(struct Curl_easy* data, int certnum, const char* algo, struct Curl_asn1Element* param,
 ...
 * Return error.
 */
static CURLcode DNtostr(struct dynbuf* store, struct Curl_asn1Element* dn) { return encodeDN(store, dn); }
