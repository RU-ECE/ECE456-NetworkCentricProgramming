...
#if (OPENSSL_VERSION_NUMBER >= 0x10100000L) &&                                                                         \
	!(defined(LIBRESSL_VERSION_NUMBER) && LIBRESSL_VERSION_NUMBER < 0x2070100fL) && !defined(OPENSSL_IS_BORINGSSL) &&  \
...
static BIO_METHOD* ossl_bio_cf_method_create(void) { return &ossl_bio_cf_meth_1_0; }
 ...
 */
static bool is_pkcs11_uri(const char* string) { return string && strncasecompare(string, "pkcs11:", 7); }
...
static int cert_stuff(struct Curl_easy* data, SSL_CTX* ctx, char* cert_file, const struct curl_blob* cert_blob,
					  const char* cert_type, char* key_file, const struct curl_blob* key_blob, const char* key_type,
					  char* key_passwd) {
	...
	if (cert_file || cert_blob || (file_type == SSL_FILETYPE_ENGINE)) {
		...
		case SSL_FILETYPE_ASN1:
			...
			if (cert_use_result != 1) {
				failf(data, "unable to set private key file: '%s' type %s", key_file ? key_file : "(memory blob)",
...
CURLcode Curl_ossl_set_client_cert(struct Curl_easy* data, SSL_CTX* ctx, char* cert_file,
								   const struct curl_blob* cert_blob, const char* cert_type, char* key_file,
			...
			if (SSL_shutdown(backend->handle) == 1) {
				...
				default:
					...
					CURL_TRC_CF(data, cf, "SSL shutdown, error: '%s', errno %d",
								(sslerr ? ossl_strerror(sslerr, buf, sizeof(buf)) : SSL_ERROR_to_str(err)), SOCKERRNO);
					break;
				}
			}
 ...
 */
static bool subj_alt_hostcheck(struct Curl_easy* data, const char* match_pattern, size_t matchlen, const char* hostname,
							   size_t hostlen, const char* dispname) {
...
*/
CURLcode Curl_ossl_verifyhost(struct Curl_easy* data, struct connectdata* conn, struct ssl_peer* peer,
							  X509* server_cert) {
	bool matched = FALSE;
...
#ifdef ENABLE_IPV6
		if (conn->bits.ipv6_ip && Curl_inet_pton(AF_INET6, peer->hostname, &addr)) {
			target = GEN_IPADD;
			...
			if (check->type == target) {
				...
				case GEN_DNS: /* name/pattern comparison */
					...
					*/
					if ((altlen == strlen(altptr)) &&
						...
						subj_alt_hostcheck(data, altptr, altlen, peer->hostname, hostlen, peer->dispname)) {
						dnsmatched = TRUE;
					}
					break;
				...
				case GEN_IPADD: /* IP address comparison */
					...
					if ((altlen == addrlen) && !memcmp(altptr, &addr, altlen)) {
						ipmatched = TRUE;
						infof(data, " subjectAltName: host \"%s\" matched cert's IP address!", peer->dispname);
					}
					break;
				}
			}
	...
	if (matched)
		...
		if (result)
			...
			result = CURLE_PEER_FAILED_VERIFICATION;
		} else if (!Curl_cert_hostcheck((const char*)peer_CN, peerlen, peer->hostname, hostlen)) {
			failf(data,
 ...
 */
static void ossl_trace(int direction, int ssl_ver, int content_type, const void* buf, size_t len, SSL* ssl,
					   void* userp) {
	const char* verstr = "???";
...
#if (OPENSSL_VERSION_NUMBER < 0x10100000L) /* 1.1.0 */
static CURLcode ossl_set_ssl_version_min_max_legacy(ctx_option_t* ctx_options, struct Curl_cfilter* cf,
	...
	*imported = false;
	...
	if (hStore) {
		...
		for (;;) {
			...
			if (CertGetIntendedKeyUsage(pContext->dwCertEncodingType, pContext->pCertInfo, key_usage,
										sizeof(key_usage))) {
...
static CURLcode populate_x509_store(struct Curl_cfilter* cf, struct Curl_easy* data, X509_STORE* store) {
	...
	CURL_TRC_CF(data, cf, "populate_x509_store, path=%s, blob=%d", ssl_cafile ? ssl_cafile : "none", !!ca_info_blob);
	if (!store)
		return CURLE_OUT_OF_MEMORY;
	...
	if (verifypeer) {
		...
		if (ssl_config->native_ca_store) {
			...
			for (i = 0; i < ARRAYSIZE(storeNames); ++i) {
				bool imported = false;
				result = import_windows_cert_store(data, storeNames[i], store, &imported);
				if (result)
					return result;
				if (imported) {
					...
					infof(data, "error importing Windows %s store, continuing anyway", storeNames[i]);
				}
			}
		}
#endif
...
CURLcode Curl_ssl_setup_x509_store(struct Curl_cfilter* cf, struct Curl_easy* data, SSL_CTX* ssl_ctx) {
	...
	/* Consider the X509 store cacheable if it comes exclusively from a CAfile,
	   ...
	   default. */
	cache_criteria_met = (data->set.general_ssl.ca_cache_timeout != 0) && conn_config->verifypeer &&
		!conn_config->CApath && !conn_config->ca_info_blob && !ssl_config->primary.CRLfile &&
...
static CURLcode ossl_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	*/
	...
	if (ssl_cert || ssl_cert_blob || ssl_cert_type) {
		if (!result &&
			!cert_stuff(data, backend->ctx, ssl_cert, ssl_cert_blob, ssl_cert_type, ssl_config->key,
...
static CURLcode ossl_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (1 != err) {
		...
		if (backend->io_result == CURLE_AGAIN) {
			...
			if ((lib == ERR_LIB_SSL) &&
				...
				if (lerr != X509_V_OK) {
					...
					msnprintf(error_buffer, sizeof(error_buffer), "SSL certificate problem: %s",
			...
			if (CURLE_SSL_CONNECT_ERROR == result && errdetail == 0) {
				...
				failf(data, OSSL_PACKAGE " SSL_connect: %s in connection to %s:%d ",
					  extramsg[0] ? extramsg : SSL_ERROR_to_str(detail), connssl->peer.hostname, connssl->port);
				return result;
			}
		...
		/* Informational message */
		infof(data, "SSL connection using %s / %s / %s / %s", SSL_get_version(backend->handle),
			  SSL_get_cipher(backend->handle), negotiated_group_name ? negotiated_group_name : "[blank]",
...
#if (OPENSSL_VERSION_NUMBER >= 0x10100000L) &&                                                                         \
	!(defined(LIBRESSL_VERSION_NUMBER) && LIBRESSL_VERSION_NUMBER < 0x3060000fL) && !defined(OPENSSL_IS_BORINGSSL) &&  \
	!defined(OPENSSL_IS_AWSLC) && !defined(CURL_DISABLE_VERBOSE_STRINGS)
...
#define EVP_PKEY_get_security_bits EVP_PKEY_security_bits
		...
		infof(data,
			  ...
			  cert_level, type_name ? type_name : "?", get_group_name == 0 ? "" : group_name_final, key_bits,
			  key_sec_bits, cert_algorithm);
	}
}
#else
...
static CURLcode servercert(struct Curl_cfilter* cf, struct Curl_easy* data, bool strict) {
	...
	if (conn_config->verifyhost) {
		result = Curl_ossl_verifyhost(data, conn, &connssl->peer, backend->server_cert);
		if (result) {
	...
	ptr = Curl_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY]
...
static ssize_t ossl_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len,
						 CURLcode* curlcode) {
	...
	if (rc <= 0) {
		...
		case SSL_ERROR_SYSCALL: {
			...
			if (sslerror)
				...
				msnprintf(error_buffer, sizeof(error_buffer), "%s", SSL_ERROR_to_str(err));
...
static size_t ossl_version(char* buffer, size_t size) {
	...
	return count;
#else
	return msnprintf(buffer, size, "%s/%lx.%lx.%lx", OSSL_PACKAGE, (LIBRESSL_VERSION_NUMBER >> 28) & 0xf,
					 (LIBRESSL_VERSION_NUMBER >> 20) & 0xff, (LIBRESSL_VERSION_NUMBER >> 12) & 0xff);
#endif
#elif defined(OPENSSL_IS_BORINGSSL)
...
#endif
					 ,
					 OSSL_PACKAGE, (ssleay_value >> 28) & 0xf, (ssleay_value >> 20) & 0xff, (ssleay_value >> 12) & 0xff,
					 sub);
