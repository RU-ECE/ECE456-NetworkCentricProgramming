...
static CURLcode add_certs_data_to_store(HCERTSTORE trust_store, const char* ca_buffer, size_t ca_buffer_size,
										const char* ca_file_text, struct Curl_easy* data) {
	...
	while (more_certs && (current_ca_file_ptr < ca_buffer_limit)) {
		const char* begin_cert_ptr =
			c_memmem(current_ca_file_ptr, ca_buffer_limit - current_ca_file_ptr, BEGIN_CERT, begin_cert_len);
		if (!begin_cert_ptr || !is_cr_or_lf(begin_cert_ptr[begin_cert_len])) {
			more_certs = 0;
		} else {
			const char* end_cert_ptr =
				c_memmem(begin_cert_ptr, ca_buffer_limit - begin_cert_ptr, END_CERT, end_cert_len);
			if (!end_cert_ptr) {
				...
				if (!CryptQueryObject(CERT_QUERY_OBJECT_BLOB, &cert_blob, CERT_QUERY_CONTENT_FLAG_CERT,
									  CERT_QUERY_FORMAT_FLAG_ALL, 0, NULL, &actual_content_type, NULL, NULL, NULL,
					...
					if (CERT_QUERY_CONTENT_CERT != actual_content_type) {
						...
						add_cert_result =
							CertAddCertificateContextToStore(trust_store, cert_context, CERT_STORE_ADD_ALWAYS, NULL);
...
static CURLcode add_certs_file_to_store(HCERTSTORE trust_store, const char* ca_file, struct Curl_easy* data) {
	 ...
	 */
	ca_file_handle =
		CreateFile(ca_file_tstr, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	if (!GetFileSizeEx(ca_file_handle, &file_size)) {
		char buffer[STRERROR_LEN];
		failf(data, "schannel: failed to determine size of CA file '%s': %s", ca_file,
			  Curl_winapi_strerror(GetLastError(), buffer, sizeof(buffer)));
		result = CURLE_SSL_CACERT_BADFILE;
		goto cleanup;
	}
...
static DWORD cert_get_name_string(struct Curl_easy* data, CERT_CONTEXT* cert_context, LPTSTR host_names, DWORD length) {
	...
	ret_val =
		CryptDecodeObjectEx(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, szOID_SUBJECT_ALT_NAME2, extension->Value.pbData,
							extension->Value.cbData, CRYPT_DECODE_ALLOC_FLAG | CRYPT_DECODE_NOCOPY_FLAG, &decode_para,
...
CURLcode Curl_verify_host(struct Curl_cfilter* cf, struct Curl_easy* data) {
	 ...
	 */
	result = CURLE_PEER_FAILED_VERIFICATION;
	while (cert_hostname_buff_index < len && cert_hostname_buff[cert_hostname_buff_index] != TEXT('\0') &&
		   result == CURLE_PEER_FAILED_VERIFICATION) {
...
CURLcode Curl_verify_certificate(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (result == CURLE_OK && (conn_config->CAfile || conn_config->ca_info_blob) &&
		...
		if (curlx_verify_windows_version(6, 1, 0, PLATFORM_WINNT, VERSION_LESS_THAN)) {
			...
			if (trust_store) {
				...
				trust_store =
					CertOpenStore(CERT_STORE_PROV_MEMORY, 0, (HCRYPTPROV)NULL, CERT_STORE_CREATE_NEW_FLAG, NULL);
				if (!trust_store) {
					char buffer[STRERROR_LEN];
					...
					if (ca_info_blob) {
						result = add_certs_data_to_store(trust_store, (const char*)ca_info_blob->data,
														 ca_info_blob->len, "(memory blob)", data);
					} else {
		...
		if (result == CURLE_OK) {
			...
			create_engine_result =
				CertCreateCertificateChainEngine((CERT_CHAIN_ENGINE_CONFIG*)&engine_config, &cert_chain_engine);
	...
	if (result == CURLE_OK) {
		...
		if (!CertGetCertificateChain(cert_chain_engine, pCertContextServer, NULL, pCertContextServer->hCertStore,
									 &ChainPara, (ssl_config->no_revoke ? 0 : CERT_CHAIN_REVOCATION_CHECK_CHAIN), NULL,
									 &pChainContext)) {
			char buffer[STRERROR_LEN];
		...
		if (result == CURLE_OK) {
			...
			if (dwTrustErrorMask) {
				if (dwTrustErrorMask & CERT_TRUST_IS_REVOKED) {
					...
					failf(data, "schannel: CertGetCertificateChain error mask: 0x%08lx", dwTrustErrorMask);
				}
				result = CURLE_PEER_FAILED_VERIFICATION;
			}
		}
