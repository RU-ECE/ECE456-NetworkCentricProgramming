...
static const unsigned char ecDsaSecp256r1SpkiHeader[] = {0x30, 0x59, 0x30, 0x13, 0x06, 0x07, 0x2a, 0x86, 0x48,
														 0xce, 0x3d, 0x02, 0x01, 0x06, 0x08, 0x2a, 0x86, 0x48,
														 0xce, 0x3d, 0x03, 0x01, 0x07, 0x03, 0x42, 0x00};

static const unsigned char ecDsaSecp384r1SpkiHeader[] = {0x30, 0x76, 0x30, 0x10, 0x06, 0x07, 0x2a, 0x86,
...
static OSStatus sectransp_bio_cf_out_write(SSLConnectionRef connection, const void* buf,
...
static OSStatus CopyIdentityWithLabel(char* label, SecIdentityRef* out_cert_and_key) {
	...
	if (SecItemCopyMatching && kSecClassIdentity) {
		...
		query_dict = CFDictionaryCreate(NULL, (const void**)keys, (const void**)values, 5L,
...
static OSStatus CopyIdentityFromPKCS12File(const char* cPath, const struct curl_blob* blob, const char* cPassword,
	...
	if (ssl_config->primary.sessionid) {
		...
		/* If there isn't one, then let's make one up! This has to be done prior
		   to starting the handshake. */
			...
			CURLcode result;
			ssl_sessionid = aprintf("%s:%d:%d:%s:%d", ssl_cafile ? ssl_cafile : "(blob memory)", verifypeer,
									conn_config->verifyhost, connssl->peer.hostname, connssl->port);
...
static int append_cert_to_array(struct Curl_easy* data, const unsigned char* buf, size_t buflen,
...
static CURLcode verify_cert_buf(struct Curl_cfilter* cf, struct Curl_easy* data, const unsigned char* certbuf,
								size_t buflen, SSLContextRef ctx) {
	int n = 0, rc;
	long res;
	unsigned char* der;
...
static CURLcode verify_cert(struct Curl_cfilter* cf, struct Curl_easy* data, const char* cafile,
							const struct curl_blob* ca_info_blob, SSLContextRef ctx) {
	CURLcode result;
...
#ifdef SECTRANSP_PINNEDPUBKEY
static CURLcode pkp_pin_peer_pubkey(struct Curl_easy* data, SSLContextRef ctx, const char* pinnedpubkey) { /* Scratch */
	...
	if (err != noErr) {
		...
		case -9841:
			if ((conn_config->CAfile || conn_config->ca_info_blob) && conn_config->verifypeer) {
				CURLcode result =
					verify_cert(cf, data, conn_config->CAfile, conn_config->ca_info_blob, backend->ssl_ctx);
				if (result)
					return result;
			}
		...
		default:
			...
			failf(data, "Unknown SSL protocol error in connection to %s:%d", connssl->peer.hostname, err);
			break;
		}
		return CURLE_SSL_CONNECT_ERROR;
	} else {
		...
		if (connssl->alpn) {
			if (__builtin_available(macOS 10.13.4, iOS 11, tvOS 11, *)) {
				...
				Curl_multiuse_state(data,
									cf->conn->alpn == CURL_HTTP_VERSION_2 ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
...
static CURLcode collect_server_cert_single(struct Curl_cfilter* cf, struct Curl_easy* data,
										   SecCertificateRef server_cert, CFIndex idx) {
	CURLcode result = CURLE_OK;
	struct ssl_config_data* ssl_config = Curl_ssl_cf_get_config(cf, data);
...
static CURLcode sectransp_connect_common(struct Curl_cfilter* cf, struct Curl_easy* data, bool nonblocking,
										 bool* done) {
	CURLcode result;
	struct ssl_connect_data* connssl = cf->ctx;
...
static size_t sectransp_version(char* buffer, size_t size) { return msnprintf(buffer, size, "SecureTransport"); }
...
static ssize_t sectransp_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len,
							  CURLcode* curlcode) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
...
static ssize_t sectransp_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t buffersize,
							  CURLcode* curlcode) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct st_ssl_backend_data* backend = (struct st_ssl_backend_data*)connssl->backend;
	struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
