	...
	for (;;) {
		...
		if ((rc == GNUTLS_E_AGAIN) || (rc == GNUTLS_E_INTERRUPTED)) {
			connssl->connecting_state =
				gnutls_record_get_direction(session) ? ssl_connect_2_writing : ssl_connect_2_reading;
			continue;
...
static CURLcode set_ssl_version_min_max(struct Curl_easy* data, struct ssl_primary_config* conn_config,
										const char** prioritylist, const char* tls13support) {
	...
	*pverifyresult = 0;
	...
	if (config->verifypeer) {
		...
		if (config->CAfile) {
			...
			if (rc < 0) {
				infof(data, "error reading ca cert file %s (%s)%s", config->CAfile, gnutls_strerror(rc),
		...
		if (config->CApath) {
			...
			if (rc < 0) {
				infof(data, "error reading ca cert file %s (%s)%s", config->CApath, gnutls_strerror(rc),
					  (imported_native_ca ? ", continuing anyway" : ""));
				if (!imported_native_ca) {
					*pverifyresult = rc;
					return CURLE_SSL_CACERT_BADFILE;
	...
	if (sni && peer->sni) {
		if (gnutls_server_name_set(gtls->session, GNUTLS_NAME_DNS, peer->sni, strlen(peer->sni)) < 0) {
	...
	if (config->clientcert) {
		if (ssl_config->key_passwd) {
			const unsigned int supported_key_encryption_algorithms = GNUTLS_PKCS_USE_PKCS12_3DES |
				GNUTLS_PKCS_USE_PKCS12_ARCFOUR | GNUTLS_PKCS_USE_PKCS12_RC2_40 | GNUTLS_PKCS_USE_PBES2_3DES |
				GNUTLS_PKCS_USE_PBES2_AES_128 | GNUTLS_PKCS_USE_PBES2_AES_192 | GNUTLS_PKCS_USE_PBES2_AES_256;
			rc = gnutls_certificate_set_x509_key_file2(
				gtls->cred, config->clientcert, ssl_config->key ? ssl_config->key : config->clientcert,
		...
		} else if (gnutls_certificate_set_x509_key_file(gtls->cred, config->clientcert,
...
static CURLcode gtls_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	result = gtls_client_init(data, conn_config, ssl_config, &connssl->peer, &backend->gtls, pverifyresult);
	if (result)
		return result;
...
#define use_addr in_addr
	...
	if (!rc) {
		if (config->verifyhost) {
			...
			infof(data, "  common name: %s (does not match '%s')", certname, peer->dispname);
		}
	} else {
...
static CURLcode gtls_verifyserver(struct Curl_cfilter* cf, struct Curl_easy* data, gnutls_session_t session) {
	...
	struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
	struct ssl_config_data* ssl_config = Curl_ssl_cf_get_config(cf, data);
	const char* pinned_key = Curl_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY]
													  : data->set.str[STRING_SSL_PINNEDPUBLICKEY];
	CURLcode result;

	result = Curl_gtls_verifyserver(data, session, conn_config, ssl_config, &connssl->peer, pinned_key);
	if (result)
		goto out;
...
static ssize_t gtls_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len,
						 CURLcode* curlcode) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
	ssize_t rc;
...
static ssize_t gtls_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t buffersize,
						 CURLcode* curlcode) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct gtls_ssl_backend_data* backend = (struct gtls_ssl_backend_data*)connssl->backend;
	ssize_t ret;
