...
static CURLcode schannel_set_ssl_version_min_max(DWORD* enabled_protocols, struct Curl_cfilter* cf,
...
static CURLcode schannel_acquire_credential_handle(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (conn_config->verifypeer) {
		...
		flags =
			SCH_CRED_MANUAL_CRED_VALIDATION | SCH_CRED_IGNORE_NO_REVOCATION_CHECK | SCH_CRED_IGNORE_REVOCATION_OFFLINE;
	...
	if (data->set.ssl.primary.clientcert || data->set.ssl.primary.cert_blob) {
		...
		if (fInCert || blob) {
			...
			*/
			...
			if (pszPassword) {
				if (pwd_len > 0) {
					str_w_len = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, data->set.ssl.key_passwd,
			...
			client_certs[0] = CertFindCertificateInStore(cert_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,
			...
			cert_store = CertOpenStore(CURL_CERT_STORE_PROV_SYSTEM, 0, (HCRYPTPROV)NULL,
									   CERT_STORE_OPEN_EXISTING_FLAG | cert_store_name, cert_store_path);
			if (!cert_store) {
				char* path_utf8 = curlx_convert_tchar_to_UTF8(cert_store_path);
				failf(data,
					  "schannel: Failed to open cert store %lx %s, "
					  "last error is 0x%lx",
					  cert_store_name, (path_utf8 ? path_utf8 : "(unknown)"), GetLastError());
			...
			if (!CryptStringToBinary(cert_thumbprint_str, CERT_THUMBPRINT_STR_LEN, CRYPT_STRING_HEX,
									 cert_thumbprint_data, &cert_thumbprint.cbData, NULL, NULL)) {
				curlx_unicodefree(cert_path);
				CertCloseStore(cert_store, 0);
				return CURLE_SSL_CERTPROBLEM;
			}
	...
	if (!conn_config->cipher_list &&
		curlx_verify_windows_version(10, 0, 17763, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
		...
		if (ciphers13) {
			...
			*/
			...
			while (startCur && (0 != *startCur) && (algCount < remaining_ciphers)) {
				...
				if (disable_aes_gcm_sha384 && algo("TLS_AES_256_GCM_SHA384", namep, n)) {
					disable_aes_gcm_sha384 = FALSE;
				} else if (disable_aes_gcm_sha256 && algo("TLS_AES_128_GCM_SHA256", namep, n)) {
					disable_aes_gcm_sha256 = FALSE;
				} else if (disable_chacha_poly && algo("TLS_CHACHA20_POLY1305_SHA256", namep, n)) {
					disable_chacha_poly = FALSE;
				} else if (disable_aes_ccm_8_sha256 && algo("TLS_AES_128_CCM_8_SHA256", namep, n)) {
					disable_aes_ccm_8_sha256 = FALSE;
				} else if (disable_aes_ccm_sha256 && algo("TLS_AES_128_CCM_SHA256", namep, n)) {
		...
		if (disable_aes_gcm_sha384 && disable_aes_gcm_sha256 && disable_chacha_poly && disable_aes_ccm_8_sha256 &&
			...
			*/
			...
			if (disable_aes_gcm_sha384 != disable_aes_gcm_sha256) {
				...
				crypto_settings[crypto_settings_idx].strCngAlgId.Buffer =
					(PWSTR)(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM : BCRYPT_SHA256_ALGORITHM);
		...
		*/
		...
		sspi_status =
			s_pSecFn->AcquireCredentialsHandle(NULL, (TCHAR*)UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &credentials, NULL,
											   NULL, &backend->cred->cred_handle, &backend->cred->time_stamp);
	} else {
		...
		sspi_status =
			s_pSecFn->AcquireCredentialsHandle(NULL, (TCHAR*)UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &schannel_cred,
...
static CURLcode schannel_connect_step1(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	DEBUGASSERT(backend);
	DEBUGF(
		infof(data, "schannel: SSL/TLS connection with %s port %d (step 1/3)", connssl->peer.hostname, connssl->port));
...
#error "compiler too old to support requisite manual cert verify for Win CE"
	...
	if (backend->use_alpn) {
		...
		*extension_len = (unsigned int)(*list_len + sizeof(unsigned int) + sizeof(unsigned short));
	...
	*/
	sspi_status = s_pSecFn->InitializeSecurityContext(
		&backend->cred->cred_handle, NULL, backend->cred->sni_hostname, backend->req_flags, 0, 0,
		(backend->use_alpn ? &inbuf_desc : NULL), 0, &backend->ctxt->ctxt_handle, &outbuf_desc, &backend->ret_flags,
...
static CURLcode schannel_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	DEBUGF(
		infof(data, "schannel: SSL/TLS connection with %s port %d (step 2/3)", connssl->peer.hostname, connssl->port));
	...
	for (;;) {
		if (doread) {
			...
			nread = Curl_conn_cf_recv(cf->next, data, (char*)(backend->encdata_buffer + backend->encdata_offset),
		...
		DEBUGF(infof(data, "schannel: encrypted data buffer: offset %zu length %zu", backend->encdata_offset,
		...
		sspi_status = s_pSecFn->InitializeSecurityContext(
			&backend->cred->cred_handle, &backend->ctxt->ctxt_handle, backend->cred->sni_hostname, backend->req_flags,
			0, 0, &inbuf_desc, 0, NULL, &outbuf_desc, &backend->ret_flags, &backend->ctxt->time_stamp);
			...
			*/
			...
			if (backend->encdata_offset > inbuf[1].cbBuffer) {
				memmove(backend->encdata_buffer,
						(backend->encdata_buffer + backend->encdata_offset) - inbuf[1].cbBuffer, inbuf[1].cbBuffer);
	...
	pubkey_ptr = Curl_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY]
...
static void traverse_cert_store(const CERT_CONTEXT* context, Read_crt_func func, void* arg) {
	...
	while (should_continue &&
		   (current_context = CertEnumCertificatesInStore(context->hCertStore, current_context)) != NULL) {
...
static CURLcode schannel_connect_step3(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	DEBUGF(
		infof(data, "schannel: SSL/TLS connection with %s port %d (step 3/3)", connssl->peer.hostname, connssl->port));
	...
	if (data->set.ssl.certinfo) {
		int certs_count = 0;
		sspi_status = s_pSecFn->QueryContextAttributes(&backend->ctxt->ctxt_handle, SECPKG_ATTR_REMOTE_CERT_CONTEXT,
													   &ccert_context);
...
static ssize_t schannel_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							 CURLcode* err) {
	ssize_t written = -1;
	size_t data_len = 0;
	...
	if (backend->stream_sizes.cbMaximumMessage == 0) {
		sspi_status = s_pSecFn->QueryContextAttributes(&backend->ctxt->ctxt_handle, SECPKG_ATTR_STREAM_SIZES,
	...
	InitSecBuffer(&outbuf[2], SECBUFFER_STREAM_TRAILER, ptr + backend->stream_sizes.cbHeader + len,
...
static ssize_t schannel_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t len, CURLcode* err) {
	...
	*/
		...
		nread =
			Curl_conn_cf_recv(cf->next, data, (char*)(backend->encdata_buffer + backend->encdata_offset), size, err);
		if (*err) {
			nread = -1;
			if (*err == CURLE_AGAIN)
	...
	while (backend->encdata_offset > 0 && sspi_status == SEC_E_OK &&
		...
		if (sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE || sspi_status == SEC_I_CONTEXT_EXPIRED) {
			...
			if (inbuf[1].BufferType == SECBUFFER_DATA) {
				...
				size = inbuf[1].cbBuffer > CURL_SCHANNEL_BUFFER_FREE_SIZE ? inbuf[1].cbBuffer
				...
				DEBUGF(infof(data, "schannel: decrypted cached: offset %zu length %zu", backend->decdata_offset,
			...
			if (inbuf[3].BufferType == SECBUFFER_EXTRA && inbuf[3].cbBuffer > 0) {
				...
				if (backend->encdata_offset > inbuf[3].cbBuffer) {
					/* move remaining encrypted data forward to the beginning of
					   buffer */
					memmove(backend->encdata_buffer,
							(backend->encdata_buffer + backend->encdata_offset) - inbuf[3].cbBuffer, inbuf[3].cbBuffer);
					backend->encdata_offset = inbuf[3].cbBuffer;
				}

				DEBUGF(infof(data, "schannel: encrypted cached: offset %zu length %zu", backend->encdata_offset,
							 backend->encdata_length));
			} else {
	...
	DEBUGF(infof(data, "schannel: decrypted data buffer: offset %zu length %zu", backend->decdata_offset,
				 backend->decdata_length));
...
static int schannel_shutdown(struct Curl_cfilter* cf, struct Curl_easy* data) {
	...
	if (backend->ctxt) {
		infof(data, "schannel: shutting down SSL/TLS connection with %s port %d", connssl->peer.hostname,
			  connssl->port);
	}
	...
	if (backend->cred && backend->ctxt) {
		...
		sspi_status = s_pSecFn->InitializeSecurityContext(
			&backend->cred->cred_handle, &backend->ctxt->ctxt_handle, backend->cred->sni_hostname, backend->req_flags,
			0, 0, NULL, 0, &backend->ctxt->ctxt_handle, &outbuf_desc, &backend->ret_flags, &backend->ctxt->time_stamp);
...
static void schannel_close(struct Curl_cfilter* cf, struct Curl_easy* data) { schannel_shutdown(cf, data); }
...
static CURLcode schannel_pkp_pin_peer_pubkey(struct Curl_cfilter* cf, struct Curl_easy* data,
	...
	do {
		...
		result = Curl_pin_peer_pubkey(data, pinnedpubkey, (const unsigned char*)pubkey->header,
...
static void schannel_checksum(const unsigned char* input, size_t inputlen, unsigned char* checksum, size_t checksumlen,
...
static CURLcode schannel_sha256sum(const unsigned char* input, size_t inputlen, unsigned char* sha256sum,
								   size_t sha256len) {
...
HCERTSTORE Curl_schannel_get_cached_cert_store(struct Curl_cfilter* cf, const struct Curl_easy* data) {
	struct ssl_primary_config* conn_config = Curl_ssl_cf_get_primary_config(cf);
	...
	if (ca_info_blob) {
		...
		if (mbackend->CAinfo_blob_size != ca_info_blob->len)
			return NULL;
		schannel_sha256sum((const unsigned char*)ca_info_blob->data, ca_info_blob->len, info_blob_digest,
						   CURL_SHA256_DIGEST_LENGTH);
		if (memcmp(mbackend->CAinfo_blob_digest, info_blob_digest, CURL_SHA256_DIGEST_LENGTH))
			return NULL;
	} else {
		if (!conn_config->CAfile || !mbackend->CAfile || strcmp(mbackend->CAfile, conn_config->CAfile))
			return NULL;
	}
...
bool Curl_schannel_set_cached_cert_store(struct Curl_cfilter* cf, const struct Curl_easy* data, HCERTSTORE cert_store) {
	...
	if (ca_info_blob) {
		...
		if (!CAinfo_blob_digest)
			return false;
		schannel_sha256sum((const unsigned char*)ca_info_blob->data, ca_info_blob->len, CAinfo_blob_digest,
						   CURL_SHA256_DIGEST_LENGTH);
...
static void schannel_free_multi_ssl_backend_data(struct multi_ssl_backend_data* msbd) {
