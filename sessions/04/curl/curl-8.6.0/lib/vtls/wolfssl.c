...
static const struct group_name_map gnm[] = {{WOLFSSL_KYBER_LEVEL1, "KYBER_LEVEL1"},
...
static void wolfssl_bio_cf_free_methods(void) { wolfSSL_BIO_meth_free(wolfssl_bio_cf_method); }
...
#define use_sni(x) Curl_nop_stmt
	...
	if (ca_info_blob) {
		if (wolfSSL_CTX_load_verify_buffer(backend->ctx, ca_info_blob->data, ca_info_blob->len, SSL_FILETYPE_PEM) !=
			SSL_SUCCESS) {
	...
	if (ssl_cafile || ssl_capath) {
		int rc =
			wolfSSL_CTX_load_verify_locations_ex(backend->ctx, ssl_cafile, ssl_capath, WOLFSSL_LOAD_FLAG_IGNORE_ERR);
		if (SSL_SUCCESS != rc) {
	...
	if (ssl_config->primary.clientcert && ssl_config->key) {
		...
		if (file_type == WOLFSSL_FILETYPE_PEM) {
			if (wolfSSL_CTX_use_certificate_chain_file(backend->ctx, ssl_config->primary.clientcert) != 1) {
	...
	if (sni && connssl->peer.sni) {
		...
		if (sni_len < USHRT_MAX) {
			if (wolfSSL_CTX_UseSNI(backend->ctx, WOLFSSL_SNI_HOST_NAME, connssl->peer.sni, (unsigned short)sni_len) !=
				1) {
	...
	if (connssl->alpn) {
		...
		if (result ||
			wolfSSL_UseALPN(backend->handle, (char*)proto.data, proto.len, WOLFSSL_ALPN_CONTINUE_ON_MISMATCH) !=
				SSL_SUCCESS) {
...
static CURLcode wolfssl_connect_step2(struct Curl_cfilter* cf, struct Curl_easy* data) {
...
#if 1
			failf(data, " subject alt name(s) or common name do not match \"%s\"", connssl->peer.dispname);
	...
	infof(data, "SSL connection using %s / %s", wolfSSL_get_version(backend->handle),
...
static ssize_t wolfssl_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* mem, size_t len,
							CURLcode* curlcode) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct wolfssl_ssl_backend_data* backend = (struct wolfssl_ssl_backend_data*)connssl->backend;
	char error_buffer[WOLFSSL_MAX_ERROR_SZ];
...
static ssize_t wolfssl_recv(struct Curl_cfilter* cf, struct Curl_easy* data, char* buf, size_t blen,
							CURLcode* curlcode) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct wolfssl_ssl_backend_data* backend = (struct wolfssl_ssl_backend_data*)connssl->backend;
	char error_buffer[WOLFSSL_MAX_ERROR_SZ];
