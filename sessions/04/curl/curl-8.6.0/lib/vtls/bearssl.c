...
static CURLcode bearssl_set_selected_ciphers(struct Curl_easy* data, br_ssl_engine_context* ssl_eng,
											 const char* ciphers) {
	...
	while (true) {
		...
		if (strncasecompare(cipher, "TLS_", 4))
			for (i = 0; i < NUM_OF_CIPHERS && (strlen(ciphertable[i].name) == clen) &&
				 !strncasecompare(cipher, ciphertable[i].name, clen);
				 ++i)
				;
		else
			for (i = 0; i < NUM_OF_CIPHERS && (strlen(ciphertable[i].alias_name) == clen) &&
				 !strncasecompare(cipher, ciphertable[i].alias_name, clen);
				 ++i)
				;
		if (i == NUM_OF_CIPHERS) {
			infof(data, "BearSSL: unknown cipher in list: %.*s", (int)clen, cipher);
			continue;
		}
...
static ssize_t bearssl_send(struct Curl_cfilter* cf, struct Curl_easy* data, const void* buf, size_t len,
							CURLcode* err) {
	struct ssl_connect_data* connssl = cf->ctx;
	struct bearssl_ssl_backend_data* backend = (struct bearssl_ssl_backend_data*)connssl->backend;
	unsigned char* app;
...
static size_t bearssl_version(char* buffer, size_t size) { return msnprintf(buffer, size, "BearSSL"); }
...
static CURLcode bearssl_sha256sum(const unsigned char* input, size_t inputlen, unsigned char* sha256sum,
