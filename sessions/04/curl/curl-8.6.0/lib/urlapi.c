...
static CURLcode concat_url(char* base, const char* relurl, char** newurl) {
	...
	*newurl = NULL;
	...
	uc = urlencode_str(&newest, useurl, strlen(useurl), !host_changed, FALSE);
	if (uc)
...
static CURLUcode junkscan(const char* url, size_t* urllen, unsigned int flags) {
	static const char badbytes[] = {
		...
		0x17,		0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0x00 /* null-terminate */
	};
	size_t n = strlen(url);
	size_t nfine;
 ...
 *
 */
static CURLUcode parse_hostname_login(struct Curl_URL* u, const char* login, size_t len, unsigned int flags,
									  size_t* offset) /* to the host name */
{
	...
	*offset = 0;
	...
	ccode = Curl_parse_login_details(login, ptr - login - 1, &userp, &passwdp,
...
static int ipv4_normalize(struct dynbuf* host) {
	...
	case 0: /* a -- 32 bits */
		...
		result =
			Curl_dyn_addf(host, "%u.%u.%u.%u", (unsigned int)(parts[0] >> 24), (unsigned int)((parts[0] >> 16) & 0xff),
						  (unsigned int)((parts[0] >> 8) & 0xff), (unsigned int)(parts[0] & 0xff));
		break;
	case 1: /* a.b -- 8.24 bits */
		if ((parts[0] > 0xff) || (parts[1] > 0xffffff))
			return HOST_NAME;
		Curl_dyn_reset(host);
		result = Curl_dyn_addf(host, "%u.%u.%u.%u", (unsigned int)(parts[0]), (unsigned int)((parts[1] >> 16) & 0xff),
							   (unsigned int)((parts[1] >> 8) & 0xff), (unsigned int)(parts[1] & 0xff));
		break;
	case 2: /* a.b.c -- 8.8.16 bits */
		if ((parts[0] > 0xff) || (parts[1] > 0xff) || (parts[2] > 0xffff))
			return HOST_NAME;
		Curl_dyn_reset(host);
		result = Curl_dyn_addf(host, "%u.%u.%u.%u", (unsigned int)(parts[0]), (unsigned int)(parts[1]),
							   (unsigned int)((parts[2] >> 8) & 0xff), (unsigned int)(parts[2] & 0xff));
		break;
	case 3: /* a.b.c.d -- 8.8.8.8 bits */
		...
		Curl_dyn_reset(host);
		result = Curl_dyn_addf(host, "%u.%u.%u.%u", (unsigned int)(parts[0]), (unsigned int)(parts[1]),
							   (unsigned int)(parts[2]), (unsigned int)(parts[3]));
		break;
	}
	if (result)
		return HOST_ERROR;
...
static CURLUcode parse_authority(struct Curl_URL* u, const char* auth, size_t authlen, unsigned int flags,
	...
	*out = 0; /* null-terminates, for inputs like "./" */
	...
	do {
		...
		if (*input == '.') {
			...
			else if (!strcmp(".", input) || !strcmp("..", input) || !strncmp(".?", input, 2) ||
					 !strncmp("..?", input, 3)) {
				*out = 0;
				break;
			} else {
...
static CURLUcode parseurl(const char* url, CURLU* u, unsigned int flags) {
	...
	schemelen =
		Curl_is_absolute_url(url, schemebuf, sizeof(schemebuf), flags & (CURLU_GUESS_SCHEME | CURLU_DEFAULT_SCHEME));
	...
	if (schemelen && !strcmp(schemebuf, "file")) {
		...
		if (schemelen) {
			...
			schemep = schemebuf;
			if (!Curl_get_scheme_handler(schemep) && !(flags & CURLU_NON_SUPPORT_SCHEME)) {
...
/*
 */
CURLU* curl_url(void) { return calloc(1, sizeof(struct Curl_URL)); }
	...
	*part = NULL;
	...
	case CURLUPART_URL: {
		...
		if (u->scheme && strcasecompare("file", u->scheme)) {
			...
			url = aprintf("%s://%s%s%s%s%s%s%s%s%s%s%s%s%s%s", scheme, u->user ? u->user : "", u->password ? ":" : "",
						  u->password ? u->password : "", options ? ";" : "", options ? options : "",
						  (u->user || u->password || options) ? "@" : "", allochost ? allochost : u->host,
						  port ? ":" : "", port ? port : "", u->path ? u->path : "/",
						  (u->query && u->query[0]) ? "?" : "", (u->query && u->query[0]) ? u->query : "",
...
CURLUcode curl_url_set(CURLU* u, CURLUPart what, const char* part, unsigned int flags) {
	...
	case CURLUPART_URL: {
		...
		if (Curl_is_absolute_url(part, NULL, 0, flags & (CURLU_GUESS_SCHEME | CURLU_DEFAULT_SCHEME)) ||
	...
	DEBUGASSERT(storep);
	{
		...
		if (appendquery && newp) {
			...
			} else if (!n || hostname_check(u, (char*)newp, n)) {
				Curl_dyn_free(&enc);
				return CURLUE_BAD_HOSTNAME;
			}
		}
