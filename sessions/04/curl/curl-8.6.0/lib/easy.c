	...
	while (m) {
		if (m->socket.fd == s) {
			...
			if (what == CURL_POLL_REMOVE) {
				...
				free(m);
				m = nxt;
				infof(easy, "socket cb: socket %" CURL_FORMAT_SOCKET_T " REMOVED", s);
			} else {
				/* The socket 's' is already being monitored, update the activity
				   mask. Convert from libcurl bitmask to the poll one. */
				m->socket.events = socketcb2poll(what);
				infof(easy, "socket cb: socket %" CURL_FORMAT_SOCKET_T " UPDATED as %s%s", s,
					  (what & CURL_POLL_IN) ? "IN" : "", (what & CURL_POLL_OUT) ? "OUT" : "");
			}
			break;
		}
		prev = m;
	...
	if (!m) {
		if (what == CURL_POLL_REMOVE) {
			...
			if (m) {
				...
				m->socket.revents = 0;
				ev->list = m;
				infof(easy, "socket cb: socket %" CURL_FORMAT_SOCKET_T " ADDED as %s%s", s,
					  (what & CURL_POLL_IN) ? "IN" : "", (what & CURL_POLL_OUT) ? "OUT" : "");
			} else {
 ...
 * transfer as previously setup.
 */
CURLcode curl_easy_perform(struct Curl_easy* data) { return easy_perform(data, FALSE); }
 ...
 */
CURLcode curl_easy_perform_ev(struct Curl_easy* data) { return easy_perform(data, TRUE); }
...
struct Curl_easy* curl_easy_duphandle(struct Curl_easy* data) {
	...
	if (data->cookies && data->state.cookie_engine) {
		...
		outcurl->cookies = Curl_cookie_init(outcurl, NULL, outcurl->cookies, data->set.cookiesession);
		if (!outcurl->cookies)
			goto fail;
	}
	...
	if (Curl_resolver_duphandle(outcurl, &outcurl->state.async.resolver, data->state.async.resolver))
		goto fail;
#endif
