...
static void mystate(struct Curl_easy* data, sshstate nowstate
	...
	if (sshc->state != nowstate) {
		infof(data, "SSH %p state change from %s to %s (line %d)", (void*)sshc, names[sshc->state], names[nowstate],
			  lineno);
	}
#endif
...
static int myssh_is_known(struct Curl_easy* data) {
	...
	} else if (keymatch != CURLKHMATCH_OK) {
		rc = SSH_ERROR;
		goto cleanup;
	}
	rc = SSH_OK;
      ...
      ssh_set_log_level(SSH_LOG_PROTOCOL);
		...
		case SSH_AUTH_PKEY_INIT:
			...
			if (data->set.str[STRING_SSH_PRIVATE_KEY]) {
				...
				rc = ssh_pki_import_privkey_file(data->set.str[STRING_SSH_PRIVATE_KEY], data->set.ssl.key_passwd, NULL,
												 NULL, &sshc->privkey);
				if (rc != SSH_OK) {
...
#define CURL_LIBSSH_VFS_SIZE_MASK PRIu64
		...
		case SSH_SFTP_TRANS_INIT:
			...
			else if (protop->path[strlen(protop->path) - 1] == '/')
			...
			if (sshc->readdir_attrs) {
				...
				if (data->set.list_only) {
					...
					if ((sshc->readdir_attrs->flags & SSH_FILEXFER_ATTR_PERMISSIONS) &&
						((sshc->readdir_attrs->permissions & SSH_S_IFMT) == SSH_S_IFLNK)) {
		...
		case SSH_SFTP_READDIR_BOTTOM:
			if (Curl_dyn_addn(&sshc->readdir_buf, "\n", 1)) {
				...
				result = Curl_client_write(data, CLIENTWRITE_BODY, Curl_dyn_ptr(&sshc->readdir_buf),
		...
		case SSH_SFTP_DOWNLOAD_STAT: {
			...
			if (data->state.resume_from) {
				if (data->state.resume_from < 0) {
					...
					/* download from where? */
					data->state.resume_from += size;
				} else if ((curl_off_t)size < data->state.resume_from) {
					failf(data,
						  "Offset (%" CURL_FORMAT_CURL_OFF_T ") was beyond file size (%" CURL_FORMAT_CURL_OFF_T ")",
						  data->state.resume_from, size);
					return CURLE_BAD_DOWNLOAD_RESUME;
		...
		case SSH_SCP_UPLOAD_INIT:
			...
			rc = ssh_scp_push_file(sshc->scp_session, protop->path, data->state.infilesize,
...
void Curl_ssh_version(char* buffer, size_t buflen) { (void)msnprintf(buffer, buflen, "libssh/%s", ssh_version(0)); }
