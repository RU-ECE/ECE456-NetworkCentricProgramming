	...
	if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
		...
		if (remotekey) {
			...
			if (!keybit) {
				...
				keycheck = libssh2_knownhost_checkp(
					sshc->kh, conn->host.name, (conn->remote_port != PORT_SSH) ? conn->remote_port : -1, remotekey,
					keylen, LIBSSH2_KNOWNHOST_TYPE_PLAIN | LIBSSH2_KNOWNHOST_KEYENC_RAW | keybit, &host);
#else
				keycheck = libssh2_knownhost_check(sshc->kh, conn->host.name, remotekey, keylen,
		...
		case CURLKHSTAT_FINE_ADD_TO_FILE:
			...
			if (keycheck != LIBSSH2_KNOWNHOST_CHECK_MATCH) {
				...
				int addrc =
					libssh2_knownhost_add(sshc->kh, conn->host.name, NULL, remotekey, keylen,
				...
				if (addrc) {
					...
					/* now we write the entire in-memory list of known hosts to the
					   known_hosts file */
					int wrc = libssh2_knownhost_writefile(sshc->kh, data->set.str[STRING_SSH_KNOWNHOSTS],
	...
	*block = 0; /* we're not blocking by default */
	...
	do {
		...
		case SSH_AUTH_PKEY:
			 ...
			 */
			rc = libssh2_userauth_publickey_fromfile_ex(sshc->ssh_session, conn->user, curlx_uztoui(strlen(conn->user)),
		...
		case SSH_AUTH_PASS:
			rc = libssh2_userauth_password_ex(sshc->ssh_session, conn->user, curlx_uztoui(strlen(conn->user)),
		...
		case SSH_AUTH_KEY:
			...
			rc = libssh2_userauth_keyboard_interactive_ex(sshc->ssh_session, conn->user,
														  curlx_uztoui(strlen(conn->user)), &kbd_callback);
			if (rc == LIBSSH2_ERROR_EAGAIN)
				break;
			if (rc == 0) {
				sshc->authed = TRUE;
		...
		case SSH_SFTP_NEXT_QUOTE:
			...
			if (sshc->quote_item) {
				state(data, SSH_SFTP_QUOTE);
			} else if (sshc->nextstate != SSH_NO_STATE) {
		...
		case SSH_SFTP_QUOTE_STAT: {
			...
			if (!strncasecompare(cmd, "chmod", 5)) {
				 ...
				 */
				rc =
					libssh2_sftp_stat_ex(sshc->sftp_session, sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)),
		...
		case SSH_SFTP_QUOTE_SETSTAT:
			rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)),
		...
		case SSH_SFTP_QUOTE_SYMLINK:
			rc = libssh2_sftp_symlink_ex(sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)),
										 sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)),
		...
		case SSH_SFTP_QUOTE_MKDIR:
			rc = libssh2_sftp_mkdir_ex(sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)),
		...
		case SSH_SFTP_QUOTE_RENAME:
			rc = libssh2_sftp_rename_ex(sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)),
										sshc->quote_path2, curlx_uztoui(strlen(sshc->quote_path2)),
		...
		case SSH_SFTP_QUOTE_STATVFS: {
			...
			rc = libssh2_sftp_statvfs(sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)),
									  &statvfs);
...
#define CURL_LIBSSH2_VFS_SIZE_MASK "llu"
				...
				char* tmp = aprintf("statvfs:\n"
									...
									statvfs.f_bsize, statvfs.f_frsize, statvfs.f_blocks, statvfs.f_bfree,
									statvfs.f_bavail, statvfs.f_files, statvfs.f_ffree, statvfs.f_favail,
		...
		case SSH_SFTP_FILETIME: {
			...
			rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshp->path, curlx_uztoui(strlen(sshp->path)),
		...
		case SSH_SFTP_TRANS_INIT:
			...
			else if (sshp->path[strlen(sshp->path) - 1] == '/')
		...
		case SSH_SFTP_UPLOAD_INIT: {
			...
			sshc->sftp_handle = libssh2_sftp_open_ex(sshc->sftp_session, sshp->path, curlx_uztoui(strlen(sshp->path)),
			...
			if (!sshc->sftp_handle) {
				...
				if (((sftperr == LIBSSH2_FX_NO_SUCH_FILE) || (sftperr == LIBSSH2_FX_FAILURE) ||
				...
				failf(data, "Upload failed: %s (%lu/%d)",
					  sftperr != LIBSSH2_FX_OK ? sftp_libssh2_strerror(sftperr) : "ssh error", sftperr, rc);
				break;
			}
			...
			if (data->state.resume_from > 0) {
				...
				if (seekerr != CURL_SEEKFUNC_OK) {
					...
					do {
						...
						size_t actuallyread;
						Curl_set_in_callback(data, true);
						actuallyread = data->state.fread_func(scratch, 1, readthisamountnow, data->state.in);
						Curl_set_in_callback(data, false);
		...
		case SSH_SFTP_CREATE_DIRS_MKDIR:
			...
			rc = libssh2_sftp_mkdir_ex(sshc->sftp_session, sshp->path, curlx_uztoui(strlen(sshp->path)),
									   data->set.new_directory_perms);
			if (rc == LIBSSH2_ERROR_EAGAIN)
				break;
			*sshc->slash_pos = '/';
			...
			if (rc < 0) {
				 ...
				 */
				sftperr = libssh2_sftp_last_error(sshc->sftp_session);
				if ((sftperr != LIBSSH2_FX_FILE_ALREADY_EXISTS) && (sftperr != LIBSSH2_FX_FAILURE) &&
		...
		case SSH_SFTP_READDIR:
			rc = libssh2_sftp_readdir_ex(sshc->sftp_handle, sshp->readdir_filename, PATH_MAX, sshp->readdir_longentry,
										 PATH_MAX, &sshp->readdir_attrs);
			if (rc == LIBSSH2_ERROR_EAGAIN)
				break;
			if (rc > 0) {
		...
		case SSH_SFTP_READDIR_LINK:
			rc = libssh2_sftp_symlink_ex(sshc->sftp_session, Curl_dyn_ptr(&sshp->readdir_link),
										 (int)Curl_dyn_len(&sshp->readdir_link), sshp->readdir_filename, PATH_MAX,
		...
		case SSH_SFTP_READDIR_BOTTOM:
			...
			if (!result) {
				result = Curl_client_write(data, CLIENTWRITE_BODY, Curl_dyn_ptr(&sshp->readdir),
		...
		case SSH_SFTP_DOWNLOAD_STAT: {
			...
			if (rc || !(attrs.flags & LIBSSH2_SFTP_ATTR_SIZE) || (attrs.filesize == 0)) {
				...
				if (data->state.use_range) {
					...
					if (from > size) {
						failf(data,
							  "Offset (%" CURL_FORMAT_CURL_OFF_T ") was beyond file size (%" CURL_FORMAT_CURL_OFF_T ")",
			...
			if (data->state.resume_from) {
				if (data->state.resume_from < 0) {
					...
					/* download from where? */
					data->state.resume_from += attrs.filesize;
				} else if ((curl_off_t)attrs.filesize < data->state.resume_from) {
					failf(data,
						  "Offset (%" CURL_FORMAT_CURL_OFF_T ") was beyond file size (%" CURL_FORMAT_CURL_OFF_T ")",
						  data->state.resume_from, (curl_off_t)attrs.filesize);
					return CURLE_BAD_DOWNLOAD_RESUME;
		...
		case SSH_SCP_UPLOAD_INIT:
			 ...
			 */
			sshc->ssh_channel =
				SCP_SEND(sshc->ssh_session, sshp->path, data->set.new_file_perms, data->state.infilesize);
			if (!sshc->ssh_channel) {
				int ssh_err;
				char* err_msg = NULL;
...
static CURLcode ssh_connect(struct Curl_easy* data, bool* done) {
	...
	if (conn->http_proxy.proxytype == CURLPROXY_HTTPS) {
		...
		*/
		...
		libssh2_session_callback_set2(sshc->ssh_session, LIBSSH2_CALLBACK_RECV, (libssh2_cb_generic*)ssh_tls_recv);
		libssh2_session_callback_set2(sshc->ssh_session, LIBSSH2_CALLBACK_SEND, (libssh2_cb_generic*)ssh_tls_send);
#else
		/*
