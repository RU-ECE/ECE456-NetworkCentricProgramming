 ...
 */
char* Curl_checkProxyheaders(struct Curl_easy* data, const struct connectdata* conn, const char* thisheader,
							 const size_t thislen) {
	struct curl_slist* head;

	for (head = (conn->bits.proxy && data->set.sep_headers) ? data->set.proxyheaders : data->set.headers; head;
		 head = head->next) {
 ...
 */
static CURLcode output_auth_headers(struct Curl_easy* data, struct connectdata* conn, struct auth* authstatus,
									const char* request, const char* path, bool proxy) {
 ...
 * @returns CURLcode
 */
CURLcode Curl_http_output_auth(struct Curl_easy* data, struct connectdata* conn, const char* request,
													...
													up the proxy tunnel */
{
	...
	if (((authhost->multipass && !authhost->done) || (authproxy->multipass && !authproxy->done)) &&
...
/* when disabled */
CURLcode Curl_http_output_auth(struct Curl_easy* data, struct connectdata* conn, const char* request,
							   Curl_HttpReq httpreq, const char* path, bool proxytunnel) {
	(void)data;
	(void)conn;
	(void)request;
	(void)httpreq;
...
#if defined(USE_SPNEGO) || defined(USE_NTLM) || !defined(CURL_DISABLE_DIGEST_AUTH) ||                                  \
	!defined(CURL_DISABLE_BASIC_AUTH) || !defined(CURL_DISABLE_BEARER_AUTH)
static int is_valid_auth_separator(char ch) { return ch == '\0' || ch == ',' || ISSPACE(ch); }
#endif
...
static size_t readmoredata(char* buffer, size_t size, size_t nitems, void* userp) {
	...
	if (data->set.max_send_speed && (data->set.max_send_speed < (curl_off_t)fullsize) &&
...
int Curl_http_getsock_do(struct Curl_easy* data, struct connectdata* conn, curl_socket_t* socks) {
	/* write mode */
	(void)conn;
...
#define Curl_buffer_send(a, b, c, d, e, f) CURLE_OK
	...
	case HTTPREQ_PUT: /* Let's PUT the data to the server! */
		...
		if ((http->postsize != -1) && !data->req.upload_chunky &&
			(conn->bits.authneg || !Curl_checkheaders(data, STRCONST("Content-Length")))) {
	...
	if (data->cookies || addcookies) {
		...
		if (data->cookies && data->state.cookie_engine) {
			...
			const bool secure_context = conn->handler->protocol & (CURLPROTO_HTTPS | CURLPROTO_WSS) ||
					strcasecompare("localhost", host) || !strcmp(host, "127.0.0.1") || !strcmp(host, "::1")
				? TRUE
				: FALSE;
	...
	if ((HTTPREQ_POST == httpreq || HTTPREQ_PUT == httpreq) && data->state.resume_from) {
		...
		if (data->state.resume_from && !data->state.followlocation) {
			...
			if (seekerr != CURL_SEEKFUNC_OK) {
				...
				do {
					char scratch[4 * 1024];
					size_t readthisamountnow = (data->state.resume_from - passed > (curl_off_t)sizeof(scratch))
						? sizeof(scratch)
						: curlx_sotouz(data->state.resume_from - passed);

					size_t actuallyread = data->state.fread_func(scratch, 1, readthisamountnow, data->state.in);
...
CURLcode Curl_http(struct Curl_easy* data, bool* done) {
	...
	*done = TRUE;
	...
	result = Curl_dyn_addf(
		...
		(data->set.str[STRING_USERAGENT] && *data->set.str[STRING_USERAGENT] && data->state.aptr.uagent)
			? data->state.aptr.uagent
			: "",
		p_accept ? p_accept : "", data->state.aptr.te ? data->state.aptr.te : "",
		(data->set.str[STRING_ENCODING] && *data->set.str[STRING_ENCODING] && data->state.aptr.accept_encoding)
	...
	if (data->req.writebytecount) {
		...
		if (!http->postsize) {
			/* already sent the entire request body, mark the "upload" as
			   complete */
			infof(data,
				  "upload completely sent off: %" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T " bytes",
...
CURLcode Curl_http_header(struct Curl_easy* data, struct connectdata* conn, char* headp) {
	...
	/* check for Content-Type: header lines to get the MIME-type */
		...
		if (!*contenttype) {
			...
			data->info.contenttype = contenttype;
			 ...
			 Curl_compareheader(headp, STRCONST("Proxy-Connection:"), STRCONST("keep-alive"))) {
		/*
		...
		infof(data, "HTTP/1.0 proxy connection set to keep alive");
			   ...
			   Curl_compareheader(headp, STRCONST("Proxy-Connection:"), STRCONST("close"))) {
		/*
		...
		*/
		...
		Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE);
		Curl_cookie_add(data, data->cookies, TRUE, FALSE, headp + strlen("Set-Cookie:"), host, data->state.up.path,
						secure_context);
		Curl_share_unlock(data, CURL_LOCK_DATA_COOKIE);
	}
#endif
	...
	} else if ((checkprefix("WWW-Authenticate:", headp) && (401 == k->httpcode)) ||
			   (checkprefix("Proxy-authenticate:", headp) && (407 == k->httpcode))) {
	...
	/* If enabled, the header is incoming and this is over HTTPS */
		...
		result = Curl_altsvc_parse(data, data->asi, headp + strlen("Alt-Svc:"), id, conn->host.name,
 ...
 */
static CURLcode http_rw_headers(struct Curl_easy* data, const char* buf, size_t blen, size_t* pconsumed) {
	struct connectdata* conn = data->conn;
	CURLcode result = CURLE_OK;
	struct SingleRequest* k = &data->req;
	char* headp;
	...
	do {
		...
		if (!end_ptr) {
			...
			*pconsumed += blen;
			...
			if (!k->headerline) {
				...
				statusline st = checkprotoprefix(data, conn, Curl_dyn_ptr(&data->state.headerb),
		...
		*pconsumed += line_length;
		...
		if (!k->headerline) {
			...
			statusline st =
				checkprotoprefix(data, conn, Curl_dyn_ptr(&data->state.headerb), Curl_dyn_len(&data->state.headerb));
			if (st == STATUS_BAD) {
				streamclose(conn, "bad HTTP: No end-of-message indicator");
				/* this is not the beginning of a protocol first header line */
				if (!data->set.http09_allowed) {
		...
		if ((0x0a == *headp) || (0x0d == *headp)) {
			...
			if (100 <= k->httpcode && 199 >= k->httpcode) {
				...
				if ((k->size == -1) && !k->chunk && !conn->bits.close && (conn->httpversion == 11) &&
					!(conn->handler->protocol & CURLPROTO_RTSP) && data->state.httpreq != HTTPREQ_HEAD) {
			...
			if (!k->header) {
				...
				/* If max download size is *zero* (nothing) we already have
				   ...
				   close the stream. */
				if (0 == k->maxdownload && !Curl_conn_is_http2(data, conn, FIRSTSOCKET) &&
		...
		if (!k->headerline++) {
			...
			if (conn->handler->protocol & PROTO_FAMILY_HTTP) {
				...
				if (fine_statusline) {
					...
					*/
					statusline check =
						checkhttpprefix(data, Curl_dyn_ptr(&data->state.headerb), Curl_dyn_len(&data->state.headerb));
 ...
 */
CURLcode Curl_http_write_resp_hds(struct Curl_easy* data, const char* buf, size_t blen, size_t* pconsumed, bool* done) {
	*done = FALSE;
	if (!data->req.header) {
		*pconsumed = 0;
		return CURLE_OK;
		...
		if (!result && !data->req.header) {
			...
			if (!data->req.no_body && Curl_dyn_len(&data->state.headerb)) {
				...
				result = Curl_client_write(data, CLIENTWRITE_BODY, Curl_dyn_ptr(&data->state.headerb),
										   Curl_dyn_len(&data->state.headerb));
			}
			Curl_dyn_free(&data->state.headerb);
		}
...
CURLcode Curl_http_write_resp(struct Curl_easy* data, const char* buf, size_t blen, bool is_eos, bool* done) {
	CURLcode result;
	size_t consumed;
	int flags;
...
CURLcode Curl_http_req_make2(struct httpreq** preq, const char* method, size_t m_len, CURLU* url,
...
static struct name_const H2_NON_FIELD[] = {
	{STRCONST("Host")},		  {STRCONST("Upgrade")},		  {STRCONST("Connection")},
	{STRCONST("Keep-Alive")}, {STRCONST("Proxy-Connection")}, {STRCONST("Transfer-Encoding")},
};
