		...
		unsigned char htype =
			(unsigned char)(type & CLIENTWRITE_CONNECT
								...
								: (type & CLIENTWRITE_1XX
									   ? CURLH_1XX
									   : (type & CLIENTWRITE_TRAILER ? CURLH_TRAILER : CURLH_HEADER)));
...
CURLcode Curl_client_write(struct Curl_easy* data, int type, char* buf, size_t blen) {
	...
	if ((type & CLIENTWRITE_BODY) && (data->conn->handler->protocol & PROTO_FAMILY_FTP) &&
	...
	DEBUGASSERT(!(type & CLIENTWRITE_BODY) || ((type & ~(CLIENTWRITE_BODY | CLIENTWRITE_EOS)) == 0));
	/* INFO is only INFO (with optional EOS) */
	DEBUGASSERT(!(type & CLIENTWRITE_INFO) || ((type & ~(CLIENTWRITE_INFO | CLIENTWRITE_EOS)) == 0));
	...
	if (data->state.tempcount) {
		...
		for (i = 0; i < count; i++) {
			/* even if one function returns error, this loops through and frees
			   all buffers */
			if (!result) {
				result = chop_write(data, writebuf[i].type, !writebuf[i].paused_body, Curl_dyn_ptr(&writebuf[i].b),
...
/* Write data using an unencoding writer stack. "nbytes" is not
   allowed to be 0. */
CURLcode Curl_cwriter_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
							size_t nbytes) {
	if (!writer)
		return CURLE_WRITE_ERROR;
...
CURLcode Curl_cwriter_def_init(struct Curl_easy* data, struct Curl_cwriter* writer) {
	(void)data;
	(void)writer;
	return CURLE_OK;
}

CURLcode Curl_cwriter_def_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
								size_t nbytes) {
	return Curl_cwriter_write(data, writer->next, type, buf, nbytes);
}

void Curl_cwriter_def_close(struct Curl_easy* data, struct Curl_cwriter* writer) {
	(void)data;
	(void)writer;
}

/* Real client writer to installed callbacks. */
static CURLcode cw_client_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
								size_t nbytes) {
	(void)writer;
	if (!nbytes)
		return CURLE_OK;
...
static const struct Curl_cwtype cw_client = {
	"client", NULL, Curl_cwriter_def_init, cw_client_write, Curl_cwriter_def_close, sizeof(struct Curl_cwriter)};
...
static CURLcode cw_download_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
								  size_t nbytes) {
	CURLcode result;
	...
	if (data->req.no_body && nbytes > 0) {
		...
		DEBUGF(infof(data, "did not want a BODY, but seeing %zu bytes", nbytes));
	...
	if (excess_len) {
		if (!data->req.ignorebody) {
			infof(data,
				  ...
				  " excess = %zu"
				  ", size = %" CURL_FORMAT_CURL_OFF_T ", maxdownload = %" CURL_FORMAT_CURL_OFF_T
				  ", bytecount = %" CURL_FORMAT_CURL_OFF_T,
				  excess_len, data->req.size, data->req.maxdownload, data->req.bytecount);
			connclose(data->conn, "excess found in a read");
		}
	} else if (nwrite < nbytes) {
		failf(data,
			  "Exceeded the maximum allowed file size "
			  "(%" CURL_FORMAT_CURL_OFF_T ") with %" CURL_FORMAT_CURL_OFF_T " bytes",
...
static const struct Curl_cwtype cw_download = {
	"download", NULL, Curl_cwriter_def_init, cw_download_write, Curl_cwriter_def_close, sizeof(struct Curl_cwriter)};

/* RAW client writer in phase CURL_CW_RAW that
 * enabled tracing of raw data. */
static CURLcode cw_raw_write(struct Curl_easy* data, struct Curl_cwriter* writer, int type, const char* buf,
							 size_t nbytes) {
...
static const struct Curl_cwtype cw_raw = {
	"raw", NULL, Curl_cwriter_def_init, cw_raw_write, Curl_cwriter_def_close, sizeof(struct Curl_cwriter)};

/* Create an unencoding writer stage using the given handler. */
CURLcode Curl_cwriter_create(struct Curl_cwriter** pwriter, struct Curl_easy* data, const struct Curl_cwtype* cwt,
...
static CURLcode do_init_stack(struct Curl_easy* data) {
	...
	DEBUGASSERT(!data->req.writer_stack);
	result = Curl_cwriter_create(&data->req.writer_stack, data, &cw_client, CURL_CW_CLIENT);
	if (result)
		return result;
