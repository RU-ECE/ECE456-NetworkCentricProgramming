...
static CURLcode imap_sendf(struct Curl_easy* data, const char* fmt, ...) CURL_PRINTF(2, 3);
		...
		case IMAP_LIST:
			if ((!imap->custom && !imap_matchresp(line, len, "LIST")) ||
				(imap->custom && !imap_matchresp(line, len, imap->custom) &&
				 (!strcasecompare(imap->custom, "STORE") || !imap_matchresp(line, len, "FETCH")) &&
				 !strcasecompare(imap->custom, "SELECT") && !strcasecompare(imap->custom, "EXAMINE") &&
				 !strcasecompare(imap->custom, "SEARCH") && !strcasecompare(imap->custom, "EXPUNGE") &&
				 !strcasecompare(imap->custom, "LSUB") && !strcasecompare(imap->custom, "UID") &&
...
static void imap_state(struct Curl_easy* data, imapstate newstate) {
	...
	static const char* const names[] = {
		"STOP",	  "SERVERGREET", "CAPABILITY",	"STARTTLS", "UPGRADETLS",	"AUTHENTICATE", "LOGIN",  "LIST",
		"SELECT", "FETCH",		 "FETCH_FINAL", "APPEND",	"APPEND_FINAL", "SEARCH",		"LOGOUT",
		/* LAST */
	};
...
static CURLcode imap_perform_fetch(struct Curl_easy* data) {
	...
	if (imap->uid) {
		...
		if (imap->partial) {
			result = imap_sendf(data, "UID FETCH %s BODY[%s]<%s>", imap->uid, imap->section ? imap->section : "",
								imap->partial);
		} else {
		...
		/* Send the FETCH command */
		if (imap->partial) {
			result = imap_sendf(data, "FETCH %s BODY[%s]<%s>", imap->mindex, imap->section ? imap->section : "",
								imap->partial);
		} else {
...
static CURLcode imap_state_auth_resp(struct Curl_easy* data, struct connectdata* conn, int imapcode,
									 imapstate instate) {
	CURLcode result = CURLE_OK;
	struct imap_conn* imapc = &conn->proto.imapc;
	saslprogress progress;
...
static CURLcode imap_state_fetch_resp(struct Curl_easy* data, struct connectdata* conn, int imapcode,
									  imapstate instate) {
	CURLcode result = CURLE_OK;
	struct imap_conn* imapc = &conn->proto.imapc;
	...
	if (ptr) {
		char* endptr;
		if (!curlx_strtoofft(ptr + 1, &endptr, 10, &size) && (endptr - ptr > 1 && *endptr == '}'))
			parsed = TRUE;
	}
	...
	if (parsed) {
		...
		if (pp->overflow) {
			...
			result = Curl_client_write(data, CLIENTWRITE_BODY, Curl_dyn_ptr(&pp->recvbuf), chunk);
			if (result)
				return result;

			infof(data, "Written %zu bytes, %" CURL_FORMAT_CURL_OFF_TU " bytes are left for transfer", chunk,
				  size - chunk);
...
static CURLcode imap_done(struct Curl_easy* data, CURLcode status, bool premature) {
	...
	if (status) {
		...
		result = status; /* use the already set error code */
			   ...
			   (imap->uid || imap->mindex || data->state.upload || data->set.mimepost.kind != MIMEKIND_NONE)) {
...
static CURLcode imap_sendf(struct Curl_easy* data, const char* fmt, ...) {
	...
	msnprintf(imapc->resptag, sizeof(imapc->resptag), "%c%03d",
			  'A' + curlx_sltosi((long)(data->conn->connection_id % 26)), ++imapc->cmdid);
