...
static const char* const multi_statename[] = {
	"INIT",			"PENDING",		   "CONNECT", "RESOLVING", "CONNECTING", "TUNNELING",
...
static void mstate(struct Curl_easy* data, CURLMstate state
	...
	if (data->mstate >= MSTATE_PENDING && data->mstate < MSTATE_COMPLETED) {
		infof(data, "STATE: %s => %s handle %p; line %d", multi_statename[oldstate], multi_statename[data->mstate],
			  (void*)data, lineno);
	}
#endif
...
#if defined(DEBUGBUILD) && !defined(CURL_DISABLE_VERBOSE_STRINGS)
	DEBUGF(infof(data, "multi_done[%s]: status: %d prem: %d done: %d", multi_statename[data->mstate], (int)status,
	...
	*/
	...
	if ((data->set.reuse_forbid
		...
		DEBUGF(infof(data,
					 ...
					 conn->connection_id, data->set.reuse_forbid, conn->bits.close, premature,
		...
		msnprintf(buffer, sizeof(buffer), "Connection #%" CURL_FORMAT_CURL_OFF_T " to host %s left intact",
				  connection_id, host);
...
bool Curl_multiplex_wanted(const struct Curl_multi* multi) { return multi && (multi->multiplexing); }
...
CURLMcode curl_multi_fdset(struct Curl_multi* multi, fd_set* read_fd_set, fd_set* write_fd_set, fd_set* exc_fd_set,
						   int* max_fd) {
...
static CURLMcode multi_wait(struct Curl_multi* multi, struct curl_waitfd extra_fds[], unsigned int extra_nfds,
							int timeout_ms, int* ret, bool extrawait, /* when no socket, wait */
							bool use_wakeup) {
	struct Curl_easy* data;
	struct easy_pollset ps;
	size_t i;
...
CURLMcode curl_multi_wait(struct Curl_multi* multi, struct curl_waitfd extra_fds[], unsigned int extra_nfds,
						  int timeout_ms, int* ret) {
	return multi_wait(multi, extra_fds, extra_nfds, timeout_ms, ret, FALSE, FALSE);
}

CURLMcode curl_multi_poll(struct Curl_multi* multi, struct curl_waitfd extra_fds[], unsigned int extra_nfds,
						  int timeout_ms, int* ret) {
 ...
 * retry.
 */
void Curl_multi_connchanged(struct Curl_multi* multi) { multi->recheckstate = TRUE; }
 ...
 */
static bool multi_handle_timeout(struct Curl_easy* data, struct curltime* now, bool* stream_error, CURLcode* result,
	...
	if (timeout_ms < 0) {
		...
		if (data->mstate == MSTATE_RESOLVING) {
			failf(data, "Resolving timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds",
				  Curl_timediff(*now, data->progress.t_startsingle));
		} else if (data->mstate == MSTATE_CONNECTING) {
			failf(data, "Connection timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds",
				  Curl_timediff(*now, data->progress.t_startsingle));
		} else {
			struct SingleRequest* k = &data->req;
			if (k->size != -1) {
				failf(data,
					  "Operation timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds with %" CURL_FORMAT_CURL_OFF_T
...
static void set_in_callback(struct Curl_multi* multi, bool value) { multi->in_callback = value; }
	...
	do {
		...
		case MSTATE_DO:
			if (data->set.fprereq) {
				...
				Curl_set_in_callback(data, true);
				prereq_rc =
					data->set.fprereq(data->set.prereq_userp, data->info.conn_primary_ip, data->info.conn_local_ip,
									  data->info.conn_primary_port, data->info.conn_local_port);
				Curl_set_in_callback(data, false);
		...
		case MSTATE_RATELIMITING: /* limit-rate exceeded in either direction */
			...
			if (result) {
				...
				if (data->set.max_send_speed) {
					send_timeout_ms =
						Curl_pgrsLimitWaitTime(data->progress.uploaded, data->progress.ul_limit_size,
				...
				if (data->set.max_recv_speed) {
					recv_timeout_ms =
						Curl_pgrsLimitWaitTime(data->progress.downloaded, data->progress.dl_limit_size,
		...
		if (data->conn && data->mstate >= MSTATE_CONNECT && data->mstate < MSTATE_DO &&
			rc != CURLM_CALL_MULTI_PERFORM && !multi_ischanged(multi, false)) {
...
static CURLMcode multi_socket(struct Curl_multi* multi, bool checkall, curl_socket_t s, int ev_bitmask,
							  int* running_handles) {
...
size_t Curl_multi_max_total_connections(struct Curl_multi* multi) { return multi ? multi->max_total_connections : 0; }
