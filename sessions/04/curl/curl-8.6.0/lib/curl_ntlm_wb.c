...
static CURLcode ntlm_wb_init(struct Curl_easy* data, struct ntlmdata* ntlm, const char* userp) {
	...
	if (child_pid == -1) {
		...
		if (domain) {
			execl(ntlm_auth, ntlm_auth, "--helper-protocol", "ntlmssp-client-1", "--use-cached-creds", "--username",
				  username, "--domain", domain, NULL);
		} else {
			execl(ntlm_auth, ntlm_auth, "--helper-protocol", "ntlmssp-client-1", "--use-cached-creds", "--username",
				  username, NULL);
		}
...
static CURLcode ntlm_wb_response(struct Curl_easy* data, struct ntlmdata* ntlm, const char* input, curlntlm state) {
	...
	struct dynbuf b;
	char* ptr = NULL;
	usigned char buf[1024] Curl_dyn_init(&b, MAX_NTLM_WB_RESPONSE);
	...
	/* Read one line */
	while (1) {
		ssize_t size = wakeup_read(ntlm->ntlm_auth_hlpr_socket, buf, sizeof(buf));
		if (size == -1) {
			if (errno == EINTR)
				continue;
			goto done;
	...
	if (state == NTLMSTATE_TYPE1 && (ptr[0] != 'Y' || ptr[1] != 'R' || ptr[2] != ' '))
		goto done;
	if (state == NTLMSTATE_TYPE2 && (ptr[0] != 'K' || ptr[1] != 'K' || ptr[2] != ' ') &&
