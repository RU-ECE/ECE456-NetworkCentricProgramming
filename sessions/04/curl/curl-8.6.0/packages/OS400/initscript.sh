...
if [ -f "${SCRIPTDIR}/config400.override" ]; then
	. "${SCRIPTDIR}/config400.override"
fi
...
make_module() {
	...
	if [ "${WITH_ZLIB}" != "0" ]; then
		CMD="${CMD} '${ZLIB_INCLUDE}'"
	fi

	if [ "${WITH_LIBSSH2}" != "0" ]; then
		CMD="${CMD} '${LIBSSH2_INCLUDE}'"
	fi
	...
	if [ "${WITH_ZLIB}" != "0" ]; then
		DEFINES="${DEFINES} HAVE_LIBZ"
	fi

	if [ "${WITH_LIBSSH2}" != "0" ]; then
		DEFINES="${DEFINES} USE_LIBSSH2"
	fi

	if [ "${DEFINES}" ]; then
		CMD="${CMD} DEFINE(${DEFINES})"
	fi
