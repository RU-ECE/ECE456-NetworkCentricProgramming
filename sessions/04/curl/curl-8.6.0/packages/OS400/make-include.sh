...
if action_needed "${SRCPF}"; then
	CMD="CRTSRCPF FILE(${TARGETLIB}/H) RCDLEN(112)"
...
if action_needed "${IFSINCLUDE}"; then
	mkdir -p "${IFSINCLUDE}"
fi
...
for HFILE in curl/*.h ${SCRIPTDIR}/ccsidcurl.h; do
	case "$(basename \"${HFILE}\" .h)" in
	...
	if action_needed "${DEST}" "${HFILE}"; then
		copy_hfile "${DEST}" "${HFILE}"
...
if action_needed "${LIBIFSNAME}/CURL.FILE"; then
	:
else
	CLcommand "DLTF FILE(${TARGETLIB}/CURL)"
fi
