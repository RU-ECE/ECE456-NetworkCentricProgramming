...
static char* keyed_string(localkey_t key, const char* ascii, unsigned int ccsid) {
	int i;
	char* ebcdic;
...
char* curl_version_ccsid(unsigned int ccsid) { return keyed_string(LK_CURL_VERSION, curl_version(), ccsid); }
...
char* curl_easy_unescape_ccsid(CURL* handle, const char* string, int length, int* outlength, unsigned int sccsid,
							   unsigned int dccsid) {
	char* s;
	char* d;
...
curl_version_info_data* curl_version_info_ccsid(CURLversion stamp, unsigned int ccsid) {
	...
	int i;
	const char** cpp;
	static const size_t charfields[] = {offsetof(curl_version_info_data, version),
			...
			case CURLINFO_CERTINFO:
				...
				if (cipf) {
					...
					if (!cipt) {
						...
						cipt->certinfo =
							(struct curl_slist**)calloc(cipf->num_of_certs + 1, sizeof(struct curl_slist*));
						if (!cipt->certinfo) {
							ret = CURLE_OUT_OF_MEMORY;
						} else {
							int i;
							...
							for (i = 0; i < cipf->num_of_certs; i++) {
								if (cipf->certinfo[i]) {
									if (!(cipt->certinfo[i] = slist_convert(ccsid, cipf->certinfo[i], ASCII_CCSID))) {
										ret = CURLE_OUT_OF_MEMORY;
										break;
									}
								}
...
CURLcode curl_easy_setopt_ccsid(CURL* easy, CURLoption tag, ...) {
	...
	default:
		if (tag / 10000 == CURLOPTTYPE_BLOB) {
			...
			if (bp && bp->data && bp->len && ccsid != NOCONV_CCSID && ccsid != ASCII_CCSID) {
...
CURLcode curl_multi_setopt_RPGnum_(CURLM* multi, CURLMoption tag, curl_off_t arg) {
...
char* curl_pushheader_byname_ccsid(struct curl_pushheaders* h, const char* header, unsigned int ccsidin,
...
static CURLcode mime_string_call(curl_mimepart* part, const char* string, unsigned int ccsid,
...
CURLcode curl_mime_data_ccsid(curl_mimepart* part, const char* data, size_t datasize, unsigned int ccsid) {
	char* s = (char*)NULL;
	CURLcode result;
...
CURLUcode curl_url_get_ccsid(CURLU* handle, CURLUPart what, char** part, unsigned int flags, unsigned int ccsid) {
	char* s = (char*)NULL;
	CURLUcode result;
...
CURLUcode curl_url_set_ccsid(CURLU* handle, CURLUPart what, const char* part, unsigned int flags, unsigned int ccsid) {
	char* s = (char*)NULL;
	CURLUcode result;
...
const struct curl_easyoption* curl_easy_option_by_name_ccsid(const char* name, unsigned int ccsid) {
...
const char* curl_easy_option_get_name_ccsid(const struct curl_easyoption* option, unsigned int ccsid) {
	char* name = NULL;
...
CURLHcode curl_easy_header_ccsid(CURL* easy, const char* name, size_t index, unsigned int origin, int request,
