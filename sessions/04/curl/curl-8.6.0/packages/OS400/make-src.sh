...
for SRC in ${CURLX_CFILES}; do
	MODULE=$(db2_name "${SRC}")
	MODULE=$(db2_name "X${MODULE}")
	make_module "${MODULE}" "${SRC}"
done

for SRC in ${CURL_CFILES}; do
	MODULE=$(db2_name "${SRC}")
	make_module "${MODULE}" "${SRC}"
done
...
if action_needed "${LIBIFSNAME}/CURLCL.PGM" "${SCRIPTDIR}/curlcl.c"; then
	CMD="CRTBNDC PGM(${TARGETLIB}/${CURLCLI})"
...
if action_needed "${LIBIFSNAME}/${CURLCMD}.CMD" "${SCRIPTDIR}/curl.cmd"; then
	CMD="CRTCMD CMD(${TARGETLIB}/${CURLCMD}) PGM(${TARGETLIB}/${CURLCLI})"
