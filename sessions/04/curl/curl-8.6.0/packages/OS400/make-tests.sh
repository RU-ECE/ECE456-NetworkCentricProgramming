...
build_all_programs() {
	...
	for PGM in ${noinst_PROGRAMS}; do
		...
		for SOURCE in ${PGMSOURCES}; do
			case "${SOURCE}" in
				*.c) #       Special processing for libxxx.c files:
					...
					if action_needed "${PGMIFSNAME}" "${MODIFSNAME}"; then
						LINK=yes
					fi
					;;
			esac
		...
		if [ "${LINK}" ]; then
			PGMLDADD="$(eval echo \"\${${PGM}_LDADD}\")"
			for ARG in ${PGMLDADD}; do
				case "${ARG}" in
					-*) ;; # Ignore non-module.
					*)
