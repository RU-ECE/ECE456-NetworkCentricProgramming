...
if action_needed "${LIBIFSNAME}"; then
	CMD="CRTLIB LIB(${TARGETLIB}) TEXT('curl: multiprotocol support API')"
	CLcommand "${CMD}"
fi

#       Create the DOCS source file if it does not exist.

if action_needed "${LIBIFSNAME}/DOCS.FILE"; then
	CMD="CRTSRCPF FILE(${TARGETLIB}/DOCS) RCDLEN(240)"
...
for TEXT in "${TOPDIR}/COPYING" "${SCRIPTDIR}/README.OS400" \
	...
	"${TOPDIR}/docs/HISTORY.md"; do
	MEMBER="$(basename \"${TEXT}\" .OS400)"
	MEMBER="$(basename \"${MEMBER}\" .md)"
	MEMBER="${LIBIFSNAME}/DOCS.FILE/$(db2_name \"${MEMBER}\").MBR"

	[ -e "${TEXT}" ] || continue

	if action_needed "${MEMBER}" "${TEXT}"; then
		CMD="CPY OBJ('${TEXT}') TOOBJ('${MEMBER}') TOCCSID(${TGTCCSID})"
...
if action_needed "${LIBIFSNAME}/RPGXAMPLES.FILE"; then
	CMD="CRTSRCPF FILE(${TARGETLIB}/RPGXAMPLES) RCDLEN(240)"
...
for EXAMPLE in "${SCRIPTDIR}/rpg-examples"/*; do
	MEMBER="$(basename \"${EXAMPLE}\")"
	IFSMEMBER="${LIBIFSNAME}/RPGXAMPLES.FILE/$(db2_name \"${MEMBER}\").MBR"

	[ -e "${EXAMPLE}" ] || continue

	if action_needed "${IFSMEMBER}" "${EXAMPLE}"; then
		CMD="CPY OBJ('${EXAMPLE}') TOOBJ('${IFSMEMBER}')"
...
if action_needed "${LIBIFSNAME}/CURLMAIN.MODULE" "${SCRIPTDIR}/curlmain.c"; then
	CMD="CRTCMOD MODULE(${TARGETLIB}/CURLMAIN)"
...
for SUBDIR in include lib src; do
	"${SCRIPTDIR}/make-${SUBDIR}.sh"
done
