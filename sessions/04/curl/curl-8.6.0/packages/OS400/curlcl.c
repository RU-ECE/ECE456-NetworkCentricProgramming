...
static int parse_command_line(const char* cmdargs, size_t len, size_t* argc, char** argv, size_t* argsize,
							  char* argbuf) {
...
int main(int argsc, struct arguments* args) {
	...
	exitcode = parse_command_line(args->cmdargs->string, args->cmdargs->len, &argc, NULL, &argsize, NULL);
	...
	if (!exitcode) {
		...
		if (!argv) {
			...
			parse_command_line(args->cmdargs->string, args->cmdargs->len, &argc, argv, &argsize,
							   (char*)(argv + argc + 1));
