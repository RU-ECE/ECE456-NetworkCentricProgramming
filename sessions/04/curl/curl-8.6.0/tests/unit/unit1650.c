...
static const struct dohrequest req[] = {{"test.host.name", DNS_TYPE_A, DNS_Q1, sizeof(DNS_Q1) - 1, 0},
...
static const struct dohresp resp[] = {
	...
	{"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x03\x66\x6f\x6f", 16, DNS_TYPE_A, DOH_DNS_OUT_OF_RANGE, NULL},
	{"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x03\x66\x6f\x6f\x00", 17, DNS_TYPE_A, DOH_DNS_OUT_OF_RANGE,
	 NULL},
	...
	{"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x01\x00\x00\x04\x61\x61\x61"
	 "\x61\x07\x65\x78\x61\x6d\x70\x6c\x65\x03\x63\x6f\x6d\x00\x00\x1c"
	 "\x00\x01\xc0\x0c\x00\x1c\x00\x01\x00\x00\x00\x37\x00\x10\x20\x20"
	 "\x20\x20\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x20\x20" LABEL_TEST LABEL_HOST LABEL_NAME DNSAAAA_EPILOGUE
	 "\x00\x00\x00\x01"
	 "\00\x04\x01\x01\x01\x01", /* RDDATA */

	 62 + 30, DNS_TYPE_AAAA, DOH_OK, "2020:2020:0000:0000:0000:0000:0000:2020 "},
...
UNITTEST_START {
	...
	for (i = 0; i < sizeof(req) / sizeof(req[0]); i++) {
		...
		if (req[i].packet && memcmp(req[i].packet, buffer, size)) {
			...
			abort_if(req[i].packet && memcmp(req[i].packet, buffer, size), "contents");
		}
	}
	...
	for (i = 1; i < sizeof(full49); i++) {
		...
		int rc;
		memset(&d, 0, sizeof(d));
		rc = doh_decode((const unsigned char*)&full49[i], sizeof(full49) - i - 1, DNS_TYPE_A, &d);
		if (!rc) {
			/* none of them should work */
