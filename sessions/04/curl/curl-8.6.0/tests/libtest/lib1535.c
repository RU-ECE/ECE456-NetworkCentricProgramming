...
int test(char* URL) {
	...
	CURL_IGNORE_DEPRECATION(res = curl_easy_getinfo(curl, CURLINFO_PROTOCOL, &protocol);)
	if (res) {
	...
	CURL_IGNORE_DEPRECATION(res = curl_easy_getinfo(dupe, CURLINFO_PROTOCOL, &protocol);)
	if (res) {
		fprintf(stderr, "%s:%d curl_easy_getinfo() failed with code %d (%s)\n", __FILE__, __LINE__, res,
				curl_easy_strerror(res));
		goto test_cleanup;
