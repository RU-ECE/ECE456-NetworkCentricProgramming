...
int test(char* URL) {
	...
	for (i = 0; i < NUM_HANDLES; i++) {
		...
		easy_init(curl[i]);
		/* specify target */
		msnprintf(target_url, sizeof(target_url), "http://server.example.curl:%s/path/1512%04i", port, i + 1);
		...
		CURL_IGNORE_DEPRECATION(easy_setopt(curl[i], CURLOPT_DNS_USE_GLOBAL_CACHE, 1L);)
	}
