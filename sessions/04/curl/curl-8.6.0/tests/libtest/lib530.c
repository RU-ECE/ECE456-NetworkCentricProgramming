...
static int addFd(struct Sockets* sockets, curl_socket_t fd, const char* what) {
	...
	if (!sockets->sockets) {
		...
		curl_socket_t* ptr = realloc(sockets->sockets, sizeof(curl_socket_t) * (sockets->max_count + 20));
		if (!ptr) {
...
static int socket_action(CURLM* curl, curl_socket_t s, int evBitmask, const char* info) {
	int numhandles = 0;
 ...
 */
static int checkFdSet(CURLM* curl, struct Sockets* sockets, fd_set* fdset, int evBitmask, const char* name) {
	int i;
	int result = 0;
	for (i = 0; i < sockets->count; ++i) {
