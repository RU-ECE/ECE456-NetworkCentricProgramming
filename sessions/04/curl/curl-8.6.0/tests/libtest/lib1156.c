...
static const struct testparams params[] = {
	...
	{F_RESUME | F_HTTP416 | F_FAIL | F_CONTENTRANGE | F_IGNOREBODY, CURLE_OK}};
...
static int onetest(CURL* curl, const char* url, const struct testparams* p, size_t num) {
	CURLcode res;
	...
	if (res != p->result) {
		printf("%zd: bad error code (%d): resume=%s, fail=%s, http416=%s, "
			   ...
			   num, res, (p->flags & F_RESUME) ? "yes" : "no", (p->flags & F_FAIL) ? "yes" : "no",
			   (p->flags & F_HTTP416) ? "yes" : "no", (p->flags & F_CONTENTRANGE) ? "yes" : "no", p->result);
		return 1;
	}
	if (hasbody && (p->flags & F_IGNOREBODY)) {
		printf("body should be ignored and is not: resume=%s, fail=%s, "
			   "http416=%s, content-range=%s\n",
			   (p->flags & F_RESUME) ? "yes" : "no", (p->flags & F_FAIL) ? "yes" : "no",
			   (p->flags & F_HTTP416) ? "yes" : "no", (p->flags & F_CONTENTRANGE) ? "yes" : "no");
		return 1;
	}
	return 0;
