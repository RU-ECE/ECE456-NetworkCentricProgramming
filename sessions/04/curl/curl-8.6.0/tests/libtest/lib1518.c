...
static size_t writecb(char* buffer, size_t size, size_t nitems, void* outstream) {
	(void)buffer;
	(void)size;
	(void)nitems;
	(void)outstream;
...
int test(char* URL) {
	...
	printf("res %d\n"
		   ...
		   "effectiveurl %s\n"
		   "redirecturl %s\n",
		   (int)res, (int)curlResponseCode, (int)curlRedirectCount, effectiveUrl, redirectUrl ? redirectUrl : "blank");
