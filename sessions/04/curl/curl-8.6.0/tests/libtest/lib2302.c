...
static size_t add_data(struct ws_data* wd, const char* buf, size_t blen, const struct curl_ws_frame* meta) {
	if ((wd->nwrites == 0) || (!!meta != !!wd->has_meta) || (meta && meta->flags != wd->meta_flags)) {
		if (wd->nwrites > 0)
			flush_data(wd);
