...
static size_t read_callback(char* ptr, size_t size, size_t nitems, void* userdata) {
...
int test(char* URL) {
	...
	res = perform_and_check_connections(curl, "First request without CURLOPT_KEEP_SENDING_ON_ERROR", 1);
	if (res != TEST_ERR_SUCCESS)
		goto test_cleanup;

	reset_data(&data, curl);

	res = perform_and_check_connections(curl, "Second request without CURLOPT_KEEP_SENDING_ON_ERROR", 1);
	if (res != TEST_ERR_SUCCESS)
		goto test_cleanup;
	...
	res = perform_and_check_connections(curl, "First request with CURLOPT_KEEP_SENDING_ON_ERROR", 1);
	if (res != TEST_ERR_SUCCESS)
		goto test_cleanup;

	reset_data(&data, curl);

	res = perform_and_check_connections(curl, "Second request with CURLOPT_KEEP_SENDING_ON_ERROR", 0);
	if (res != TEST_ERR_SUCCESS)
		goto test_cleanup;
