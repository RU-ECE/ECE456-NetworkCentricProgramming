...
static int ProcessRequest(struct httprequest* req) {
	...
	logmsg("Process %zu bytes request%s", req->offset, req->callcount > 1 ? " [CONTINUED]" : "");
		...
		if (fine) {
			...
			if (*ptr == '/') {
				if ((npath + strlen(request)) < 400) {
					msnprintf(logbuf, sizeof(logbuf), "Got request: %s %.*s HTTP/%d.%d", request, (int)npath, httppath,
							  prot_major, prot_minor);
				} else {
					msnprintf(logbuf, sizeof(logbuf), "Got a *HUGE* request HTTP/%d.%d", prot_major, prot_minor);
				}
				logmsg("%s", logbuf);
				...
				if (req->testno) {
					...
					msnprintf(logbuf, sizeof(logbuf), "Serve test number %ld part %ld", req->testno, req->partno);
					logmsg("%s", logbuf);
				} else {
			...
			if (req->testno == DOCNUMBER_NOTHING) {
				...
				if (sscanf(req->reqbuf, "CONNECT %" MAXDOCNAMELEN_TXT "s HTTP/%d.%d", doc, &prot_major, &prot_minor) ==
					3) {
					char* portp = NULL;

					msnprintf(logbuf, sizeof(logbuf), "Received a CONNECT %s HTTP/%d.%d request", doc, prot_major,
							  prot_minor);
					logmsg("%s", logbuf);
			...
			logmsg("** Unusual request. Starts with %02x %02x %02x (%c%c%c)", l[0], l[1], l[2], l[0], l[1], l[2]);
		}
	}
	...
	if (use_gopher) {
		...
		if (ptr) {
			...
			msnprintf(logbuf, sizeof(logbuf), "Requested GOPHER test number %ld part %ld", req->testno, req->partno);
			logmsg("%s", logbuf);
		}
	}
	...
	do {
		...
		} else if (strncasecompare("Transfer-Encoding: chunked", line, strlen("Transfer-Encoding: chunked"))) {
			/* chunked data coming in */
			chunked = TRUE;
		} else if (req->noexpect && strncasecompare("Expect: 100-continue", line, strlen("Expect: 100-continue"))) {
			if (req->cl)
				req->cl = 0;
			req->skipall = TRUE;
	...
	if (strstr(req->reqbuf, "Authorization: Negotiate")) {
		...
		logmsg("Negotiate: prev_testno: %ld, prev_partno: %ld", prev_testno, prev_partno);
	...
	if (req->open && req->prot_version >= 11 && req->reqbuf + req->offset > end + strlen(end_of_headers) && !req->cl &&
		(!strncmp(req->reqbuf, "GET", strlen("GET")) || !strncmp(req->reqbuf, "HEAD", strlen("HEAD")))) {
...
static void storerequest(const char* reqbuf, size_t totalsize) {
	...
	msnprintf(dumpfile, sizeof(dumpfile), "%s/%s", logdir, is_proxy ? REQUEST_PROXY_DUMP : REQUEST_DUMP);
	...
	if (writeleft == 0) {
		...
		logmsg("Wrote only (%zu bytes) of (%zu bytes) request input to %s", totalsize - writeleft, totalsize, dumpfile);
	}
...
static int send_doc(curl_socket_t sock, struct httprequest* req) {
	...
	msnprintf(responsedump, sizeof(responsedump), "%s/%s", logdir, is_proxy ? RESPONSE_PROXY_DUMP : RESPONSE_DUMP);
...
#define STREAMTHIS "a string to stream 01234567890\n"
	...
	if (req->testno < 0) {
		...
		case DOCNUMBER_WERULEZ:
			...
			msnprintf(msgbuf, sizeof(msgbuf), "WE ROOLZ: %" CURL_FORMAT_CURL_OFF_T "\r\n", our_getpid());
			msglen = strlen(msgbuf);
			if (use_gopher)
				msnprintf(weare, sizeof(weare), "%s", msgbuf);
			else
				msnprintf(weare, sizeof(weare), "HTTP/1.1 200 OK\r\nContent-Length: %zu\r\n\r\n%s", msglen, msgbuf);
			buffer = weare;
			break;
		case DOCNUMBER_404:
		default:
	...
	retry:
		...
		if (req->writedelay) {
			...
			if (msecs_left % MAX_SLEEP_TIME_MS)
				intervals++;
			logmsg("Pausing %d milliseconds after writing %zd bytes", msecs_left, written);
			while ((intervals > 0) && !got_exit_signal) {
				int sleep_time = msecs_left > MAX_SLEEP_TIME_MS ? MAX_SLEEP_TIME_MS : msecs_left;
				intervals--;
				wait_ms(sleep_time);
...
static curl_socket_t connect_to(const char* ipaddr, unsigned short port) {
	...
	if (CURL_SOCKET_BAD == serverfd) {
		error = SOCKERRNO;
		logmsg("Error creating socket for server connection: (%d) %s", error, sstrerror(error));
		return CURL_SOCKET_BAD;
	}
	...
	if (rc) {
		error = SOCKERRNO;
		logmsg("Error connecting to server port %hu: (%d) %s", port, error, sstrerror(error));
		sclose(serverfd);
		return CURL_SOCKET_BAD;
	}
...
static void http_connect(curl_socket_t* infdp, curl_socket_t rootfd, const char* ipaddr, unsigned short ipport,
	...
	while (!got_exit_signal) {
		...
		if ((clientfd[DATA] == CURL_SOCKET_BAD) && (serverfd[DATA] == CURL_SOCKET_BAD) && poll_client_rd[CTRL] &&
			...
			if ((clientfd[DATA] == CURL_SOCKET_BAD) && (serverfd[DATA] == CURL_SOCKET_BAD) &&
				...
				if (datafd != CURL_SOCKET_BAD) {
					...
					if (socket_domain_is_ip()) {
						/* Disable the Nagle algorithm */
						curl_socklen_t flag = 1;
						if (0 != setsockopt(datafd, IPPROTO_TCP, TCP_NODELAY, (void*)&flag, sizeof(flag)))
				...
				if (clientfd[i] != CURL_SOCKET_BAD) {
					...
					if (len && FD_ISSET(clientfd[i], &input)) {
						...
						if (rc <= 0) {
							...
							logmsg("[%s] READ \"%s\"", data_or_ctrl(i), data_to_hex(&readclient[i][tos[i]], rc));
							tos[i] += rc;
						}
					}
				}
				if (serverfd[i] != CURL_SOCKET_BAD) {
					...
					if (len && FD_ISSET(serverfd[i], &input)) {
						...
						if (rc <= 0) {
							...
							logmsg("[%s] READ \"%s\"", data_or_ctrl(i), data_to_hex(&readserver[i][toc[i]], rc));
							toc[i] += rc;
						}
					}
				}
				if (clientfd[i] != CURL_SOCKET_BAD) {
					if (toc[i] && FD_ISSET(clientfd[i], &output)) {
						...
						if (rc <= 0) {
							...
							logmsg("[%s] SENT \"%s\"", data_or_ctrl(i), data_to_hex(readserver[i], rc));
							if (toc[i] - rc)
				...
				if (serverfd[i] != CURL_SOCKET_BAD) {
					if (tos[i] && FD_ISSET(serverfd[i], &output)) {
						...
						if (rc <= 0) {
							...
							logmsg("[%s] SENT \"%s\"", data_or_ctrl(i), data_to_hex(readclient[i], rc));
							if (tos[i] - rc)
...
static curl_socket_t accept_connection(curl_socket_t sock) {
	...
	if (CURL_SOCKET_BAD == msgsock) {
		...
		logmsg("MAJOR ERROR: accept() failed with error: (%d) %s", error, sstrerror(error));
		return CURL_SOCKET_BAD;
	}

	if (0 != curlx_nonblock(msgsock, TRUE)) {
		error = SOCKERRNO;
		logmsg("curlx_nonblock failed with error: (%d) %s", error, sstrerror(error));
		sclose(msgsock);
		return CURL_SOCKET_BAD;
	}
	...
	if (0 != setsockopt(msgsock, SOL_SOCKET, SO_KEEPALIVE, (void*)&flag, sizeof(flag))) {
		error = SOCKERRNO;
		logmsg("setsockopt(SO_KEEPALIVE) failed with error: (%d) %s", error, sstrerror(error));
		sclose(msgsock);
		return CURL_SOCKET_BAD;
	}
...
/* returns 1 if the connection should be serviced again immediately, 0 if there
   is no data waiting, or < 0 if it should be closed */
static int service_connection(curl_socket_t msgsock, struct httprequest* req, curl_socket_t listensock,
							  const char* connecthost, int keepalive_secs) {
	if (got_exit_signal)
		return -1;
	...
	if (req->connect_request) {
		...
		if (!is_proxy) {
			...
			http_connect(&msgsock, listensock, connecthost, req->connect_port, keepalive_secs);
			return -1;
		}
	}
...
int main(int argc, char* argv[]) {
		...
		if (!strcmp("--version", argv[arg])) {
			...
			if (argc > arg) {
				...
				if (strlen(unix_socket) >= sizeof(me.sau.sun_path)) {
					fprintf(stderr, "sws: socket path must be shorter than %zu chars: %s\n", sizeof(me.sau.sun_path),
							unix_socket);
					return 0;
				}
				socket_type = "unix";
			...
			if (argc > arg) {
				...
				if ((endptr != argv[arg] + strlen(argv[arg])) || (ulnum && ((ulnum < 1025UL) || (ulnum > 65535UL)))) {
			...
			if (argc > arg) {
				char* endptr;
				unsigned long ulnum = strtoul(argv[arg], &endptr, 10);
				if ((endptr != argv[arg] + strlen(argv[arg])) || (ulnum && (ulnum > 65535UL))) {
					fprintf(stderr,
	...
	if (0 != setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void*)&flag, sizeof(flag))) {
		error = SOCKERRNO;
		logmsg("setsockopt(SO_REUSEADDR) failed with error: (%d) %s", error, sstrerror(error));
		goto sws_cleanup;
	}
	...
	if (0 != rc) {
		...
		if (socket_domain == AF_UNIX)
			logmsg("Error binding socket on path %s: (%d) %s", unix_socket, error, sstrerror(error));
		else
#endif
			logmsg("Error binding socket on port %hu: (%d) %s", port, error, sstrerror(error));
		goto sws_cleanup;
	}
	...
	if (!port) {
		...
		if (getsockname(sock, &localaddr.sa, &la_size) < 0) {
			error = SOCKERRNO;
			logmsg("getsockname() failed with error: (%d) %s", error, sstrerror(error));
			sclose(sock);
			goto sws_cleanup;
		}
	...
	logmsg("Running %s %s version on %s", protocol_type, socket_type, location_str);
	...
	for (;;) {
		...
		if (FD_ISSET(all_sockets[0], &input)) {
			...
			do {
				...
				logmsg("accept_connection %" CURL_FORMAT_SOCKET_T " returned %" CURL_FORMAT_SOCKET_T, sock, msgsock);
				if (CURL_SOCKET_BAD == msgsock)
					goto sws_cleanup;
				if (req->delay)
					wait_ms(req->delay);
		...
		for (socket_idx = 1; (socket_idx < num_sockets) && active; ++socket_idx) {
			if (FD_ISSET(all_sockets[socket_idx], &input)) {
				...
				do {
					rc = service_connection(all_sockets[socket_idx], req, sock, connecthost, keepalive_secs);
					if (got_exit_signal)
						goto sws_cleanup;
...
sws_cleanup:
	...
	if (got_exit_signal) {
		logmsg("========> %s sws (%s pid: %ld) exits with signal (%d)", socket_type, location_str, (long)getpid(),
			   exit_signal);
		/*
