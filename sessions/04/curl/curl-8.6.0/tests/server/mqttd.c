...
static void logprotocol(mqttdir dir, const char* prefix, size_t remlen, FILE* output, unsigned char* buffer,
						ssize_t len) {
	char data[12000] = "";
	ssize_t i;
	unsigned char* ptr = buffer;
...
static int encode_length(size_t packetlen, unsigned char* remlength) /* 4 bytes */
{
	int bytes = 0;
...
/* return 0 on success */
static int publish(FILE* dump, curl_socket_t fd, unsigned short packetid, char* topic, char* payload,
				   size_t payloadlen) {
...
static int fixedheader(curl_socket_t fd, unsigned char* bytep, size_t* remaining_lengthp,
	...
	*remaining_lengthp = decode_length(&buffer[1], i, remaining_length_bytesp);
	logmsg("Remaining Length: %zu [%zu bytes]", *remaining_lengthp, *remaining_length_bytesp);
	return 0;
}
	...
	do {
		...
		if (byte == MQTT_MSG_CONNECT) {
			...
			if (passwd_flag == (char)(conn_flags & passwd_flag)) {
				...
				payload_len += (size_t)(buffer[start_passwd] << 8) | buffer[start_passwd + 1];
				payload_len += 2; /* MSB and LSB for password length */
			}

			/* check the length of the payload */
			if ((ssize_t)payload_len != (rc - 12)) {
				logmsg("Payload length mismatch, expected %zx got %zx", rc - 12, payload_len);
				goto end;
			}
			...
			if (topic_len != (remaining_length - 5)) {
				logmsg("Wrong topic length, got %zu expected %zu", topic_len, remaining_length - 5);
				goto end;
			}
...
static curl_socket_t sockdaemon(curl_socket_t sock, unsigned short* listenport) {
	...
	if (rc) {
		logmsg("setsockopt(SO_REUSEADDR) failed %d times in %d ms. Error: (%d) %s", attempt, totdelay, error,
			   strerror(error));
	...
	if (rc) {
		error = SOCKERRNO;
		logmsg("Error binding socket on port %hu: (%d) %s", *listenport, error, sstrerror(error));
		sclose(sock);
		return CURL_SOCKET_BAD;
	}
	...
	if (0 != rc) {
		error = SOCKERRNO;
		logmsg("listen(%" CURL_FORMAT_SOCKET_T ", 5) failed with error: (%d) %s", sock, error, sstrerror(error));
		sclose(sock);
		return CURL_SOCKET_BAD;
	}
...
int main(int argc, char* argv[]) {
	...
	msnprintf(loglockfile, sizeof(loglockfile), "%s/%s/mqtt-%s.lock", logdir, SERVERLOGS_LOCKDIR, ipv_inuse);
