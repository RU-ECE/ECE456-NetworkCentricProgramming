...
void logmsg(const char* msg, ...) {
	...
	msnprintf(timebuf, sizeof(timebuf), "%02d:%02d:%02d.%06ld", (int)now->tm_hour, (int)now->tm_min, (int)now->tm_sec,
			  (long)tv.tv_usec);
...
static const char* win32_strerror(int err, char* buf, size_t buflen) {
	if (!FormatMessageA((FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS), NULL, (DWORD)err, LANG_NEUTRAL,
...
#endif
{
	...
	hidden_main_window = CreateWindowEx(0, TEXT("MainWClass"), TEXT("Recv WM_CLOSE msg"), WS_OVERLAPPEDWINDOW,
...
static SIGHANDLER_T set_signal(int signum, SIGHANDLER_T handler, bool restartable) {
...
#ifdef _WIN32_WCE
	thread = CreateThread(NULL, 0, &main_window_loop, (LPVOID)GetModuleHandle(NULL), 0, &thread_main_id);
#else
	thread = _beginthreadex(NULL, 0, &main_window_loop, (void*)GetModuleHandle(NULL), 0, &thread_main_id);
#endif
...
int bind_unix_socket(curl_socket_t sock, const char* unix_socket, struct sockaddr_un* sau) {
	...
	if (0 != rc && SOCKERRNO == EADDRINUSE) {
		...
		if (CURL_SOCKET_BAD == unixfd) {
			logmsg("Failed to create socket at %s: (%d) %s", unix_socket, SOCKERRNO, sstrerror(SOCKERRNO));
			return -1;
		}
		...
		if (0 != rc && ECONNREFUSED != error) {
			logmsg("Failed to connect to %s: (%d) %s", unix_socket, error, sstrerror(error));
			return rc;
		}
...
#endif
		if (0 != rc) {
			logmsg("Error binding socket, failed to stat %s: (%d) %s", unix_socket, errno, strerror(errno));
			return rc;
		}
#ifdef S_IFSOCK
		...
		if (0 != rc) {
			logmsg("Error binding socket, failed to unlink %s: (%d) %s", unix_socket, errno, strerror(errno));
			return rc;
		}
