...
static curl_socket_t socksconnect(unsigned short connectport, const char* connectaddr) {
	...
	if (rc) {
		int error = SOCKERRNO;
		logmsg("Error connecting to %s:%hu: (%d) %s", connectaddr, connectport, error, sstrerror(error));
		return CURL_SOCKET_BAD;
	}
...
static curl_socket_t sockit(curl_socket_t fd) {
	...
	if ((buffer[SOCKS5_NMETHODS] < config.nmethods_min) || (buffer[SOCKS5_NMETHODS] > config.nmethods_max)) {
		...
		for (i = 0; i < 2; i++) {
			...
			if (cp->used) {
				if (tunnel(cp, &fds_read)) {
					logmsg("SOCKS transfer completed. Bytes: < %zu > %zu", cp->fromremote, cp->fromclient);
					sclose(cp->clientfd);
					sclose(cp->remotefd);
					cp->used = FALSE;
					clients--;
		...
		if (!strcmp("--version", argv[arg])) {
			...
			if (argc > arg) {
				...
				if (strlen(unix_socket) >= sizeof(sau.sun_path)) {
					fprintf(stderr, "socksd: socket path must be shorter than %zu chars: %s\n", sizeof(sau.sun_path),
							unix_socket);
					return 0;
				}
