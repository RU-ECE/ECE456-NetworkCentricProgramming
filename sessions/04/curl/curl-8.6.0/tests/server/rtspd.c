...
static int ProcessRequest(struct httprequest* req) {
	...
	if ((req->testno == DOCNUMBER_NOTHING) &&
		sscanf(line, "%" REQUEST_KEYWORD_SIZE_TXT "s %" MAXDOCNAMELEN_TXT "s %4s/%d.%d", request, doc, prot_str,
			   &prot_major, &prot_minor) == 5) {
		char* ptr;
		char logbuf[256];
		...
		if (ptr) {
			...
			if ((strlen(doc) + strlen(request)) < 200) {
				msnprintf(logbuf, sizeof(logbuf), "Got request: %s %s %s/%d.%d", request, doc, prot_str, prot_major,
						  prot_minor);
			} else {
				msnprintf(logbuf, sizeof(logbuf), "Got a *HUGE* request %s/%d.%d", prot_str, prot_major, prot_minor);
			}
			logmsg("%s", logbuf);
			...
			msnprintf(logbuf, sizeof(logbuf), "Requested test number %ld part %ld", req->testno, req->partno);
			logmsg("%s", logbuf);
				...
				if (cmdsize) {
					...
					do {
						...
						if (!strncmp(CMD_AUTH_REQUIRED, ptr, strlen(CMD_AUTH_REQUIRED))) {
							...
							req->skip = num;
						} else if (3 <= sscanf(ptr, "rtp: part %d channel %d size %d size_err %d", &rtp_partno,
											   &rtp_channel, &rtp_size, &rtp_size_err)) {

							if (rtp_partno == req->partno) {
								int i = 0;
								logmsg("RTP: part %d channel %d size %d size_err %d", rtp_partno, rtp_channel, rtp_size,
									   rtp_size_err);
		...
		} else if (sscanf(req->reqbuf, "CONNECT %" MAXDOCNAMELEN_TXT "s HTTP/%d.%d", doc, &prot_major, &prot_minor) ==
				   3) {
			msnprintf(logbuf, sizeof(logbuf), "Received a CONNECT %s HTTP/%d.%d request", doc, prot_major, prot_minor);
			logmsg("%s", logbuf);
...
static void storerequest(char* reqbuf, size_t totalsize) {
	...
	if (!dump) {
		logmsg("Error opening file %s error: %d %s", dumpfile, error, strerror(error));
...
#define STREAMTHIS "a string to stream 01234567890\n"
	...
	if (req->testno < 0) {
		...
		case DOCNUMBER_WERULEZ:
			...
			msnprintf(msgbuf, sizeof(msgbuf), "RTSP_SERVER WE ROOLZ: %" CURL_FORMAT_CURL_OFF_T "\r\n", our_getpid());
...
int main(int argc, char* argv[]) {
	...
	msnprintf(loglockfile, sizeof(loglockfile), "%s/%s/rtsp-%s.lock", logdir, SERVERLOGS_LOCKDIR, ipv_inuse);
...
server_cleanup:
	...
	if (got_exit_signal) {
		logmsg("========> %s rtspd (port: %d pid: %ld) exits with signal (%d)", ipv_inuse, (int)port, (long)getpid(),
			   exit_signal);
		/*
