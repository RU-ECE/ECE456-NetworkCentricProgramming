 ...
 */
static bool read_data_block(unsigned char* buffer, ssize_t maxlen, ssize_t* buffer_len) {
...
#endif
{
	...
	case FILE_TYPE_CHAR:
		 ...
		 */
		while (WaitForMultipleObjectsEx(2, handles, FALSE, INFINITE, FALSE) == WAIT_OBJECT_0 + 1) {
...
static int select_ws(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, struct timeval* tv) {
	...
	for (i = 0; i < nfd; i++) {
		...
		if (wait != WAIT_FAILED && (wait - WAIT_OBJECT_0) <= i &&
			...
			if (FD_ISSET(wsasock, readfds) || FD_ISSET(wsasock, writefds) || FD_ISSET(wsasock, exceptfds))
				ret++;
		} else {
		...
		if (!memcmp("PING", buffer, 4)) {
			...
			if (*mode == PASSIVE_LISTEN) {
				...
				if (bytes_written != buffer_len)
					logmsg("Not all data was sent. Bytes to send: %zd sent: %zd", buffer_len, bytes_written);
			}
		} else if (!memcmp("DISC", buffer, 4)) {
			/* disconnect! */
			if (!write_stdout("ACKD\n", 5))
...
int main(int argc, char* argv[]) {
	...
	while (argc > arg) {
		if (!strcmp("--version", argv[arg])) {
			...
			if (argc > arg) {
				char* endptr;
				unsigned long ulnum = strtoul(argv[arg], &endptr, 10);
				if ((endptr != argv[arg] + strlen(argv[arg])) || (ulnum < 1025UL) || (ulnum > 65535UL)) {
	...
	if (connectport) {
		...
		if (rc) {
			error = SOCKERRNO;
			logmsg("Error connecting to port %hu: (%d) %s", connectport, error, sstrerror(error));
			write_stdout("FAIL\n", 5);
			goto sockfilt_cleanup;
		}
		logmsg("====> Client connect");
