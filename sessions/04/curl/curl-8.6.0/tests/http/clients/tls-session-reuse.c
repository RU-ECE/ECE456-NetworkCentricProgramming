...
static int debug_cb(CURL* handle, curl_infotype type, char* data, size_t size, void* userdata) {
	...
	if (!curl_easy_getinfo(handle, CURLINFO_XFER_ID, &xfer_id) && xfer_id >= 0)
		if (!curl_easy_getinfo(handle, CURLINFO_CONN_ID, &conn_id) && conn_id >= 0)
			curl_msnprintf(idsbuf, sizeof(idsbuf), TRC_IDS_FORMAT_IDS_2, xfer_id, conn_id);
		else
...
static void add_transfer(CURLM* multi, CURLSH* share, struct curl_slist* resolve, const char* url) {
	CURL* easy;
	CURLMcode mc;
	...
	do {
		...
		while ((msg = curl_multi_info_read(multi, &msgs_in_queue))) {
			if (msg->msg == CURLMSG_DONE) {
				...
				if (msg->data.result == CURLE_SEND_ERROR || msg->data.result == CURLE_RECV_ERROR) {
					/* We get these if the server had a GOAWAY in transit on
					 * re-using a connection */
				} else if (msg->data.result) {
					fprintf(stderr, "transfer #%" CURL_FORMAT_CURL_OFF_T ": failed with %d\n", xfer_id,
							msg->data.result);
					exit(1);
				} else if (status != 200) {
					fprintf(stderr, "transfer #%" CURL_FORMAT_CURL_OFF_T ": wrong http status %ld (expected 200)\n",
							xfer_id, status);
					exit(1);
				}
