...
static size_t my_write_cb(char* buf, size_t nitems, size_t buflen, void* userdata) {
	struct transfer* t = userdata;
	size_t nwritten;

	if (!t->resumed && t->recv_size < t->pause_at && ((t->recv_size + (curl_off_t)(nitems * buflen)) >= t->pause_at)) {
