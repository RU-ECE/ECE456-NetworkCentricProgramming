...
int main(int argc, char* argv[]) {
	...
	for (i = 0; i < HANDLECOUNT; i++) {
		...
		if (!handles[i].h || curl_easy_setopt(handles[i].h, CURLOPT_WRITEFUNCTION, cb) != CURLE_OK ||
			...
			curl_easy_setopt(handles[i].h, CURLOPT_DEBUGFUNCTION, debug_cb) != CURLE_OK ||
					...
					if (msg->easy_handle == handles[i].h) {
						if (handles[i].paused != 1 || !handles[i].resumed) {
							fprintf(stderr,
									...
									"result %d - wtf?\n",
									i, handles[i].paused, handles[i].resumed, msg->data.result);
							rc = 1;
							goto out;
						}
					}
