...
bool progress_meter(struct GlobalConfig* global, struct timeval* start, bool final) {
	...
	if (final || (diff > 500)) {
		...
		if (dlknown && all_dltotal) {
			...
			msnprintf(dlpercen, sizeof(dlpercen), "%3" CURL_FORMAT_CURL_OFF_T, all_dlnow * 100 / all_dltotal);
		}
		if (ulknown && all_ultotal) {
			/* TODO: handle integer overflow */
			msnprintf(ulpercen, sizeof(ulpercen), "%3" CURL_FORMAT_CURL_OFF_T, all_ulnow * 100 / all_ultotal);
		}
		...
		fprintf(tool_stderr,
				...
				dlpercen, /* 3 letters */
				ulpercen, /* 3 letters */
				max5data(all_dlnow, buffer[0]), max5data(all_ulnow, buffer[1]), all_xfers, all_running, time_total,
				time_spent, time_left, max5data(speed, buffer[2]), /* speed */
				final ? "\n" : "");
		return TRUE;
	}
	return FALSE;
