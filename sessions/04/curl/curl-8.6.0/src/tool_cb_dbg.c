...
static void dump(const char* timebuf, const char* idsbuf, const char* text, FILE* stream, const unsigned char* ptr,
				 size_t size, trace tracetype, curl_infotype infotype);
...
static const char* hms_for_sec(time_t tv_sec) {
	...
	if (tv_sec != cached_tv_sec) {
		...
		msnprintf(hms_buf, sizeof(hms_buf), "%02d:%02d:%02d", now->tm_hour, now->tm_min, now->tm_sec);
		cached_tv_sec = tv_sec;
	}
	return hms_buf;
}

static void log_line_start(FILE* log, const char* timebuf, const char* idsbuf, curl_infotype type) {
	/*
...
int tool_debug_cb(CURL* handle, curl_infotype type, char* data, size_t size, void* userdata) {
	...
	if (config->tracetime) {
		tv = tvnow();
		msnprintf(timebuf, sizeof(timebuf), "%s.%06ld ", hms_for_sec(tv.tv_sec), (long)tv.tv_usec);
	} else {
		timebuf[0] = 0;
	}

	if (handle && config->traceids && !curl_easy_getinfo(handle, CURLINFO_XFER_ID, &xfer_id) && xfer_id >= 0)
		if (!curl_easy_getinfo(handle, CURLINFO_CONN_ID, &conn_id) && conn_id >= 0)
		...
		case CURLINFO_SSL_DATA_OUT:
			if (!traced_data) {
				/* if the data is output to a tty and we're sending this debug trace
				   ...
				   function */
				if (!config->isatty || ((output != tool_stderr) && (output != stdout))) {
					if (!newl)
						log_line_start(output, timebuf, idsbuf, type);
	...
	dump(timebuf, idsbuf, text, output, (unsigned char*)data, size, config->tracetype, type);
	return 0;
}
