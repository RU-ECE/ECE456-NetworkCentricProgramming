...
static int writeTime(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
					 bool use_json);

static int writeString(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
					   bool use_json);

static int writeLong(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
					 bool use_json);

static int writeOffset(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
					   bool use_json);
...
static const struct httpmap http_version[] = {
	{"0", CURL_HTTP_VERSION_NONE}, {"1", CURL_HTTP_VERSION_1_0}, {"1.1", CURL_HTTP_VERSION_1_1},
	{"2", CURL_HTTP_VERSION_2},	   {"3", CURL_HTTP_VERSION_3},	 {NULL, 0} /* end of list */
};
...
static const struct writeoutvar variables[] = {
	...
	{"proxy_ssl_verify_result", VAR_PROXY_SSL_VERIFY_RESULT, CURLINFO_PROXY_SSL_VERIFYRESULT, writeLong},
	...
	{"speed_download", VAR_SPEED_DOWNLOAD, CURLINFO_SPEED_DOWNLOAD_T, writeOffset},
	{"speed_upload", VAR_SPEED_UPLOAD, CURLINFO_SPEED_UPLOAD_T, writeOffset},
	{"ssl_verify_result", VAR_SSL_VERIFY_RESULT, CURLINFO_SSL_VERIFYRESULT, writeLong},
	{"stderr", VAR_STDERR, CURLINFO_NONE, NULL},
	{"stdout", VAR_STDOUT, CURLINFO_NONE, NULL},
	{"time_appconnect", VAR_APPCONNECT_TIME, CURLINFO_APPCONNECT_TIME_T, writeTime},
	{"time_connect", VAR_CONNECT_TIME, CURLINFO_CONNECT_TIME_T, writeTime},
	{"time_namelookup", VAR_NAMELOOKUP_TIME, CURLINFO_NAMELOOKUP_TIME_T, writeTime},
	{"time_pretransfer", VAR_PRETRANSFER_TIME, CURLINFO_PRETRANSFER_TIME_T, writeTime},
	{"time_redirect", VAR_REDIRECT_TIME, CURLINFO_REDIRECT_TIME_T, writeTime},
	{"time_starttransfer", VAR_STARTTRANSFER_TIME, CURLINFO_STARTTRANSFER_TIME_T, writeTime},
...
static int writeTime(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
	...
	if (valid) {
		...
		fprintf(stream, "%" CURL_FORMAT_CURL_OFF_TU ".%06" CURL_FORMAT_CURL_OFF_TU, secs, us);
	} else {
		if (use_json)
...
static int urlpart(struct per_transfer* per, writeoutid vid, const char** contentp) {
	CURLU* uh = curl_url();
	int rc = 0;
	if (uh) {
		...
		if (!rc && curl_url_set(uh, CURLUPART_URL, url, CURLU_GUESS_SCHEME | CURLU_NON_SUPPORT_SCHEME))
			rc = 2;
...
static int writeString(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
	...
	if (wovar->ci) {
			...
			if (per->certinfo) {
				...
				for (i = 0; (i < per->certinfo->num_of_certs) && !error; i++) {
					...
					for (slist = per->certinfo->certinfo[i]; slist; slist = slist->next) {
						...
						} else if (curlx_dyn_add(&buf, slist->data)) {
							error = TRUE;
							break;
						}
		...
		case VAR_ERRORMSG:
			if (per_result) {
				strinfo = (per->errorbuffer && per->errorbuffer[0]) ? per->errorbuffer : curl_easy_strerror(per_result);
				valid = true;
			}
			break;
...
static int writeLong(FILE* stream, const struct writeoutvar* wovar, struct per_transfer* per, CURLcode per_result,
	...
	if (valid) {
		...
		else if (wovar->id == VAR_HTTP_CODE || wovar->id == VAR_HTTP_CODE_PROXY)
...
void ourWriteOut(struct OperationConfig* config, struct per_transfer* per, CURLcode per_result) {
	FILE* stream = stdout;
			...
			if ('%' == ptr[1]) {
				...
				if ('{' == ptr[1]) {
					...
					for (i = 0; variables[i].name; i++) {
						if ((strlen(variables[i].name) == vlen) && curl_strnequal(ptr, variables[i].name, vlen)) {
							match = TRUE;
							...
							case VAR_HEADER_JSON:
								...
								break;
							default:
								(void)variables[i].writefunc(stream, &variables[i], per, per_result, false);
								break;
							}
							break;
						}
					...
					if (end) {
						...
						if (vlen < sizeof(hname)) {
							...
							hname[vlen] = 0;
							if (CURLHE_OK == curl_easy_header(per->curl, hname, 0, CURLH_HEADER, -1, &header))
