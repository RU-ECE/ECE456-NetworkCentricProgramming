...
CURLcode ipfs_url_rewrite(CURLU* uh, const char* protocol, char** url, struct OperationConfig* config) {
	...
	if (config->ipfs_gateway) {
		...
		if (!curl_url_set(gatewayurl, CURLUPART_URL, config->ipfs_gateway, CURLU_GUESS_SCHEME)) {
	...
	if (curl_url_get(gatewayurl, CURLUPART_SCHEME, &gwscheme, CURLU_URLDECODE))
		goto clean;
	...
	pathbuffer = aprintf("%s%s/%s%s", gwpath, protocol, cid, inputpath ? inputpath : "");
	if (!pathbuffer)
		goto clean;
