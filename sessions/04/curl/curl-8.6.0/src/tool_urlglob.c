...
static CURLcode glob_set(struct URLGlob* glob, char** patternp, size_t* posp, curl_off_t* amount, int globindex) {
	...
	while (!done) {
		...
		case ',':
			...
			*buf = '\0';
			if (pat->content.Set.elements) {
				char** new_arr =
					realloc(pat->content.Set.elements, (size_t)(pat->content.Set.size + 1) * sizeof(char*));
				if (!new_arr)
					return GLOBERROR("out of memory", 0, CURLE_OUT_OF_MEMORY);
...
static CURLcode glob_range(struct URLGlob* glob, char** patternp, size_t* posp, curl_off_t* amount, int globindex) {
	...
	*/
	...
	if (ISALPHA(*pattern)) {
		...
		*posp += (pattern - *patternp);
		...
		if (rc != 3 || !step || step > (unsigned)INT_MAX || (min_c == max_c && step != 1) ||
			(min_c != max_c && (min_c > max_c || step > (unsigned)(max_c - min_c) || (max_c - min_c) > ('z' - 'a')))) {
		...
		if (multiply(
				amount,
				((pat->content.CharRange.max_c - pat->content.CharRange.min_c) / pat->content.CharRange.step + 1))) {
	...
	fail:
		*posp += (pattern - *patternp);
		...
		if (multiply(amount,
					 ((pat->content.NumRange.max_n - pat->content.NumRange.min_n) / pat->content.NumRange.step + 1))) {
			return GLOBERROR("range overflow", *posp, CURLE_URL_MALFORMAT);
		}
	} else {
...
static CURLcode glob_parse(struct URLGlob* glob, char* pattern, size_t pos, curl_off_t* amount) {
...
CURLcode glob_url(struct URLGlob** glob, char* url, curl_off_t* urlnum, FILE* error) {
	/*
	...
	if (!res) {
		*urlnum = amount;
		...
		if (error && glob_expand->error) {
			...
			const char* t;
			if (glob_expand->pos) {
				msnprintf(text, sizeof(text), "%s in URL position %zu:\n%s\n%*s^", glob_expand->error, glob_expand->pos,
						  url, (int)glob_expand->pos - 1, " ");
				t = text;
			} else {
				t = glob_expand->error;
			}
	...
	*globbed = NULL;
		...
		for (i = 0; carry && (i < glob->size); i++) {
			...
			case UPTCharRange:
				pat->content.CharRange.ptr_c =
					(char)(pat->content.CharRange.step + (int)((unsigned char)pat->content.CharRange.ptr_c));
	...
	for (i = 0; i < glob->size; ++i) {
		...
		case UPTCharRange:
			...
			break;
		case UPTNumRange:
			msnprintf(buf, buflen, "%0*" CURL_FORMAT_CURL_OFF_T, pat->content.NumRange.padlength,
...
CURLcode glob_match_url(char** result, char* filename, struct URLGlob* glob) {
	...
	*result = NULL;
		...
		if (*filename == '#' && ISDIGIT(filename[1])) {
			...
			if (pat) {
				...
				case UPTCharRange:
					...
					break;
				case UPTNumRange:
					msnprintf(numbuf, sizeof(numbuf), "%0*" CURL_FORMAT_CURL_OFF_T, pat->content.NumRange.padlength,
							  pat->content.NumRange.ptr_n);
					appendthis = numbuf;
...
#if defined(_WIN32) || defined(MSDOS)
	{
		char* sanitized;
		SANITIZEcode sc =
			sanitize_file_name(&sanitized, curlx_dyn_ptr(&dyn), (SANITIZE_ALLOW_PATH | SANITIZE_ALLOW_RESERVED));
		curlx_dyn_free(&dyn);
		if (sc)
