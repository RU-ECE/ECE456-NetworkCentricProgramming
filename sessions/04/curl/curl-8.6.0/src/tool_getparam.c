	...
	while (*param_place) {
		...
		case ':':
			/* Since we live in a world of weirdness and confusion, the win32
			   ...
			   windows. */
#ifdef _WIN32
			if ((param_place == &cert_parameter[1]) && (cert_parameter[2] == '\\' || cert_parameter[2] == '/') &&
...
static size_t replace_url_encoded_space_by_plus(char* url) {
	...
	while (orig_index < orig_len) {
		if ((url[orig_index] == '%') && (url[orig_index + 1] == '2') && (url[orig_index + 2] == '0')) {
			url[new_index] = '+';
			orig_index += 3;
		} else {
 ...
 */
static ParameterError GetSizeParameter(struct GlobalConfig* global, const char* arg, const char* which,
...
/* --data-urlencode */
static ParameterError data_urlencode(struct GlobalConfig* global, char* nextarg, char** postp, size_t* lenp) {
...
static void sethttpver(struct GlobalConfig* global, struct OperationConfig* config, long httpversion) {
...
static CURLcode set_trace_config(struct GlobalConfig* global, const char* config) {
...
static ParameterError url_query(char* nextarg, struct GlobalConfig* global, struct OperationConfig* config) {
	size_t size = 0;
...
static ParameterError set_data(cmdline_t cmd, char* nextarg, struct GlobalConfig* global,
	...
	if (curlx_dyn_len(&config->postdata)) {
		...
		if (!err && (cmd != C_JSON) && curlx_dyn_addn(&config->postdata, "&", 1))
			err = PARAM_NO_MEM;
	}
...
static ParameterError set_rate(struct GlobalConfig* global, char* nextarg) {
	/* --rate */
...
ParameterError getparameter(const char* flag, /* f or -long-flag */
							...
							argv_item_t cleararg, bool* usedarg, /* set to TRUE if the arg
																	has been used */
							struct GlobalConfig* global, struct OperationConfig* config) {
	int rc;
	...
	*usedarg = FALSE; /* default is that we don't use the arg */
	...
	if (('-' != flag[0]) || ('-' == flag[1])) {
		...
		a = bsearch(&key, aliases, sizeof(aliases) / sizeof(aliases[0]), sizeof(aliases[0]), findarg);
		if (a) {
			longopt = TRUE;
		} else {
		...
		if (noflagged && (a->desc != ARG_BOOL)) {
			...
			if ((a->desc != ARG_STRG) && (a->desc != ARG_FILE)) {
	...
	do {
		...
		if (a->desc >= ARG_STRG) {
			...
			if ((a->desc == ARG_FILE) && (nextarg[0] == '-') && nextarg[1]) {
				/* if the file name looks like a command line option */
				warnf(global, "The file name argument '%s' looks like a flag.", nextarg);
			}
		...
		case C_SSL: /* --ssl */
			if (toggle && !feature_ssl) {
				...
				if (config->ftp_ssl)
					warnf(global, "--ssl is an insecure option, consider --ssl-reqd instead");
			}
			break;
		...
		case C_ENGINE: /* --engine */
			...
			if (!err && config->engine && !strcmp(config->engine, "list"))
		...
		case C_FAIL_WITH_BODY: /* --fail-with-body */
			...
			if (config->failonerror && config->failwithbody) {
				errorf(config->global,
					   "You must select either --fail or "
		...
		case C_RANGE: /* --range */
			...
			if (ISDIGIT(*nextarg) && !strchr(nextarg, '-')) {
				...
				if (curlx_strtoofft(nextarg, NULL, 10, &value)) {
					...
					msnprintf(buffer, sizeof(buffer), "%" CURL_FORMAT_CURL_OFF_T "-", value);
	...
	for (i = 1, stillflags = TRUE; i < argc && !result; i++) {
		...
		if (stillflags && ('-' == orig_opt[0])) {
			...
			if (!strcmp("--", orig_opt)) {
				...
				result = getparameter(orig_opt, nextarg, argv[i + 1], &passarg, global, config);
