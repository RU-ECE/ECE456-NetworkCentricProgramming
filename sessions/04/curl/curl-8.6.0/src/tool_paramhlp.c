...
ParameterError str2num(long* val, const char* str) { return getnum(val, str, 10); }
...
ParameterError proto2num(struct OperationConfig* config, const char* const* val, char** ostr, const char* str) {
	char* buffer;
	const char* sep = ",";
	char* token;
	const char** protoset;
	...
	for (token = strtok(buffer, sep); token; token = strtok(NULL, sep)) {
		...
		if (curl_strequal(token, "all")) {
			...
			case allow:
			case set:
				memcpy((char*)protoset, built_in_protos, (proto_count + 1) * sizeof(*protoset));
				break;
			}
		} else {
	...
	qsort((char*)protoset, protoset_index(protoset, NULL), sizeof(*protoset), struplocompare4sort);
							...
							char** userpwd) /* pointer to allocated string */
{
	...
	if (!psep && **userpwd != ';') {
		...
		if (!i && last) {
			curlx_msnprintf(prompt, sizeof(prompt), "Enter %s password for user '%s':", kind, *userpwd);
		} else {
			curlx_msnprintf(prompt, sizeof(prompt), "Enter %s password for user '%s' on URL #%zu:", kind, *userpwd,
							i + 1);
		}
...
static bool inlist(const struct curl_slist* head, const char* checkfor) {
	...
	for (; head; head = head->next)
		if (curl_strnequal(head->data, checkfor, thislen) && isheadersep(head->data[thislen]))
			return TRUE;
