...
#ifdef LINK
static void write_linked_location(CURL* curl, const char* location, size_t loclen, FILE* stream);
#endif
...
size_t tool_header_cb(char* ptr, size_t size, size_t nmemb, void* userdata) {
	...
	if (hdrcbdata->config->show_headers &&
		(scheme == proto_http || scheme == proto_https || scheme == proto_rtsp || scheme == proto_file)) {
		...
		if (value) {
			...
			if (curl_strnequal("Location", ptr, namelen))
				write_linked_location(per->curl, &value[1], cb - namelen - 1, outs->stream);
			else
				fwrite(&value[1], cb - namelen - 1, 1, outs->stream);
#endif
		} else {
...
static void write_linked_location(CURL* curl, const char* location, size_t loclen, FILE* stream) {
	...
	if (!strcmp("http", scheme) || !strcmp("https", scheme) || !strcmp("ftp", scheme) || !strcmp("ftps", scheme)) {
		fprintf(stream, "%.*s" LINK "%s" LINKST "%.*s" LINKOFF, space_skipped, location, finalurl,
