...
static CURLcode single_transfer(struct GlobalConfig* global, struct OperationConfig* config, CURLSH* share,
								  ...
								  long* delay) /* milliseconds! */
{
	...
	*delay = 0; /* for no retry, keep it zero */
	...
	if (is_vms_shell()) {
...
#endif
		if (!config->synthetic_error && result && (!global->silent || global->showerror)) {
		const char* msg = per->errorbuffer;
		fprintf(tool_stderr, "curl: (%d) %s\n", result, (msg && msg[0]) ? msg : curl_easy_strerror(result));
		...
		if (code >= 400) {
			if (!global->silent || global->showerror) {
				fprintf(tool_stderr, "curl: (%d) The requested URL returned error: %ld\n", CURLE_HTTP_RETURNED_ERROR,
						code);
			}
	...
	if (!result && config->xattr && outs->fopened && outs->stream) {
		...
		if (rc)
			warnf(config->global, "Error setting extended attributes on '%s': %s", outs->filename, strerror(errno));
	}
	...
	if (per->retry_numretries &&
		(!config->retry_maxtime || (tvdiff(tvnow(), per->retrystart) < config->retry_maxtime * 1000L))) {
		enum {
			RETRY_NO,
			RETRY_ALL_ERRORS,
			RETRY_TIMEOUT,
		...
		if ((CURLE_OPERATION_TIMEDOUT == result) || (CURLE_COULDNT_RESOLVE_HOST == result) ||
			...
			if (ECONNREFUSED == oserrno)
				retry = RETRY_CONNREFUSED;
				   ...
				   ((config->failonerror || config->failwithbody) && (CURLE_HTTP_RETURNED_ERROR == result))) {
		...
		if (retry) {
			...
			if (outs->bytes && outs->filename && outs->stream) {
				 ...
				 */
				notef(config->global, "Throwing away %" CURL_FORMAT_CURL_OFF_T " bytes", outs->bytes);
				fflush(outs->stream);
 ...
 */
static CURLcode url_proto(char** url, struct OperationConfig* config, char** scheme) {
	CURLcode result = CURLE_OK;
	CURLU* uh = curl_url();
	...
	if (uh) {
		if (*url) {
			...
			if (!curl_url_set(uh, CURLUPART_URL, *url, CURLU_GUESS_SCHEME | CURLU_NON_SUPPORT_SCHEME) &&
				...
				if (curl_strequal(schemep, proto_ipfs) || curl_strequal(schemep, proto_ipns)) {
				...
				*added = TRUE;
				...
				if ((urlnode->flags & GETOUT_USEREMOTE) || (per->outfile && strcmp("-", per->outfile))) {
					...
					if (!per->outfile) {
						...
						if (result) {
							errorf(global,
								   "Failed to extract a sensible file name"
				...
				if (per->uploadfile && !stdin_upload(per->uploadfile)) {
					 ...
					 */
					result = add_file_name_to_url(per->curl, &per->this_url, per->uploadfile);
					if (result)
						break;
					...
					if (!strcmp(per->uploadfile, ".")) {
						if (curlx_nonblock((curl_socket_t)per->infd, TRUE) < 0)
							warnf(global, "fcntl failed on fd=%d: %s", per->infd, strerror(errno));
					}
				}
				...
				if (httpgetfields || config->query) {
					...
					if (uh) {
						CURLUcode uerr;
						uerr = curl_url_set(uh, CURLUPART_URL, per->this_url, CURLU_GUESS_SCHEME);
						if (uerr) {
				...
				my_setopt(curl, CURLOPT_NOPROGRESS, global->noprogress || global->silent ? 1L : 0L);
				if (config->no_body)
				...
				case HTTPREQ_SIMPLEPOST:
					if (config->resume_from) {
						...
						my_setopt_str(curl, CURLOPT_POSTFIELDS, curlx_dyn_ptr(&config->postdata));
						my_setopt(curl, CURLOPT_POSTFIELDSIZE_LARGE, (curl_off_t)curlx_dyn_len(&config->postdata));
					}
					break;
				...
				if (feature_ssl) {
					...
					{
						long mask = (config->proxy_ssl_allow_beast ? CURLSSLOPT_ALLOW_BEAST : 0) |
				...
				if ((use_proto == proto_scp || use_proto == proto_sftp) && !config->insecure_ok) {
...
#define MAX_COOKIE_LINE 8200
				...
				if ((global->progressmode == CURL_PROGRESS_BAR) && !global->noprogress && !global->silent) {
				...
				/* curl 7.69.x */
				my_setopt(curl, CURLOPT_MAIL_RCPT_ALLOWFAILS, config->mail_rcpt_allowfails ? 1L : 0L);
				...
				if (config->expect100timeout_ms > 0)
					my_setopt_str(curl, CURLOPT_EXPECT_100_TIMEOUT_MS, config->expect100timeout_ms);
				...
				per->retry_sleep_default =
					(config->retry_delay) ? config->retry_delay * 1000L : RETRY_SLEEP_DEFAULT; /* ms */
 ...
 */
static CURLcode add_parallel_transfers(struct GlobalConfig* global, CURLM* multi, CURLSH* share, bool* morep,
									   bool* addedp) {
		...
		if (!mcode) {
			...
			do {
				...
				if (msg) {
					...
					if (ended->abort && (tres == CURLE_ABORTED_BY_CALLBACK) && ended->errorbuffer) {
	...
	for (per = transfers; per;) {
		...
		if (per && global->ms_per_transfer) {
			...
			if (milli < global->ms_per_transfer) {
				notef(global, "Transfer took %ld ms, waits %ldms as set by --rate", milli,
					  global->ms_per_transfer - milli);
...
static CURLcode transfer_per_config(struct GlobalConfig* global, struct OperationConfig* config, CURLSH* share,
									bool* added) {
	CURLcode result = CURLE_OK;
