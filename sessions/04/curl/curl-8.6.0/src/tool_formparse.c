...
static struct tool_mime* tool_mime_new_parts(struct tool_mime* parent) { return tool_mime_new(parent, TOOLMIME_PARTS); }

static struct tool_mime* tool_mime_new_data(struct tool_mime* parent, char* mime_data) {
	char* mime_data_copy;
...
static struct tool_mime* tool_mime_new_filedata(struct tool_mime* parent, const char* filename, bool isremotefile,
												CURLcode* errcode) {
	...
	if (m) {
		...
		if (!ret) {
			...
			case TOOLMIME_STDINDATA:
				ret = curl_mime_data_cb(part, m->size, (curl_read_callback)tool_mime_stdin_read,
 ...
 */
static char* get_param_word(struct OperationConfig* config, char** str, char** end_pos, char endchar) {
	char* ptr = *str;
...
static int read_field_headers(struct OperationConfig* config, const char* filename, FILE* fp,
	...
	for (;;) {
		...
		case EOF:
			if (ferror(fp)) {
				errorf(config->global, "Header file %s read error: %s", filename, strerror(errno));
				return -1;
			}
			return 0; /* Done. */
		case '\r':
		...
		if (!incomment) {
			if (hdrlen == sizeof(hdrbuf) - 1) {
				warnf(config->global, "File %s line %d: header too long (truncated)", filename, lineno);
				c = ' ';
			}
			if (hdrlen <= sizeof(hdrbuf) - 1)
...
static int get_param_part(struct OperationConfig* config, char endchar, char** str, char** pdata, char** ptype,
						  char** pfilename, char** pencoder, struct curl_slist** pheaders) {
	char* p = *str;
	char* type = NULL;
	char* filename = NULL;
	char* encoder = NULL;
			...
			*endpos = '\0';
			...
			if (*p == '@' || *p == '<') {
				...
				*endpos = '\0';
				...
				if (!fp) {
					warnf(config->global, "Cannot read from %s: %s", hdrfile, strerror(errno));
				} else {
...
int formparse(struct OperationConfig* config, const char* input, struct tool_mime** mimeroot,
			  struct tool_mime** mimecurrent, bool literal_value) {
		...
		*contp++ = '\0';
		...
		if (*contp == '(' && !literal_value) {
			...
			*mimecurrent = (*mimecurrent)->parent;
			...
			if (sep) {
				*contp = (char)sep;
				warnf(config->global, "garbage at end of field specification: %s", contp);
			}
		}
