...
SANITIZEcode sanitize_file_name(char** const sanitized, const char* file_name, int flags) {
	...
	*sanitized = NULL;
	...
	for (; *p; ++p) {
		...
		if ((1 <= *p && *p <= 31) || (!(flags & (SANITIZE_ALLOW_COLONS | SANITIZE_ALLOW_PATH)) && *p == ':') ||
...
SANITIZEcode msdosify(char** const sanitized, const char* file_name, int flags) {
	...
	*d = '\0';
	...
	if (*s) {
		...
		if (!(flags & SANITIZE_ALLOW_TRUNCATE) || strpbrk(s, "\\/:") || truncate_dryrun(dos_name, d - dos_name))
			return SANITIZE_ERR_INVALID_PATH;
	}
...
SANITIZEcode rename_if_reserved_dos_device_name(char** const sanitized, const char* file_name, int flags) {
	...
	*sanitized = NULL;
	...
	for (p = fname; p; p = (p == fname && fname != base ? base : NULL)) {
		size_t p_len;
		int x = (curl_strnequal(p, "CON", 3) || curl_strnequal(p, "PRN", 3) || curl_strnequal(p, "AUX", 3) ||
				 curl_strnequal(p, "NUL", 3))
			? 3
			: (curl_strnequal(p, "CLOCK$", 6))							   ? 6
			: (curl_strnequal(p, "COM", 3) || curl_strnequal(p, "LPT", 3)) ? (('1' <= p[3] && p[3] <= '9') ? 4 : 3)
																		   : 0;
...
CURLcode FindWin32CACert(struct OperationConfig* config, curl_sslbackend backend, const TCHAR* bundle_file) {
...
static void init_terminal(void) {
	...
	if (TerminalSettings.hStdOut == INVALID_HANDLE_VALUE ||
		...
		!curlx_verify_windows_version(10, 0, 16299, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
		return;
	}
