...
int getfiletime(const char* filename, struct GlobalConfig* global, curl_off_t* stamp) {
	int rc = 1;
...
/* Windows stat() may attempt to adjust the unix GMT file time by a daylight
   ...
   access to a 64-bit type we can bypass stat and get the times directly. */
	...
	hfile = CreateFile(tchar_filename, FILE_READ_ATTRIBUTES, (FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE),
...
/* Windows utime() may attempt to adjust the unix GMT file time by a daylight
   ...
   access to a 64-bit type we can bypass utime and set the times directly. */
		...
		if (filetime > CURL_OFF_T_C(910670515199)) {
			warnf(global, "Failed to set filetime %" CURL_FORMAT_CURL_OFF_T " on outfile: overflow", filetime);
			curlx_unicodefree(tchar_filename);
			return;
		}
		...
		if (hfile != INVALID_HANDLE_VALUE) {
			...
			warnf(global,
				  "Failed to set filetime %" CURL_FORMAT_CURL_OFF_T " on outfile: CreateFile failed: GetLastError %u",
				  filetime, (unsigned int)GetLastError());
		}
		...
		if (utimes(filename, times)) {
			warnf(global, "Failed to set filetime %" CURL_FORMAT_CURL_OFF_T " on '%s': %s", filetime, filename,
				  strerror(errno));
		}
