...
static const QUIC_BUFFER Alpn = {(uint32_t)(sizeof("filequic") - 1), (uint8_t*)"filequic"};

static void quic_check(QUIC_STATUS st, const char* ctx) {
	if (QUIC_FAILED(st)) {
		ostringstream oss;
		oss << ctx << " failed: 0x" << hex << st;
...
static HQUIC make_config(bool is_server, const char* cert_file = nullptr, const char* key_file = nullptr) {
	QUIC_SETTINGS s{};
	...
	quic_check(MsQuic->ConfigurationOpen(Reg, &Alpn, 1, &s, sizeof(s), nullptr, &config), "ConfigurationOpen");
	...
	if (is_server) {
		...
		quic_check(MsQuic->ConfigurationLoadCredential(config, &cred), "ConfigurationLoadCredential(server)");
	} else {
		...
		quic_check(MsQuic->ConfigurationLoadCredential(config, &cred), "ConfigurationLoadCredential(client)");
	}
	return config;
}
...
static QUIC_STATUS QUIC_API stream_recv_cb(HQUIC Stream, void* ctx, QUIC_STREAM_EVENT* ev) {
	...
	case QUIC_STREAM_EVENT_PEER_SEND_SHUTDOWN:
		...
		rc->ofs.close();
		cout << "[recv] file complete: " << rc->bytes_in << " bytes -> " << rc->outfile << '\n';
...
static QUIC_STATUS QUIC_API conn_recv_cb(HQUIC Conn, void* ctx, QUIC_CONNECTION_EVENT* ev) {
	...
	case QUIC_CONNECTION_EVENT_CONNECTED:
		cout << "[recv] client connected\n";
		MsQuic->ConnectionSendResumptionTicket(Conn, QUIC_SEND_RESUMPTION_FLAG_NONE, 0, nullptr);
		break;

	case QUIC_CONNECTION_EVENT_PEER_STREAM_STARTED: {
		HQUIC stream = ev->PEER_STREAM_STARTED.Stream;
		rc->ofs.open(rc->outfile, ios::binary | ios::trunc);
		if (!rc->ofs) {
			cerr << "[recv] cannot open output file: " << rc->outfile << '\n';
			MsQuic->ConnectionShutdown(Conn, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, 1);
			break;
		}
		cout << "[recv] stream started, writing to " << rc->outfile << '\n';
...
static QUIC_STATUS QUIC_API listener_cb(HQUIC, void* ctx, QUIC_LISTENER_EVENT* ev) {
	...
	if (ev->Type == QUIC_LISTENER_EVENT_NEW_CONNECTION) {
		...
		if (QUIC_FAILED(MsQuic->ConnectionSetConfiguration(conn, lc->config))) {
			cerr << "[recv] ConnectionSetConfiguration failed\n";
...
static void do_recv(const string& outfile, uint16_t port) {
	...
	quic_check(MsQuic->ListenerOpen(Reg, listener_cb, &lc, &listener), "ListenerOpen");
	...
	cout << "[recv] listening on port " << port << '\n';

	// Block main thread until the transfer completes
	unique_lock<mutex> lk(rc.mtx);
...
static QUIC_STATUS QUIC_API stream_send_cb(HQUIC Stream, void* ctx, QUIC_STREAM_EVENT* ev) {
	...
	case QUIC_STREAM_EVENT_SEND_COMPLETE:
		cout << "[send] data delivered to QUIC stack\n";
		break;

	case QUIC_STREAM_EVENT_SHUTDOWN_COMPLETE:
		// Stream fully closed — now close the connection
		MsQuic->StreamClose(Stream);
		MsQuic->ConnectionShutdown(sc->conn, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, 0);
		break;
...
static QUIC_STATUS QUIC_API conn_send_cb(HQUIC Conn, void* ctx, QUIC_CONNECTION_EVENT* ev) {
	...
	case QUIC_CONNECTION_EVENT_CONNECTED: {
		cout << "[send] connected — opening stream\n";
		HQUIC stream = nullptr;
		if (QUIC_FAILED(MsQuic->StreamOpen(Conn, QUIC_STREAM_OPEN_FLAG_UNIDIRECTIONAL, stream_send_cb, ctx, &stream))) {
			cerr << "[send] StreamOpen failed\n";
			MsQuic->ConnectionShutdown(Conn, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, 1);
			break;
		}
		if (QUIC_FAILED(MsQuic->StreamStart(stream, QUIC_STREAM_START_FLAG_NONE))) {
			cerr << "[send] StreamStart failed\n";
			MsQuic->StreamClose(stream);
			MsQuic->ConnectionShutdown(Conn, QUIC_CONNECTION_SHUTDOWN_FLAG_NONE, 1);
			break;
		}
		...
		cout << "[send] sending " << sc->data.size() << " bytes\n";
...
static void do_send(const string& infile, const string& host, uint16_t port) {
	...
	if (!ifs)
		throw runtime_error("cannot open: " + infile);
	auto sz = ifs.tellg();
	ifs.seekg(0);
	...
	cout << "[send] read " << sz << " bytes from " << infile << '\n';

	HQUIC config = make_config(false);

	quic_check(MsQuic->ConnectionOpen(Reg, conn_send_cb, &sc, &sc.conn), "ConnectionOpen");
	quic_check(MsQuic->ConnectionStart(sc.conn, config, QUIC_ADDRESS_FAMILY_UNSPEC, host.c_str(), port),
			   "ConnectionStart");

	unique_lock<mutex> lk(sc.mtx);
...
int main(int argc, char** argv) {
	try {
		...
		if (mode == "recv") {
			if (argc < 4)
				throw invalid_argument("recv needs <outfile> <port>");
			do_recv(argv[2], static_cast<uint16_t>(stoi(argv[3])));

		} else if (mode == "send") {
			if (argc < 5)
				throw invalid_argument("send needs <infile> <ip> <port>");
			do_send(argv[2], argv[3], static_cast<uint16_t>(stoi(argv[4])));
