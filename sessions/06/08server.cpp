...
void wait_for_client(const char* port) {
    ...
    do {
        ...
        int bytes_written = send(client_sock, "Hello, yourself!", 17, 0);
        if (bytes_written == -1) {
            close(client_sock);
...
int main(int argc, char* argv[]) {
    // I am the server, I don't have to specify a name. I do have to specify a port to listen on. I can use the default port 8080
