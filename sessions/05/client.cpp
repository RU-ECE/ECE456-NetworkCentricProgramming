...
void connect_to_server(const char* server_name, const char* port) {
	...
	if (int status; (status = getaddrinfo(server_name, port, &hints, &res)) != 0)
	...
	for (int message_num = 0; message_num < 5; message_num++) { // let's write to the server 5 times, just to harass it
		...
		int total = 0;
		while (total < n) {
			const int written = send(sock, p + total, n - total, 0);
			if (written == -1)
				cerr << "lost contact with socket total=" << total << endl;
			total += written;
		}
